...
{
    public partial class mscodes_am : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        cmd.Connection = conn;
                        string query = "INSERT INTO mscodesam (ztime,zid,xtype,xcode,xdescdet,xprops,xcodealt,zactive,zemail,xcodealt1) " +
                                       "VALUES(@ztime,@zid,@xtype,@xcode,@xdescdet,@xprops,@xcodealt,@zactive,@zemail,@xcodealt1) ";
                        ...
                        cmd.Parameters.AddWithValue("@xcodealt", xcodealt12);
                        cmd.Parameters.AddWithValue("@zactive", zactive1);
                        cmd.Parameters.AddWithValue("@zemail", zemail1);
                        cmd.Parameters.AddWithValue("@xcodealt1", xcodealt123);
                        cmd.ExecuteNonQuery();
            ...
            try
            {
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        SqlCommand cmd = new SqlCommand();
                        cmd.Connection = conn;
                        string query = "UPDATE mscodesam SET zutime=@zutime,xdescdet=@xdescdet,xprops=@xprops,xcodealt=@xcodealt,xemail=@xemail,zactive=@zactive,xcodealt1=@xcodealt1 " +
        ...
        protected void FillControls(object sender, EventArgs e)
        {
            try
            {
                ...
                xcodealt1.Text = dtztcode.Rows[0]["xcodealt1"].ToString();
        ...
        public void fnFillDataGrid()
        {
            ...
            dts.Reset();
            string str = "SELECT TOP " + _gridrow.Text.Trim().ToString() + " xtype,xcode,xdescdet,xprops,xcodealt1,xcodealt,zactive FROM mscodesam where xtype=@xtype and zid=@zid ORDER BY xcodealt"; //ORDER BY xtype,xcode
            SqlDataAdapter da = new SqlDataAdapter(str, conn1);
            int _zid = Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"]));
            da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
