...
{
    public partial class mssup : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xtrnsup.Text == "" || xtrnsup.Text == string.Empty || xtrnsup.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Supplier Prefix</li>";
                    isValid = false;
                }
                if (xorg.Text == "" || xorg.Text == string.Empty || xorg.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Company</li>";
                    isValid = false;
                }
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xsup"] == null)
                    {
                        ...
                        using (SqlConnection con = new SqlConnection(zglobal.constring))
                        {
                            using (DataSet dts = new DataSet())
                            {
                                con.Open();
                                string query =
                                    "SELECT * FROM mssup WHERE zid=@zid AND xorg=@xorg and xtrnsup=@xtrnsup";

                                SqlDataAdapter da = new SqlDataAdapter(query, con);

                                da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                da.SelectCommand.Parameters.AddWithValue("@xorg", xorg.Text.ToString().Trim());
                                da.SelectCommand.Parameters.AddWithValue("@xtrnsup", xtrnsup.Text.ToString().Trim());
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                               "INSERT INTO mssup (ztime,zid,xtrnsup,xsup,xorg,xfirst,xadd1,xbankref,xzip,xcountry,xlast,xdesig,xemail1,xphone,xmobile,xfax,xurl,xaccap," +
                               "xgsup,xtaxscope,xstatussup,xcrlimit,xcrterms,zemail) " +
                               "VALUES(@ztime,@zid,@xtrnsup,@xsup,@xorg,@xfirst,@xadd1,@xbankref,@xzip,@xcountry,@xlast,@xdesig,@xemail1,@xphone,@xmobile,@xfax,@xurl,@xaccap," +
                               "@xgsup,@xtaxscope,@xstatussup,@xcrlimit,@xcrterms,@zemail) ";
                            ...
                            cmd.Parameters.AddWithValue("@xurl", xurl1);
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xsupp == "")
                        {
                            ...
                            query = "UPDATE mssup SET zutime=@zutime,xemail=@xemail, xorg=@xorg, " +
                                    "xfirst=@xfirst,xadd1=@xadd1,xbankref=@xbankref,xzip=@xzip,xcountry=@xcountry,xlast=@xlast,xdesig=@xdesig," +
                                   "xemail1=@xemail1,xphone=@xphone," +
                                    "xmobile=@xmobile,xfax=@xfax,xurl=@xurl,xaccap=@xaccap,xgsup=@xgsup,xtaxscope=@xtaxscope,xstatussup=@xstatussup, " +
                                    "xcrlimit=@xcrlimit, xcrterms=@xcrterms " +
                                   "WHERE zid=@zid AND xsup=@xsup ";
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " xsup,xorg,xfirst,xadd1,xstatussup " +
                         "FROM mssup WHERE zid=@zid  " +
                         "order by xsup asc";
        ...
        private void fnFillControl(string xsup1)
        {
            ...
            string str = "SELECT  * " +
                         "FROM mssup where zid=@zid  and xsup=@xsup ";
            ...
            SqlDataAdapter da = new SqlDataAdapter(str, conn1);
            da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
            da.SelectCommand.Parameters.AddWithValue("@xsup", xsup1);
            da.Fill(dts, "tempztcode");
            ...
            {
                message.InnerHtml = "Wrong supplier code.";
                message.Style.Value = zglobal.am_warningmsg;
                return;
            }
            ...
            xtrnsup.Text = dttemp.Rows[0]["xtrnsup"].ToString();
            xorg.Text = dttemp.Rows[0]["xorg"].ToString();
            xfirst.Text = dttemp.Rows[0]["xfirst"].ToString();
            xadd1.Text = dttemp.Rows[0]["xadd1"].ToString();
            ...
            xurl.Text = dttemp.Rows[0]["xurl"].ToString();
                    ...
                    if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM mssup WHERE zid=@zid AND xsup=@xsup";
                                cmd.Parameters.Clear();
