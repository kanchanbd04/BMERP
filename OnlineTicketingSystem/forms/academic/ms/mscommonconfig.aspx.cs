...
{
    public partial class mscommonconfig : System.Web.UI.Page
    {
                ...
                if (txtconformmessageValue1.Value == "Yes")
                {
                    using (TransactionScope tran = new TransactionScope())
                    {
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query = "DELETE FROM amclasssubd WHERE zid=@zid AND xrow=@xrow";
                            cmd.Parameters.Clear();

                            cmd.CommandText = query;
                            cmd.Parameters.AddWithValue("@zid", _zid);
                            cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(GridView1.Rows[rowID].Cells[6].Text));
                            cmd.ExecuteNonQuery();

                            query = "DELETE FROM amclasssubex WHERE zid=@zid AND xrow=@xrow";
                            cmd.CommandText = query;
                            cmd.ExecuteNonQuery();

                            query = "DELETE FROM amclasssubh WHERE zid=@zid AND xrow=@xrow";
                            cmd.CommandText = query;
                            cmd.ExecuteNonQuery();
                        }
