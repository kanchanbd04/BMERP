    ...
    public partial class glyearend : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    xacc.Text = zglobal.fnGetValue("xaccyearend", "msdef",
                    "zid=" + _zid);

                }

                xacctitle.Text = zglobal.fnGetValue("xdesc", "glmst",
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xyear.Text == "" || xyear.Text == string.Empty || xyear.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Year</li>";
                    isValid = false;
                }

                if (xacc.Text == "" || xacc.Text == string.Empty || xacc.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Account</li>";
                    isValid = false;
                }

                if (xtrngl.Text == "" || xtrngl.Text == string.Empty || xtrngl.Text == "[Select]")
                {
                ...
                int nyear = 0 + Convert.ToInt32(xyear.Text.Trim().ToString()) + 1;

                //int offset;
                //int per;
                //int year = Convert.ToInt32(xyear.Text.Trim().ToString());
                ...
                //int lenper = tempper.ToString().Length;

                //string dt;

                //if (lenper == 2)
                //{
                //    dt = dt = nyear.ToString() + "/" + tempper.ToString() + "/01";
                //}
                //else
                //{
                //    dt = dt = nyear.ToString() + "/0" + tempper.ToString() + "/01";
                //}
                ...
                //    if (!yrper.Contains(s))
                //    {
                //        message.InnerHtml = "<span>Year and Period Must Match the Following Period:</span><br><span>" + yrper + "</span>" + " -----" + tempyear + "/" + tempper;
                //        message.Style.Value = zglobal.warningmsg;
                //        //globalerr = 1;
                //        return;
                //    }
                ...
                //else
                //{
                //    message.InnerHtml = "<span>Pointer mismatch.</span>";
                //    message.Style.Value = zglobal.warningmsg;
                //    //globalerr = 1;
                //    return;
                //}

                //string chktrn = zglobal.fnGetValue("xtrn", "mstrn",
                //    "zid=" + _zid + " and xtypetrn='GL Voucher' and xtrn='"+ xtrngl.Text +"'");
                //if (chktrn == "")
                //{
                //    message.InnerHtml = "<span>Wrong Transaction Type</span>";
                //    message.Style.Value = zglobal.warningmsg;
                //    //globalerr = 1;
                //    return;
                //}

                string chkacc = zglobal.fnGetValue("xacc", "glmst",
                    "zid=" + _zid + "  and xacc='" + xacc.Text.Trim().ToString() + "'");
                if (chkacc == "")
                {
                    message.InnerHtml = "<span>Invalid Transfer Account</span>";
                    message.Style.Value = zglobal.warningmsg;
                    //globalerr = 1;
                    return;
                }

                 string zemail123 = Convert.ToString(HttpContext.Current.Session["curuser"]);
            ...
            //DateTime xdate1 = xdate.Text.Trim() != string.Empty ? DateTime.ParseExact(xdate.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture) : DateTime.ParseExact("01/01/1900", "dd/MM/yyyy", CultureInfo.InvariantCulture);
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    string balacc = xacc.Text.Trim().ToString();
                    string balsub = "";

                    string oldvoucher = zglobal.fnGetValue("xvoucher", "glheader",
                      "zid=" + _zid + " AND xvoucher like '"+ xtrngl.Text +"%' and xyear="+nyear+" and xper=0");
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        string query =
                                "DELETE FROM gldetail  WHERE zid=@zid and xvoucher=@xvoucher";

                        cmd.Parameters.Clear();

                        cmd.CommandText = query;
                        cmd.Parameters.AddWithValue("@zid", _zid);
                        cmd.Parameters.AddWithValue("@xvoucher", oldvoucher);
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        string query =
                                "DELETE FROM glheader  WHERE zid=@zid and xvoucher=@xvoucher";
                    ...
                    string xvoucher_1 = zglobal.fnGetValue("xvoucher", "glheader",
                      "zid=" + _zid + " AND xvoucher like '" + xtrngl.Text + "%' order by xvoucher desc");

                    string last_num = "0";

                    if (xvoucher_1 != "")
                    {
                        last_num = xvoucher_1.Substring(4, 6);
                    }

                    last_num = (0 + 1 + Convert.ToInt32(last_num)).ToString().PadLeft(6,'0');

                    xvoucher_1 = xtrngl.Text + last_num;

                    int count = 0;
                    int i = 0;
                    int err = 0;

                    using (SqlConnection connection = new SqlConnection(zglobal.constring))
                    {
                        connection.Open();
                        string clist = "xyear,xacc,xsub,xdiv,xsec,xproj,xcur,xexch,xprime,xlineamt";
                        string sql = "select " + clist + " from glbalyear where zid='" + _zid + "' and xyear='" +
                                     xyear.Text.Trim().ToString() + "' ";
                        ...
                        temptbl = dts1.Tables["tempTable"];

                        if (temptbl.Rows.Count>0)
                        {
                            string myqry = "insert into glheader (ztime,zid, xvoucher,xref,xdate,xlong,xpostflag, " +
                            ...
                            myqry = myqry + " values('" + DateTime.Now + "','" + _zid + "','" + xvoucher_1 + "','','" +
                                    dt + "', " +
                                    "'Opening Balance through Year End Processing','Posted','" + nyear +
                                    "',0,'Balanced','" + DateTime.Now + "', " +
                                    "'','','','','','',0,'" + zemail123 + "','','','',0,'','','','" + xtrngl.Text +
                                    "','', " +
                                    "'','','','','Opening Balance')";
                            ...
                            foreach(DataRow sdr in temptbl.Rows)
                            {
                                using (DataSet dtsChkStatusDataSet = new DataSet())
                                {
                                    ...
                                    SqlDataAdapter daChkStatuSqlDataAdapter = new SqlDataAdapter(query1, connection);
                                    daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                    daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@xacc", sdr["xacc"].ToString());
                                ...
                                string mysql = "insert into gldetail (zid, xvoucher,xrow,xacc,xaccusage,xaccsource,xsub, " +
                                        "xdiv,xsec,xproj,xcur,xexch,xprime,xbase,xlong,xicacc,xicsub,xacctype,zemail,xemail, " +
                                        "xstatusrfp,xicdiv,xicsec,xicproj,xamount,xrem,xallocation,xcheque,xpaytype, " +
                                        "xstatus,xtypegl) ";
                                mysql = mysql + " values('" + _zid + "','" + xvoucher_1 + "','" + glrow + "','" +
                                        sdr["xacc"].ToString() + "','" + xaccusage1 + "', " +
                                        "'" + xaccsource1 + "','" + sdr["xsub"].ToString() + "','" +
                                        sdr["xdiv"].ToString() + "','" + sdr["xsec"].ToString() + "','" +
                                        sdr["xproj"].ToString() + "','" + sdr["xcur"].ToString() + "','" +
                                        sdr["xexch"].ToString() + "', " +
                                        "'" + sdr["xprime"].ToString() + "'," + sdr["xlineamt"].ToString() +
                                        ",'','','','" + xacctype1 + "','" + zemail123 + "','','','','','',0,'',0, " +
                                        "'','','','')";
                            ...
                            using (DataSet dtsChkStatusDataSet = new DataSet())
                            {
                                ...
                                SqlDataAdapter daChkStatuSqlDataAdapter = new SqlDataAdapter(query1, connection);
                                daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@xacc", balacc.ToString());
                                daChkStatuSqlDataAdapter.Fill(dtsChkStatusDataSet, "tempTable");
                                DataTable tempTableChkStatusDataTable = new DataTable();
                                tempTableChkStatusDataTable = dtsChkStatusDataSet.Tables["tempTable"];
                                if (tempTableChkStatusDataTable.Rows.Count > 0)
                            ...
                            using (DataSet dtsChkStatusDataSet = new DataSet())
                            {
                                string query1 = "SELECT sum(xprime*xexch) as xamount FROM gldetailview WHERE zid=@zid and xyear=@xyear and (xacctype='Income' or xacctype='Expenditure') and xstatusjv='Balanced' and xpostflag='Posted'";

                                SqlDataAdapter daChkStatuSqlDataAdapter = new SqlDataAdapter(query1, connection);
                                daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@xyear", xyear.Text.Trim().ToString());
                                daChkStatuSqlDataAdapter.Fill(dtsChkStatusDataSet, "tempTable");
                                DataTable tempTableChkStatusDataTable = new DataTable();
                                tempTableChkStatusDataTable = dtsChkStatusDataSet.Tables["tempTable"];
                                if (DBNull.Value.Equals(tempTableChkStatusDataTable.Rows[0]["xamount"])==false)
                                {
                                    balamt = balamt + Convert.ToDecimal(tempTableChkStatusDataTable.Rows[0]["xamount"]);
                                }
                            ...
                            string mysql1 =
                                "insert into gldetail (zid, xvoucher,xrow,xacc,xaccusage,xaccsource,xsub, " +
                                "xdiv,xsec,xproj,xcur,xexch,xprime,xbase,xlong,xicacc,xicsub,xacctype,zemail,xemail, " +
                                "xstatusrfp,xicdiv,xicsec,xicproj,xamount,xrem,xallocation,xcheque,xpaytype, " +
                                "xstatus,xtypegl) ";
                                
                            mysql1 = mysql1 + " values('" + _zid + "','" + xvoucher_1 + "','" + glrow + "','" + balacc +
                                    "','" + xaccusage1 + "', " +
                                    "'" + xaccsource1 + "','" + balsub + "','','','','BDT',1, " +
                                    balamt + "," + balamt + ",'','','','" + xacctype1 + "','" + zemail123 +
                                    "','','','','','',0,'',0, " +
                                    "'','','','')";
                            ...
                            cmd3.CommandText = mysql1;
                            cmd3.ExecuteNonQuery();

                            decimal tot_prime = 0;
                            using (DataSet dtsChkStatusDataSet = new DataSet())
                            {
                                string query1 = "SELECT sum(xprime) as xamount FROM gldetail WHERE zid=@zid and xvoucher=@xvoucher ";

                                SqlDataAdapter daChkStatuSqlDataAdapter = new SqlDataAdapter(query1, connection);
                                daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@xvoucher", xvoucher_1);
                                daChkStatuSqlDataAdapter.Fill(dtsChkStatusDataSet, "tempTable");
                                DataTable tempTableChkStatusDataTable = new DataTable();
                                tempTableChkStatusDataTable = dtsChkStatusDataSet.Tables["tempTable"];
                                if (DBNull.Value.Equals(tempTableChkStatusDataTable.Rows[0]["xamount"]) == false)
                                {
                                    tot_prime = tot_prime + Convert.ToDecimal(tempTableChkStatusDataTable.Rows[0]["xamount"]);
                                }
                            ...
                            if (tot_prime != 0)
                            {
                                message.InnerHtml = "<span>Cannot Update(" + xvoucher_1 + ")Out of Balance.</span>";
                                message.Style.Value = zglobal.warningmsg;
                                return;
                            }
                        ...
                        {
                            message.InnerHtml = "<span>Invalid Operation. No Row for Create Voucher.</span>";
                            message.Style.Value = zglobal.warningmsg;
                            return;
                        }
                    ...
                    message.InnerHtml = "</br><span class=bl>Voucher Number " + xvoucher_1 + " Created for Opening Balances for Year <font color=red>" + nyear + "</font></span>";
