...
{
    public partial class glpostvoucher : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    zglobal.fnGetComboDataCDTRN("'GL Voucher'", xftype);
                    zglobal.fnGetComboDataCDTRN("'GL Voucher'", xttype);
                    xfdatefilter.Text = DateTime.Now.ToString("dd/MM/yyyy");
                    xtdatefilter.Text = DateTime.Now.ToString("dd/MM/yyyy");
                    //list.Visible = false;
                    grdtitle.InnerHtml = "Showing All Unposted Voucher(s).";
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    {
                        query = "SELECT  xvoucher,xdate,xref,xyear,xper,xstatusjv,xpostflag," +
                                "(select xdesc from mstrn where zid=glheader.zid and xtrn=glheader.xtrngl and xtypetrn='GL Voucher') as xdesctrn  " +
                                "FROM glheader WHERE zid=@zid AND xpostflag='' " +
                                "order by xdate ";
                    }
                    else
                    {
                        query = "SELECT  xvoucher,xdate,xref,xyear,xper,xstatusjv,xpostflag," +
                                "(select xdesc from mstrn where zid=glheader.zid and xtrn=glheader.xtrngl and xtypetrn='GL Voucher') as xdesctrn  " +
                                "FROM glheader WHERE zid=@zid AND xpostflag='' AND " +
                                "xtrngl>=@xftype AND xtrngl<=@xttype AND xdate>=@xfdate AND xdate<=@xtdate " +
                                "order by xdate ";
                    }
            ...
            if (RadioButtonList1.Items.FindByValue("Show All").Selected)
            {
                ...
                {
                    grdtitle.InnerHtml = "No Unposted(s) Voucher Found.";
                }
            }
            else
            {
                if (flag == 1)
                {
                    grdtitle.InnerHtml = "Showing Unposted Voucher(s)- From Type: <b>" + xftype.Text + "</b>   To Type: <b>" + xttype.Text + "</b>   From Date: <b>" + xfdatefilter.Text + "</b>  To Date: <b>" + xtdatefilter.Text + "</b>";
                }
                else
                {
                    grdtitle.InnerHtml = "No Unposted Voucher(s) Found- From Type: <b>" + xftype.Text + "</b>   To Type: <b>" + xttype.Text + "</b>   From Date: <b>" + xfdatefilter.Text + "</b>  To Date: <b>" + xtdatefilter.Text + "</b>";
                }
