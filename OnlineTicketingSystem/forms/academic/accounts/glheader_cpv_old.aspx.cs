...
{
    public partial class glheader_cpv_old : System.Web.UI.Page
    {
        ...
        private void fnFillControl(string xrow123)
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         @"select xrow,xsub,(select case when xaccsource<>'None' then xdescsub + ' -(' + xacc + '-' + xdesc + ')' else xdescsub end from glmstvw 
