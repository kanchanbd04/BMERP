...
{
    public partial class imtemptrnsrtn : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                   ...
                   // xsession.Text = zglobal.fnDefaults("xsession", "Student");
                    ...
                    ViewState["xtrnim"] = "SRTN";
                    xtrnim.Text = ViewState["xtrnim"].ToString();

                    ViewState["xmode"] = "Sales Return";
                    ...
                    zglobal.fnGetComboDataCD("Warehouse", xtwh);
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dtCurrentTable.Rows.Count - 1; i++)
                    {
                        ...
                        TextBox box3 = (TextBox)GridView1.Rows[i].Cells[3].FindControl("xqty");
                        TextBox box4 = (TextBox)GridView1.Rows[i].Cells[4].FindControl("xunit");
                        TextBox box5 = (TextBox)GridView1.Rows[i].Cells[5].FindControl("xrate");
                        TextBox box6 = (TextBox)GridView1.Rows[i].Cells[6].FindControl("xremarks");
                        TextBox box7 = (TextBox)GridView1.Rows[i].Cells[8].FindControl("xcf");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    rtnMessage = rtnMessage + "<li>Student</li>";
                    isValid = false;
                }
                if (xtwh.Text == "" || xtwh.Text == string.Empty || xtwh.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Warehouse</li>";
                    isValid = false;
                }

                if (xref.Text == "" || xref.Text == string.Empty || xref.Text == "[Select]")
                {
                ...
                string isreffound = zglobal.fnGetValue("ximtmptrn", "imtemptrn",
                      "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + "  and coalesce(xstdid,'')='" + xstdid.Text.ToString().Trim() + "' and ximtmptrn='" + xref.Text.ToString().Trim() + "' and coalesce(xfwh,'')='" + xtwh.Text.ToString().Trim() + "'");
                ...
                string isrefdate = zglobal.fnGetValue("xdate", "imtemptrn",
                     "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + "  and ximtmptrn='" + xref.Text.ToString().Trim() + "'");

                //message.InnerHtml = Convert.ToDateTime(isrefdate).ToString() + " " + xdate111.ToString();
                //return;

                if (Convert.ToDateTime(isrefdate) > xdate111)
                {
                    message.InnerHtml = "Sales Return Date Cannot be Less then Issue Date.";
                    message.Style.Value = zglobal.warningmsg;
                    globalerr = 1;
                    return;
                }
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["ximtmptrn"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                              "INSERT INTO imtemptrn (ztime,zid,ximtmptrn,xtrnim,xref,xdate,xstatustrn,xtwh,xsign,xremarks,zemail,xaction,xmode,xyear,xper,xstdid) " +
                                    "VALUES(@ztime,@zid,@ximtmptrn,@xtrnim,@xref,@xdate,@xstatustrn,@xtwh,@xsign,@xremarks,@zemail,@xaction,@xmode,@xyear,@xper,@xstdid )";
                            ...
                            cmd.Parameters.AddWithValue("@xtwh", xtwh1);
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (ximtmptrn_1 == "")
                        {
                            ...
                            query = "UPDATE imtemptrn SET zutime=@zutime,xemail=@xemail, " +
                                   "xdate=@xdate, xtwh=@xtwh, xref=@xref, xremarks=@xremarks,xyear=@xyear,xper=@xper,xstdid=@xstdid " +
                        ...
                        foreach (GridViewRow row in GridView1.Rows)
                        {
                            ...
                            if (xitem1234.Text != string.Empty)
                            {
                                ...
                                //string xqtyiss = zglobal.fnGetValue("xqty", "imtrn",
                                //       "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + " and ximtrnnum='" + xref.Text.ToString().Trim() + "' and xitem='" + xitem1234.Text.Trim().ToString() + "'");

                                //string xvaliss = zglobal.fnGetValue("xval", "imtrn",
                                //      "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + " and ximtrnnum='" + xref.Text.ToString().Trim() + "' and xitem='" + xitem1234.Text.Trim().ToString() + "'");

                                //decimal xrateiss = decimal.Parse(xvaliss)/decimal.Parse(xqtyiss);
                                //xrate123 = xrateiss/xqty123;

                                string xratetemp = zglobal.fnGetValue("xrate", "imtrn",
                                      "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + " and ximtrnnum='" + xref.Text.ToString().Trim() + "' and xitem='" + xitem1234.Text.Trim().ToString() + "'");
                                xrate123 = decimal.Parse(xratetemp);
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " ximtmptrn,xdate,xtwh,xaction,xstatustrn,xstdid,(select xname from amadmis where zid=imtemptrn.zid and xstdid=imtemptrn.xstdid) as xname, xref " +
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                       "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " ximtmptrn,xdate,xtwh,xaction,xstatustrn,xstdid,(select xname from amadmis where zid=imtemptrn.zid and xstdid=imtemptrn.xstdid) as xname, xref " +
        ...
        private void fnFillControl(string xrow123)
        {
            ...
            string str = "SELECT  xtrnim,ximtmptrn,xdate,xsup,xtwh,xremarks,xstatustrn as xstatus,xref,xstdid " +
          ...
          xstdid.Text.ToString().Trim() + "' and xsession='" + zglobal.fnDefaults("xsessionacc", "Student") + "'  ");
            ...
            xtwh.Text = dttemp.Rows[0]["xtwh"].ToString();
                ...
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    if (tempTable.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in tempTable.Rows)
                        {
                            ...
                            dr["Column3"] = tempTable.Rows[i]["xqty"].ToString();
                            dr["Column4"] = tempTable.Rows[i]["xunit"].ToString();
                            dr["Column5"] = tempTable.Rows[i]["xrate"].ToString();
                            dr["Column6"] = tempTable.Rows[i]["xremarks"].ToString();
                            dr["Column7"] = tempTable.Rows[i]["xcf"].ToString();
                            dt.Rows.Add(dr);
