...
{
    public partial class imtemptrnadjr : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                   ...
                   // xsession.Text = zglobal.fnDefaults("xsession", "Student");
                    ...
                    ViewState["xtrnim"] = "AJR-";
                    xtrnim.Text = ViewState["xtrnim"].ToString();

                    ViewState["xmode"] = "Adjust Receive Entry";
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (xsup.Text == "" || xsup.Text == string.Empty || xsup.Text == "[Select]")
                //{
                //    rtnMessage = rtnMessage + "<li>Supplier No</li>";
                //    isValid = false;
                //}
                ...
                //string issupfound = zglobal.fnGetValue("xsup", "mssup",
                //      "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + "  and xsup='" + xsup.Text.ToString().Trim() + "'");

                //if (issupfound == "")
                //{
                //    message.InnerHtml = "Wrong Supplier Selected.";
                //    message.Style.Value = zglobal.warningmsg;
                //    return;
                //}
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["ximtmptrn"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                              "INSERT INTO imtemptrn (ztime,zid,ximtmptrn,xtrnim,xref,xdate,xstatustrn,xtwh,xsign,xremarks,zemail,xaction,xmode,xyear,xper) " +
                                    "VALUES(@ztime,@zid,@ximtmptrn,@xtrnim,@xref,@xdate,@xstatustrn,@xtwh,@xsign,@xremarks,@zemail,@xaction,@xmode,@xyear,@xper )";
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (ximtmptrn_1 == "")
                        {
                            ...
                            query = "UPDATE imtemptrn SET zutime=@zutime,xemail=@xemail, " +
                                   "xdate=@xdate, xtwh=@xtwh, xref=@xref, xremarks=@xremarks,xyear=@xyear,xper=@xper " +
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " ximtmptrn,xdate,xtwh,xaction,xstatustrn " +
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                       "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " ximtmptrn,xdate,xtwh,xaction,xstatustrn " +
        ...
        private void fnFillControl(string xrow123)
        {
            ...
            string str = "SELECT  xtrnim,ximtmptrn,xdate,xsup,xtwh,xremarks,xstatustrn as xstatus,xref " +
