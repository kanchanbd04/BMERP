...
{
    public partial class imconfirmtran : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    RadioButtonList1.Items.FindByValue("Show All").Selected = true;
                    listfilter.Visible = false;

                    zglobal.fnGetComboDataCDTRN("'Im Transaction','Transfer Transaction'", xftype);
                    zglobal.fnGetComboDataCDTRN("'Im Transaction','Transfer Transaction'", xttype);
                    xfdatefilter.Text = DateTime.Now.ToString("dd/MM/yyyy");
                    xtdatefilter.Text = DateTime.Now.ToString("dd/MM/yyyy");
                    //list.Visible = false;
                    grdtitle.InnerHtml = "Showing All Open Transection(s).";
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //    if (ViewState["xrow"] != null)
            //    {
            //        xstatus2 = zglobal.fnGetValue("xstatus", "amdropout",
            //              "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
            ...
            //                xrow = zglobal.GetMaximumIdInt("xrow", "amdropout", " zid=" + _zid, 1, 1);
            //                ViewState["xrow"] = xrow;
            ...
            //                query = "UPDATE amdropout SET zutime=@zutime,xemail=@xemail, " +
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query = "";
                    //"SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " xrow,xsession, xname,  xstdid, xclass, xgroup, xsection, " +
                    //"xtype, xreason, xstatus, xdate,xdateacc " +
                    //"FROM amdropout WHERE zid=@zid AND coalesce(xstatus1,'')='' order by xrow desc";

                    //  "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " xvoucher,xdate,xref,xyear,xper,xstatusjv,xpostflag," +
                    //  "(select xdesc from mstrn where zid=glheader.zid and xtrn=glheader.xtrngl and xtypetrn='GL Voucher') as xdesctrn  " +
                    //"FROM glheader WHERE zid=@zid AND xpostflag='' " +
                    //"order by xdate desc";

                    if (RadioButtonList1.Items.FindByValue("Show All").Selected)
                    {
                        query = "SELECT  ximtmptrn,xdate,xfwh,xtwh,xaction,xstatustrn, " +
                                "(select xdesc from mstrn where zid=imtemptrn.zid and xtrn=imtemptrn.xtrnim and xtypetrn in ('Im Transaction','Transfer Transaction')) as xdesctrn  " +
                                "FROM imtemptrn WHERE zid=@zid AND xstatustrn='Open' " +
                                "order by xdate ";
                    }
                    else
                    {
                        query = "SELECT  ximtmptrn,xdate,xfwh,xtwh,xaction,xstatustrn, " +
                                "(select xdesc from mstrn where zid=imtemptrn.zid and xtrn=imtemptrn.xtrnim and xtypetrn in ('Im Transaction','Transfer Transaction')) as xdesctrn  " +
                                "FROM imtemptrn WHERE zid=@zid AND xstatustrn='Open' AND " +
                                "xtrnim>=@xftype AND xtrnim<=@xttype AND xdate>=@xfdate AND xdate<=@xtdate " +
                                "order by xdate ";
                    }

                    int year = int.Parse(DateTime.ParseExact(xfdatefilter.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture).Year.ToString());
                    int month = int.Parse(DateTime.ParseExact(xfdatefilter.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture).Month.ToString());
                    int day = int.Parse(DateTime.ParseExact(xfdatefilter.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture).Day.ToString());

                    int year1 = int.Parse(DateTime.ParseExact(xtdatefilter.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture).Year.ToString());
                    int month1 = int.Parse(DateTime.ParseExact(xtdatefilter.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture).Month.ToString());
                    int day1 = int.Parse(DateTime.ParseExact(xtdatefilter.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture).Day.ToString());

                    SqlDataAdapter da1 = new SqlDataAdapter(query, con);

                    da1.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                    da1.SelectCommand.Parameters.AddWithValue("@xftype", xftype.Text.Trim().ToString());
                    da1.SelectCommand.Parameters.AddWithValue("@xttype", xttype.Text.Trim().ToString());
                    da1.SelectCommand.Parameters.AddWithValue("@xfdate", new DateTime(year, month, day));
                    da1.SelectCommand.Parameters.AddWithValue("@xtdate", new DateTime(year1, month1, day1));
            ...
            if (RadioButtonList1.Items.FindByValue("Show All").Selected)
            {
                ...
                {
                    grdtitle.InnerHtml = "No Open Transection(s) Found.";
                }
            }
            else
            {
                if (flag == 1)
                {
                    grdtitle.InnerHtml = "Showing All Open Transection(s)- From Type: <b>" + xftype.Text + "</b>   To Type: <b>" + xttype.Text + "</b>   From Date: <b>" + xfdatefilter.Text + "</b>  To Date: <b>" + xtdatefilter.Text + "</b>";
                }
                else
                {
                    grdtitle.InnerHtml = "No Open Transection(s) Found- From Type: <b>" + xftype.Text + "</b>   To Type: <b>" + xttype.Text + "</b>   From Date: <b>" + xfdatefilter.Text + "</b>  To Date: <b>" + xtdatefilter.Text + "</b>";
                }
            ...
            //            "xtype, xreason, xstatus, xdate,xdateacc " +
            //            "FROM amdropout WHERE zid=@zid AND coalesce(xstatus1,'')='Approved' order by xrow desc";
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                //             "(select xgroup from amstudentvw where zid=amdropout.zid and xsession=amdropout.xsession and xrow=amdropout.xsrow) as xgroup, " +
                //             "(select xsection from amstudentvw where zid=amdropout.zid and xsession=amdropout.xsession and xrow=amdropout.xsrow) as xsection, " +
                //             "(select xfname from amstudentvw where zid=amdropout.zid and xsession=amdropout.xsession and xrow=amdropout.xsrow) as xfname, " +
                //             "(select xmname from amstudentvw where zid=amdropout.zid and xsession=amdropout.xsession and xrow=amdropout.xsrow) as xmname, " +
                //             "xtype,xreason,xtccertificate,xmarksheet,xcharcertificate,xreference,xdate,xstatus,zemail,xapprovedby,xremarks " +
                //             " FROM amdropout where zid=@zid  and xrow=@xrow ";


                string str = "SELECT  xsession, xstdid, xname, xclass, xgroup, xsection, " +
                             "(select xfname from amadmis where zid=amdropout.zid and xrow=amdropout.xsrow) as xfname, " +
                             "(select xmname from amadmis where zid=amdropout.zid and xrow=amdropout.xsrow) as xmname, " +
                             "xtype,xreason,xtccertificate,xmarksheet,xcharcertificate,xreference,xdate,xstatus,zemail,xapprovedby,xremarks " +
                             "FROM amdropout where zid=@zid  and xrow=@xrow ";
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            //                    string query = "DELETE FROM amdropout WHERE zid=@zid AND xrow=@xrow";
            //                    cmd.Parameters.Clear();
        ...
        protected void btnOk_OnClick(object sender, EventArgs e)
        {
            ...
            //        //message.InnerHtml = rowID.ToString();

            //        //message.InnerHtml = ((HyperLink)gvRow.FindControl("xvoucher")).Text ;

            //        string xvoucher1 = ((HyperLink)gvRow.FindControl("xvoucher")).Text;

            //        string drtot = zglobal.fnGetValue("sum(xbase)", "gldetail",
            //                "zid=" + _zid + " and xvoucher='" + xvoucher1 + "'");
            ...
            //                cmd.CommandText = query;
            //                cmd.Parameters.AddWithValue("@zid", _zid);
            //                cmd.Parameters.AddWithValue("@xvoucher", xvoucher1);
        ...
        protected void btnOk1_OnClick(object sender, EventArgs e)
        {
            //try
            //{
            //    if (txtconformmessageValue1.Value == "Yes")
            ...
            //            foreach (GridViewRow row in dgvunposted.Rows)
            //            {
            //                if (((CheckBox)row.FindControl("chkvoucher")).Checked)
            //                {
            //                    //message.InnerHtml = message.InnerHtml+ " " + ((HyperLink)row.FindControl("xvoucher")).Text;
            //                    string xvoucher1 = ((HyperLink)row.FindControl("xvoucher")).Text;
            ...
            //                    if (drtot == "")
            //                    {
            //                        message.InnerHtml = "Voucher: " + xvoucher1 + ", Detail Has No Record. Can't Post.";
            //                        message.Style.Value = zglobal.warningmsg;
            //                        return;
            //                    }
            //                    else if (Convert.ToDecimal(drtot) != 0)
            //                    {
            //                        message.InnerHtml = "Voucher: " + xvoucher1 + ", Out of Balance Voucher. Can't Post.";
            //                        message.Style.Value = zglobal.warningmsg;
            //                        return;
            //                    }
            //                    else
