...
{
    public partial class imitemsetdt : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    spanxclass.InnerHtml = xclass1;
                    spanxsetname.InnerHtml = xsetname1;

                    xclass.Text = xclass1;
                    xsetname.Text = xsetname1;
                ...
                tmpdesc.Text = zglobal.fnGetValue("xdesc", "msitem",
               "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + " and xitem='" + xitem.Text.ToString().Trim() + "' ");
        ...
        public void fnRegisterPostBack()
        {
            ...
            {
                LinkButton lnkFull1 = row.FindControl("xitem") as LinkButton;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xqty.Text == "" || xqty.Text == string.Empty || xqty.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Quantity</li>";
                    isValid = false;
                }
                ...
                if (xitem.Text.Trim().ToString() != "")
                {
                    string tmpchk = zglobal.fnGetValue("xitem", "msitem",
                        "zid=" + _zid + "  and xitem='" + xitem.Text.ToString().Trim() + "'");

                    if (tmpchk == "")
                    {
                        message.InnerHtml = "Not a valid Item.";
                        message.Style.Value = zglobal.warningmsg;
                        return;
                    }
                }
                ...
                //        "zid=" + _zid + "  and xpaycode='" + xpcode.Text.ToString().Trim() + "'");
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts = new DataSet())
                    {
                        con.Open();
                        string query =
                            "SELECT * FROM imitemsetdt WHERE zid=@zid AND xclass=@xclass and xsetname=@xsetname and xitem=@xitem ";
                        ...
                        da.SelectCommand.Parameters.AddWithValue("@xitem", xitem.Text.Trim().ToString());
                        ...
                        if (tempTable.Rows.Count > 0)
                        {
                            ViewState["xpaycode"] = xitem.Text.Trim().ToString();
                        }
                        else
                        {
                            ViewState["xpaycode"] = null;
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xpaycode"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                               "INSERT INTO imitemsetdt (zid,ztime,xclass,xsetname,xitem,xqty,zemail) " +
                               "VALUES(@zid,@ztime,@xclass,@xsetname,@xitem,@xqty,@zemail) ";
                            ...
                            ViewState["xpaycode"] = xitem.Text.Trim().ToString();
                            xsupp = xitem.Text.Trim().ToString();
                            ...
                            xclass1 = xclass.Text.ToString();
                            xqty1 = Convert.ToDecimal(xqty.Text.ToString().Trim());
                            ...
                            cmd.Parameters.AddWithValue("@xitem", xitem.Text.ToString().Trim());
                            cmd.Parameters.AddWithValue("@xqty", xqty1);
                            cmd.ExecuteNonQuery();
                        }
                    }
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xsupp == "")
                        {
                            ...
                            query = "UPDATE imitemsetdt SET zutime=@zutime,xqty=@xqty, xemail=@xemail " +
                                    "WHERE zid=@zid AND xclass=@xclass and xsetname=@xsetname and xitem=@xitem  ";
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " *,(select xdesc from msitem where zid=imitemsetdt.zid and xitem=imitemsetdt.xitem) as xdesc " +
                       "FROM imitemsetdt WHERE zid=@zid and xclass=@xclass and xsetname=@xsetname " +
                       "order by zid,xclass,xsetname,xitem ";

                    SqlDataAdapter da1 = new SqlDataAdapter(query, con);

                    da1.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                    da1.SelectCommand.Parameters.AddWithValue("@xclass", xclass1);
                    da1.SelectCommand.Parameters.AddWithValue("@xsetname", xsetname1);
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                string xsetname1 = Request.QueryString["xsetname"] != null ? Request.QueryString["xsetname"].ToString().Trim() : "";
                //xyear1 = Request.QueryString["xyear"] != null ? Request.QueryString["xyear"].ToString().Trim() : "";
                string xitem1 = ((LinkButton)sender).Text;

                fnFillControl(xclass1, xsetname1,xitem1);
        ...
        private void fnFillControl(string xclass1, string xsetname1,string xitem1)
        {
            message.InnerHtml = "";
            ...
            string str = "SELECT  * " +
                         "FROM imitemsetdt where zid=@zid  and xclass=@xclass and xsetname=@xsetname and xitem=@xitem ";
                    ...
                    if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM imitemsetdt WHERE zid=" + _zid + " AND xclass='" + xclass1 + "' and xsetname='"+ xsetname1 +"' and xitem='" + xitem.Text.Trim().ToString() + "'";
                                cmd.Parameters.Clear();
        ...
        protected void btnBack_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string url =
                         string.Format(
                            "~/forms/academic/inventory/imitemset.aspx?subnav=myDropdown16&link=LinkButton16&xid=175005010&xclass={0}&xsetname={1}",
                            xclass1, xsetname1);
