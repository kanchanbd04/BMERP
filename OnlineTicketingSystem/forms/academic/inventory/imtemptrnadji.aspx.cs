...
{
    public partial class imtemptrnadji : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                   ...
                   // xsession.Text = zglobal.fnDefaults("xsession", "Student");
                    ...
                    ViewState["xtrnim"] = "AJIS";
                    xtrnim.Text = ViewState["xtrnim"].ToString();

                    ViewState["xmode"] = "Adjust Issue Entry";
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["ximtmptrn"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                              "INSERT INTO imtemptrn (ztime,zid,ximtmptrn,xtrnim,xref,xdate,xstatustrn,xfwh,xsign,xremarks,zemail,xaction,xmode,xyear,xper) " +
                                    "VALUES(@ztime,@zid,@ximtmptrn,@xtrnim,@xref,@xdate,@xstatustrn,@xfwh,@xsign,@xremarks,@zemail,@xaction,@xmode,@xyear,@xper )";
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (ximtmptrn_1 == "")
                        {
                            ...
                            query = "UPDATE imtemptrn SET zutime=@zutime,xemail=@xemail, " +
                                   "xdate=@xdate, xfwh=@xfwh, xref=@xref, xremarks=@xremarks,xyear=@xyear,xper=@xper " +
        ...
        private void fnFillControl(string xrow123)
        {
            ...
            string str = "SELECT  xtrnim,ximtmptrn,xdate,xsup,xfwh,xremarks,xstatustrn as xstatus,xemp,xstdid,xref " +
