...
{
    public partial class amtfcconfig1 : System.Web.UI.Page
    {
...
//                }
        ...
        private void BindGrid()
        {
            ...
            //xdistype.Text = zglobal.fnGetValue("top 1 xdistype", "amtfcconfig",
            //           "zid=" + _zid + " AND xsrow=" + xsrow1 + " and xsession='" + xsession.Text + "' and xeffdate='" + xeffdate1 + "'");
                ...
                //{
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        using (DataSet dts1 = new DataSet())
                        {
                            string query = "SELECT * FROM amtfcconfig WHERE zid=@zid AND xsrow=@xsrow and xsession=@xsession and xeffdate=@xeffdate";
                            SqlDataAdapter da1 = new SqlDataAdapter(query, conn);
           ...
           // }
            ...
            //{
              ...
              str = zglobal.fnxtfcdefault4("Admission");
              xrow1 = 0;
            //}
            ...
            if (dtmarks.Rows.Count > 0)
            {
                ...
                tfield3.HeaderText = "Discount Type";
        ...
        protected void OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    //dpDisType.AutoPostBack = true;
                    //dpDisType.SelectedIndexChanged += dpDisType_SelectedIndexChanged;
                    e.Row.Cells[8].Controls.Add(dpType);

                    DropDownList discounttype = new DropDownList();
                    discounttype.ID = "discounttype";

                    zglobal.fnGetComboDataCD("Discount Type", discounttype);
                    //dpDisType.Items.Add("%");
                    //dpDisType.Items.Add("Fixed");
                    //dpDisType.AutoPostBack = true;
                    //dpDisType.SelectedIndexChanged += dpDisType_SelectedIndexChanged;
                    e.Row.Cells[9].Controls.Add(discounttype);
                    ...
                    if (ViewState["amtfcd"] != null)
                    {
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            if (row["xtfccode"].ToString() == e.Row.Cells[0].Text.ToString())
                            {
                                ...
                                discounttype.Text = row["xdistype"].ToString();
                        ...
                        if (f == 0)
                        {
                            ...
                            //discounttype.Text = row["xdistype1"].ToString();
                        }
                    }
                    else
                    {
                        ...
                        dpType.Text = (e.Row.DataItem as DataRowView).Row["xtype1"].ToString();
                        //discounttype.Text = xdistype.Text;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xdistype.Text == "" || xdistype.Text == string.Empty || xdistype.Text == "[Select]")
                {
                ...
                foreach (GridViewRow row in GridView1.Rows)
                {
                    DropDownList discounttype123 = row.FindControl("discounttype") as DropDownList;

                    if (discounttype123.Text == String.Empty || discounttype123.Text == "")
                    {
                        discounttype123.Text = xdistype.Text;
                    }

                     TextBox txtTextBox1 = row.FindControl("xdiscount") as TextBox;
                     decimal xdiscount123;
                     ...
                     {
                         xdiscount123 = decimal.Parse(txtTextBox1.Text.Trim().ToString());
                     }

                     if (discounttype123.Text == "Default Amount" && xdiscount123!=0)
                     {
                ...
                if (flag == 1)
                {
                    message.InnerHtml = "<span style=\"text-size:16px;\" >Discounts cannot be given on Default Amount.</span>";
                    message.Style.Value = zglobal.warningmsg;
                    return;
                }
               ...
               : 0;
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        foreach (GridViewRow row in GridView1.Rows)
                        {
                            query = "DELETE FROM amtfcconfigt WHERE zid=@zid AND " +
                                    "xrow=(select xrow from amtfcconfig WHERE zid=@zid AND xsrow=@xsrow and xsession=@xsession and xeffdate=@xeffdate and xtfccode=@xtfccode)";
                            ...
                            query = "DELETE FROM amtfcconfig WHERE zid=@zid AND xsrow=@xsrow and xsession=@xsession and xeffdate=@xeffdate and xtfccode=@xtfccode";
                            cmd.Parameters.Clear();
                        ...
                        foreach (GridViewRow row in GridView1.Rows)
                        {
                            ...
                            DropDownList discounttype1 = row.FindControl("discounttype") as DropDownList;
                            ...
                            int row111 = zglobal.GetMaximumIdInt("xrow", "amtfcconfig", " zid=" + _zid, 1, 1);
                            ViewState["xrow"] = row111;

                            query =
                                     "INSERT INTO amtfcconfig (ztime,zid,xrow,xtfccode,xclass,xgroup,xsection,xstdid,xamount,xdisfixed,xdisperc,xvatfixed,xvatperc,xsign,xeffdate,zactive,xstatus,xremarks,zemail,xsrow,xtype1,xsession,xdistype,xdistype1) " +
                                     "VALUES(@ztime,@zid,@xrow,@xtfccode,@xclass,'','',@xstdid,@xamount,@xdisfixed,@xdisperc,0,0,1,@xeffdate,1,'Submited',@xremarks,@zemail,@xsrow,@xtype1,@xsession,@xdistype,@xdistype1) ";
                            ...
                            cmd.Parameters.AddWithValue("@xsession", xsession.Text);
                            cmd.Parameters.AddWithValue("@xsrow", xsrow1);
                            cmd.Parameters.AddWithValue("@xdistype", discounttype1.Text == String.Empty || discounttype1.Text == "" ? xdistype.Text : discounttype1.Text);
                            cmd.Parameters.AddWithValue("@xdistype1", xdistype.Text.Trim().ToString());
                            ...
                            //{
                                ...
                                {
                                    //int xline = zglobal.GetMaximumIdInt("xline", "amtfcconfigt", " zid=" + _zid + " and xrow=" + xrow111, "line");
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                         ...
                         "(select xname from amadmis where zid=amtfcconfig.zid and xrow=amtfcconfig.xsrow) as xname,coalesce(xdistype1,'-') as xdistype1 " +
                         "FROM amtfcconfig WHERE zid=@zid AND xstdid=@xstdid AND coalesce(xsession,'')<>''" +
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                        ...
                        "(select xname from amadmis where zid=amtfcconfig.zid and xrow=amtfcconfig.xsrow) as xname,coalesce(xdistype1,'-') as xdistype1 " +
                        "FROM amtfcconfig WHERE zid=@zid AND coalesce(xstdid,'')<>'' AND coalesce(xsession,'')<>''" +
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                xdistype.Text = row.Cells[6].Text == "-" ? "" : row.Cells[6].Text.Trim().ToString();
        ...
        protected void xcdate_OnTextChanged(object sender, EventArgs e)
        {
          ...
          //      }
                ...
                int day = Int32.Parse(date[0]);
                DateTime xeffdate1 = new DateTime(year, month, day);
                xdistype.Text = zglobal.fnGetValue("top 1 xdistype1", "amtfcconfig",
                           "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) +
                           " AND xsrow=(select xrow from amadmis where zid=amtfcconfig.zid and xstdid='"+xstdid.Text.Trim().ToString() +"') and xsession='" + xsession.Text + "' and xeffdate='" + xeffdate1 + "'");
