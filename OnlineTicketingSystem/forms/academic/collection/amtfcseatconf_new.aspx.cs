...
{
    public partial class amtfcseatconf_new : System.Web.UI.Page
    {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                         ...
                         "FROM amtfch WHERE zid=@zid AND xtype in ('Seat Conf','Due Rcv') AND xstatus='New' " +
                         "order by xrow desc";
                ...
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                       "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " xrow,xsession,xcdate, " +
                         ...
                         "(select xstdid from amadmis where zid=amtfch.zid and xrow=amtfch.xsrow) as xstdid, " +
                         "(select xname from amadmis where zid=amtfch.zid and xrow=amtfch.xsrow) as xname " +
                         "FROM amtfch WHERE zid=@zid AND xtype in ('Seat Conf','Due Rcv') AND xstatus='Paid' " +
                         "order by xrow desc";
