...
{
    public partial class amtfcdue2 : System.Web.UI.Page
    {
       ...
       xstdid.Text.ToString().Trim() + "' ");
        ...
        private void BindGrid()
        {
            ...
            str = @"select h.zid,h.xrow,h.xtype,h.xsession,h.xcdate,d.xline,d.xsrow,d.xtfccode,d.xamount,d.xclassd,d.xupdflag,
                    (select xdescdet from mscodesam where zid=h.zid and xtype='TFC Code' and xcode=d.xtfccode) as xdescdet, d.xdisfixed
                    from 
                    amtfcdueh as h inner join amtfcdued as d on h.zid=d.zid and h.xrow=d.xrow
                    where xtype not in ('Auto Generated') and h.zid=@zid and xsession=@xsession and d.xsrow=(select xrow from amadmis where zid=@zid and xstdid=@xstdid) 
                    order by h.xcdate,d.xtfccode";
            ...
            if (dtmarks.Rows.Count > 0)
            {
                ...
                bfield = new BoundField();
                bfield.HeaderText = "Charge Period";
                bfield.DataField = "xcdate";
                bfield.ItemStyle.Width = 100;
                bfield.ItemStyle.HorizontalAlign = HorizontalAlign.Center;
                bfield.DataFormatString = "{0:MMM-yyyy}";
                GridView1.Columns.Add(bfield);
                ...
                tfield = new TemplateField();
                tfield.HeaderText = "Discount Amount";
                tfield.ItemStyle.HorizontalAlign = HorizontalAlign.Right;
                tfield.ItemStyle.Width = 120;
                GridView1.Columns.Add(tfield);
        ...
        protected void OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    e.Row.Cells[4].Controls.Add(xamtdue1);

                    xamtdue1.Text = DataBinder.Eval(e.Row.DataItem, "xamount").ToString();

                    TextBox xdisfixed1 = new TextBox();
                    xdisfixed1.ID = "xdisfixed";
                    xdisfixed1.CssClass = "bm_academic_textbox_grid bm_academic_ctl_global bm_academic_textbox right xamtdue";
                    xdisfixed1.Attributes.Add("onkeyup", "enter(this,event)");
                    e.Row.Cells[5].Controls.Add(xdisfixed1);

                    xdisfixed1.Text = DataBinder.Eval(e.Row.DataItem, "xdisfixed").ToString();
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        foreach (GridViewRow row in GridView1.Rows)
                        {
                            ...
                            TextBox xreceived1 = row.FindControl("xamtdue") as TextBox;
                            ...
                            int xrow123 = Convert.ToInt32(row.Cells[7].Text.ToString());
                            int xline123 = Convert.ToInt32(row.Cells[8].Text.ToString());

                            query = "UPDATE amtfcdued SET xamount=@xamount, xamtdue=@xamtdue, xdisfixed=@xdisfixed, xdiscount=@xdiscount, xupdflag='Yes' " +
                            ...
                            cmd.Parameters.AddWithValue("@xline", xline123);
                            cmd.Parameters.AddWithValue("@xamount", xreceived12);
                            cmd.Parameters.AddWithValue("@xamtdue", (xreceived12 - xdisfixed123));
