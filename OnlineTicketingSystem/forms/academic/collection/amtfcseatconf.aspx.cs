...
{
    public partial class amtfcseatconf : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    xsessioncur.Text = zglobal.fnDefaults("xsessionaccseatconf", "Student");

                    classpro.Visible = false;
                }
         ...
         xstdid.Text.ToString().Trim() + "' ");
             ...
             xstdid.Text.ToString().Trim() + "' and xsession='" + xsessioncur.Text.ToString().Trim() + "'  ");
                ...
                //if (classpro.Visible == true)
                //{
                //    if (xclasspro.Text == xclasscur.Text)
                //    {
                //        xclasspro.Text = "Not Promoted";
                //    }
                //}
                //       string xclasspro1 = zglobal.fnGetValue("xclasspro", "amstudentclassconfvw1",
         //"zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + " and xstdid='" +
         //xstdid.Text.ToString().Trim() + "' and xsession='" + xsessioncur.Text.ToString().Trim() + "'");

         //       if (xclasspro1 != "")
         //       {
         //           classpro.Visible = true;
         //           classpro.Visible = true;
         //           if (xclasscur.Text == xclasspro1)
         //           {
         //               xclasspro.Text = "Not Promoted.";
         //           }
         //           else
         //           {
         //               xclasspro.Text = xclasspro1;
         //           }
...
//                }
        ...
        private void BindGrid()
        {
            ...
            if (dtmarks.Rows.Count > 0)
            {
                ...
                //Div3.InnerHtml = xsrow1.ToString()+ "<br/>";
                using (SqlConnection conn = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
        ...
        protected void OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    //if (ViewState["amtfcspecialdis"] != null && ViewState["xrow"] == null)
                    ...
                    else
                    {
                        ...
                        //    dpDisType.Text = (e.Row.DataItem as DataRowView).Row["xspecialdistype"].ToString();
                        //    txtDiscount.Text = (e.Row.DataItem as DataRowView).Row["xspecialdis"].ToString();
                        //    e.Row.Cells[5].Text = (e.Row.DataItem as DataRowView).Row["xnetamount"].ToString();
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //string isStudentFound1 = zglobal.fnGetValue("xname", "amstudentvwextacc",
                //  "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + " and xstdid='" +
                //  xstdid.Text.ToString().Trim() + "' and xsession='" + xsession.Text.ToString().Trim() + "'");
         ...
         xstdid.Text.ToString().Trim() + "' and xsession='" + xsessioncur.Text.ToString().Trim() + "'  and zactiveacc=1");
                ...
                string isStudentFound1 = zglobal.fnGetValue("xname", "amstudentvwext",
      "zid=" + Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"])) + " and xstdid='" +
      xstdid.Text.ToString().Trim() + "' and xsession='" + xsession.Text.ToString().Trim() + "'  and zactive=0");

                if (isStudentFound1 != "" && RadioButtonList1.SelectedItem.Value == "Seat Confirmation")
                {
                    message.InnerHtml = "Student not found.";
                    message.Style.Value = zglobal.warningmsg;
                    xstdname.Text = "";
                ...
                if (RadioButtonList1.SelectedItem.Value == "Seat Confirmation")
                {
                    ViewState["xtype"] = "Seat Conf";
                }
                else if (RadioButtonList1.SelectedItem.Value == "Hold")
                {
                    ViewState["xtype"] = "Hold";
                }
                else
                {
                    ViewState["xtype"] = "Due Rcv";
                }
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        using (SqlConnection con = new SqlConnection(zglobal.constring))
                        {
                            using (DataSet dts = new DataSet())
                            {
                                con.Open();
                                query =
                                    "select * from ampromotionh as h inner join ampromotiond as d on h.zid=d.zid and h.xrow=d.xrow " +
                                    "WHERE h.zid=@zid AND xsessionpro=@xsession AND xsrow=(select xrow from amadmis where zid=@zid and xstdid=@xstdid) and xstatus='Approved'";
                        ...
                        query = "INSERT INTO amtfcseatconf (zid,xrow,xsession,xstdid,xclass,xgroup,xsection,xsessionconf,xclassconf,xgroupconf,xsectionconf,xstatusconf,xstatus) " +
                                   "VALUES(@zid,@xrow,@xsession,@xstdid,@xclass,@xgroup,@xsection,@xsessionconf,@xclassconf,@xgroupconf,@xsectionconf,@xstatusconf,@xstatus) ";
                        ...
                        cmd.Parameters.AddWithValue("@xstatus", ViewState["xtype"].ToString());
                        cmd.ExecuteNonQuery();
                        ...
                        if (xrow == 0)
                        {
                            ...
                            query = "UPDATE amtfch SET zutime=@zutime,xemail=@xemail, xsession=@xsession, " +
                                   ...
                                   "xtdate=@xtdate,xbank=@xbank,xchqdate=@xchqdate, xchqno=@xchqno, xremarks=@xremarks,xtype=@xtype, " +
                                    "xclass=@xclass, xadvreceive=@xadvreceive " +
                                   "WHERE zid=@zid AND xrow=@xrow ";
                            //(select xclass from amstudentvwextacc where zid=@zid and xstdid=@xstdid and xsession=@xsession)
        ...
        private void fnFillControl(string xrow123)
        {
            ...
            string str = "SELECT  xrow,xsession, " +
                         ...
                         "xcdate,xtdate,xremarks,xstatus,xbank,xchqdate,xchqno,xtype,coalesce(xadvreceive,'') as xadvreceive " +
                         "FROM amtfch where zid=@zid  and xrow=@xrow and xtype not in ('Tuition Fee','Admission')";
          ...
          //  }
            ...
            using (SqlConnection conn = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query = @"select zid,xsession,xstdid,xname,xclass,xcodealt, codealt1,xclassconf,xclasspro,xpromoted from amstudentclassconfvw1 
                                         where zid=@zid and xsession=@xsession and xstdid=@xstdid";
                    ...
                    if (amexamd.Rows.Count > 0)
                    {
                        ...
                        if (amexamd.Rows[0]["xclasspro"].ToString() != "")
                        {
                            classpro.Visible = true;

                            if (Int32.Parse(amexamd.Rows[0]["xpromoted"].ToString()) != 100)
                            {
                                if (Int32.Parse(amexamd.Rows[0]["xpromoted"].ToString()) == 1)
                                {
                                    xclasspro.Text = amexamd.Rows[0]["xclassconf"].ToString();
                                }
                                else if (Int32.Parse(amexamd.Rows[0]["xpromoted"].ToString()) == 10)
                                {
                                    xclasspro.Text = "A2 Level Completed.";
                                }
                                else
                                {
                                    xclasspro.Text = "Not Promoted";
        ...
        protected void xcdate_OnTextChanged(object sender, EventArgs e)
        {
          ...
          xstdid.Text.ToString().Trim() + "' and xsession='" + xsessioncur.Text.ToString().Trim() + "'  and zactiveacc=1");
                ...
                if (GridView1.DataSource != null)
                {
                    ...
                    foreach (GridViewRow row in GridView1.Rows)
                    {
                        if (row.Cells[0].Text.ToString() == "111")
                        {
                            continue;
                        }
...
// where zid=@zid and xsession=@xsession and xstdid=@xstdid";
                ...
                //if (classpro.Visible == true)
                //{
                //    if (xclasspro.Text == xclasscur.Text && xclasscur.Text!="A2 Level")
                //    {
                //        xclasspro.Text = "Not Promoted";
                //    }
                //}
