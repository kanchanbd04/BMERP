...
{
    public partial class amtfcspecialdisconfig : System.Web.UI.Page
    {
                    ...
                    //{
                        if (ViewState["xrow"] == null)
                        {
                            ...
                            using (SqlConnection con = new SqlConnection(zglobal.constring))
                            {
                                using (DataSet dts = new DataSet())
                                {
                                    con.Open();
                                    string query =
                                        "SELECT * FROM amtfcspecialdisconf WHERE zid=@zid AND xtfccode=@xtfccode AND xclass=@xclass AND xgroup=@xgroup AND xsection=@xsection AND xstdid=@xstdid AND xeffdate=@xeffdate ";
                                    ...
                                    da.SelectCommand.Parameters.AddWithValue("@xeffdate", xeffdate.Text.ToString().Trim() != string.Empty
                            ...
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query =
                                    "INSERT INTO amtfcspecialdisconf (ztime,zid,xrow,xtfccode,xclass,xgroup,xsection,xstdid,xamount,xdisfixed,xdisperc,xvatfixed,xvatperc,xsign,xeffdate,zactive,xstatus,xremarks,zemail,xsrow,xtype1) " +
                                    "VALUES(@ztime,@zid,@xrow,@xtfccode,@xclass,@xgroup,@xsection,@xstdid,@xamount,@xdisfixed,@xdisperc,@xvatfixed,@xvatperc,@xsign,@xeffdate,@zactive,@xstatus,@xremarks,@zemail,(select xrow from amadmis where zid=@zid and xstdid=@xstdid),@xtype1) ";

                                xrow = zglobal.GetMaximumIdInt("xrow", "amtfcspecialdisconf", " zid=" + _zid, 1, 1);
                                ViewState["xrow"] = xrow;
                                ...
                                cmd.Parameters.AddWithValue("@xdisfixed", xdisfixed1);
                                cmd.Parameters.AddWithValue("@xdisperc", xdisperc1);
                                cmd.Parameters.AddWithValue("@xvatfixed", xvatfixed1);
                                ...
                                cmd.Parameters.AddWithValue("@xtype1", xtype12);
                                cmd.ExecuteNonQuery();
                            }
                        }
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            if (xrow == 0)
                            {
                                ...
                                query = "UPDATE amtfcspecialdisconf SET zutime=@zutime,xemail=@xemail, " +
                                       "xamount=@xamount,xdisfixed=@xdisfixed,xdisperc=@xdisperc, xvatfixed=@xvatfixed, xvatperc=@xvatperc,xsign=@xsign, " +
                                       "xeffdate=@xeffdate, zactive=@zactive, xremarks=@xremarks, xtype1=@xtype1 " +
                                       "WHERE zid=@zid AND xrow=@xrow ";
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                         ...
                         "xdisperc,xdisfixed " +
                         "FROM amtfcspecialdisconf WHERE zid=@zid  AND xstatus='New' and coalesce(xstdid,'')=''  order by xtfccode,xeffdate desc";
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                string str = "SELECT  * FROM amtfcspecialdisconf where zid=@zid  and xrow=@xrow ";
                    ...
                    //if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM amtfcspecialdisconf WHERE zid=@zid AND xrow=@xrow";
                                cmd.Parameters.Clear();
