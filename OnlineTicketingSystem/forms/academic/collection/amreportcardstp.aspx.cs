...
{
    public partial class amreportcardstp : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    zglobal.fnGetComboDataCDWithValue("Term", xterm);
        ...
        private void BindGrid()
        {
            ...
            string[] trm = xterm.Text.Trim().ToString().Split(' ');
            if (trm[0] == "Mock")
            {
                xterm1 = trm[1].Trim().ToString();
            }
...
//                   ORDER BY xname";
            ...
            if (dtmarks.Rows.Count > 0)
            {
                ...
                string str2 = "";                
                    ...
                    TemplateField tfield700 = new TemplateField();
                    tfield700.HeaderText = "";
                    tfield700.ItemStyle.HorizontalAlign = HorizontalAlign.Center;
                    tfield700.ItemStyle.Width = Unit.Pixel(150);
                    // tfield7.Visible = false;
                    GridView1.Columns.Add(tfield700);
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    //chkCheckBox.Enabled = false;
                        ...
                        btnResend.CssClass = "bm_academic_button5 bm_am_btn_sendmail";
                        btnResend.ID = "btnResend";
                        btnResend.Text = "Resend Email";
                    btnResend.Width = 100;
                        btnResend.Visible = false;
                        btnResend.Click += btnResend_Click;
                        e.Row.Cells[6].Controls.Add(btnResend);
        ...
        protected void btnResend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string xsession1 = xsession.Text.Trim().ToString();
                string xterm1 = xterm.Text.Trim().ToString();
                string xstdid1 = gvr.Cells[1].Text.Trim().ToString();
                ...
                SendMail(_zid, xsession1, xterm1, xstdid1);

                message.InnerHtml = "Successfully resend email.";
        ...
        private string NotSendMailList = "";
        protected void btnSend_OnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
                      ...
                      "WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass AND xsection=@xsection and xtype in ('Term Exam','Class Test','Mock Exam')";
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
                       ...
                       "WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass AND xsection=@xsection ";
                ...
                //        string query =
                //            "select * from amexamhh where zid=@zid and xsession=@xsession and xclass=@xclass  and xsection=@xsection and xtype=@xtype and xstatus='Approved'";
                ...
                if (xterm.Text == "3rd")
                {
                    using (SqlConnection con = new SqlConnection(zglobal.constring))
                    {
                        using (DataSet dts1 = new DataSet())
                        {
                            ...
                            {
                                message.InnerHtml = "Please generate final result.";
                                message.Style.Value = zglobal.warningmsg;
                ...
                if (GridView1.Rows.Count > 0)
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        foreach (GridViewRow row in GridView1.Rows)
                        {
                            ...
                            xstatus1 = zglobal.fnGetValue("coalesce(xemailst,'') as xstdid", "amaccclearance",
                               "zid=" + _zid + " AND xsession='" + xsession1 + "' and xterm='" + xterm1 + "' and xsrow=" + Int64.Parse(row.Cells[row.Cells.Count - 1].Text.ToString()) + " and xtype='Email Report'");
                            ...
                            if (xstatus1 == "")
                            {
                                ...
                                cmd.Parameters.AddWithValue("@zemail",
                                    Convert.ToString(HttpContext.Current.Session["curuser"]));
                                cmd.Parameters.AddWithValue("@xterm", xterm1);
                                cmd.Parameters.AddWithValue("@xtype", "Email Report");
                            ...
                            else
                            {
                                ...
                                if (xemailst1 == 1)
                                {
                                    if (xstatus1 == "0")
                                    {
                                        ...
                                        query =
                                    "UPDATE amaccclearance SET xclearance=@xclearance,xemailst=1,zutime=@zutime,xemail=@xemail " +
                                    "WHERE zid=@zid AND xsession=@xsession AND xsrow=@xsrow and xtype=@xtype and xterm=@xterm ";
                                ...
                                else
                                {
                                    if (xstatus1 == "1")
                                    {
                                        ...
                                        query =
                                    "UPDATE amaccclearance SET xclearance=@xclearance,xemailst=0,zutime=@zutime,xemail=@xemail " +
                                    "WHERE zid=@zid AND xsession=@xsession AND xsrow=@xsrow and xtype=@xtype and xterm=@xterm ";
                ...
                if (NotSendMailList != "")
                {
                    message.InnerHtml = message.InnerHtml + "<br/> <div style=\"color:#FF0000;\">" + "Following students' email id not found : <br/>" +
                                        NotSendMailList + "</div>";
                }
        ...
        private void SendMail(int zid1, string xsession1, string xterm1, string xstdid1)
        {
            ...
            string filename = Server.MapPath("~/reportcard.html");
            ...
            //mailbody = mailbody.Replace("##TERM##", xterm2);
            ...
            string to = zglobal.fnGetValue("coalesce(xemail1,'')", "amadmis",
                         "zid=" + zid1 + " AND xstdid='" + xstdid1 + "'");

            if (to == "")
            {
                NotSendMailList = NotSendMailList + xstdid1 + " " + zglobal.fnGetValue("coalesce(xname,'')", "amadmis",
                         "zid=" + zid1 + " AND xstdid='" + xstdid1 + "'") + "<br/>";
                return;
            }
            ...
            message1.Subject = "Download Report Card";
            ...
            if (xterm1 == "3rd")
            {
                crystalReport.Load(Server.MapPath("~/reports/amreportcardfinal.rpt"));
            ...
            else
            {
                Int64 xsrow1 =
                    Convert.ToInt64(zglobal.fnGetValue("xrow", "amadmis",
                        "zid=" + zid1 + " and xstdid='" + xstdid1 + "'"));
                crystalReport.Load(Server.MapPath("~/reports/asreportcardonline.rpt"));
                crystalReport.SetParameterValue("zid", zid1);
                crystalReport.SetParameterValue("xsession", xsession1);
                crystalReport.SetParameterValue("xterm", xterm1);
                crystalReport.SetParameterValue("xsrow", xsrow1);
            }

            message1.Attachments.Add(new Attachment(crystalReport.ExportToStream(ExportFormatType.PortableDocFormat), "ReportCard.pdf"));
