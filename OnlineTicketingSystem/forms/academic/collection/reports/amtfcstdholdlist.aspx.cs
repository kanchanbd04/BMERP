...
{
    public partial class amtfcstdholdlist : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            //string xsession1 = zglobal.ReturnStringValue(" select top 1 coalesce(xcode, '') as xsession from mscodesam where zid=" + _zid + " and xtype='Session' and xcode < '" + xsession.Text.Trim().ToString() + "' order by xcode desc");
            //string xclass1 = zglobal.ReturnStringValue(" select top 1 coalesce(xcode, '') as xclass from mscodesam as a where zid=" + _zid + " and xtype='Class' and xcodealt < (select xcodealt from mscodesam as b where zid=a.zid and xtype='Class' and xcode='" + xclass.Text.ToString().Trim() + "') order by xcodealt desc");
            string xgroup1 = "";
            string xsection1 = "";
            string xsession1 = "";
            string xclass1 = "";
...
//            str = @"select xrow, xname, xstdid,xemail1,(xcellno + ', ' + xcellno1) as xcellno,
//                   xsection, xgroup, xprow as xrow1,xtype,(SELECT xcodealt FROM mscodesam WHERE zid=amstudentvwextacc.zid AND xtype='Section' AND zactive=1 and xcode=amstudentvwextacc.xsection) as xcodealt
//                   from amstudentvwextacc
//                   WHERE zid=@zid AND xsession=@xsession  AND xclass=@xclass AND xgroup in(" + xgroup1 + ")  AND " +
//                  "coalesce(xsection,'') in(" + xsection1 + ") and zactiveacc=1" +
//                  //"ORDER BY xcodealt,xname";
//                  "ORDER BY xname";

            str = @"select xrow, xname, xstdid,(xcellno + ', ' + xcellno1) as xcellno,
                    xsection, xgroup,xfname,xclass
                    from amtfcstdholdvw1
                    WHERE zid=@zid AND xsession=@xsession  and xtype='Hold'
                    ORDER BY xname";
            ...
            if (dtmarks.Rows.Count > 0)
            {
                ...
                //xtotal.Text = zglobal.fnGetValue("count(xstdid)", "amstudentvwextacc",
                //       "zid=" + _zid + " and xsession ='" + xsession1 + "' and xclass ='" + xclass1 + "' and xsection in(" +
                //       xsection1 + ") and xgroup in(" + xgroup1 + ") ");

                //xtotal.Text = dtmarks.Rows.Count.ToString();

                //xseatconf.Text = zglobal.fnGetValue("count(xstdid)", "amtfcseatconf",
                //       "zid=" + _zid + " and xsession ='" + xsession.Text.Trim().ToString() + "' and xclass ='" + xclass.Text.ToString().Trim() + "' and xsection in(" +
                //       xsection1 + ") and xgroup in(" + xgroup1 + ") and coalesce(xstatus,'Seat Conf')='Seat Conf'");

                //xseatconf.Text = zglobal.fnGetValue("count(xstdid)", "amstudentvwextacc",
                //      "zid=" + _zid + " and xsession ='" + xsession.Text.Trim().ToString() + "' and xclass ='" + xclass.Text.ToString().Trim() + "' and xsection in(" +
                //      xsection1 + ") and xgroup in(" + xgroup1 + ") and xtype='Seat Conf'");

                //xseatconf.Text = dtmarks.Select("xtype = 'Seat Conf'").Count().ToString();

                //xpromotion.Text = zglobal.ReturnStringValue("select count(xstdid) from " +
                //                        "( " +
                //                        "select h.zid as zid,xsessionpro as xsession,case when xpromoted=1 then xclasspro else xclass end as xclass, " +
                //                        "(select xstdid from amadmis where zid=h.zid and xrow=d.xsrow) as xstdid,xpromoted " +
                //                        "from ampromotionh as h inner join ampromotiond as d on h.zid=d.zid and h.xrow=d.xrow " +
                //                        ")tbl " +
                //                        "WHERE zid=" + _zid + " AND xsession='" + xsession.Text.Trim().ToString() + "' AND xclass='" + xclass.Text.Trim().ToString() + "' and xpromoted=1");

                //xpromotion.Text = zglobal.fnGetValue("count(xstdid)", "amstudentvwext",
                //       "zid=" + _zid + " and xsession ='" + xsession1 + "' and xclass ='" + xclass1 + "' and xsection in(" +
                //       xsection1 + ") and xgroup in(" + xgroup1 + ") and  ");

                //xpromotion.Text = zglobal.fnGetValue("count(xstdid)", "amstudentvwext",
                //       "zid=" + _zid + " and xsession ='" + xsession.Text.Trim().ToString() + "' and xclass ='" + xclass.Text.ToString().Trim() + "' and xsection in(" +
                //       xsection1 + ") and xgroup in(" + xgroup1 + ") and xtype='Promoted'");
...
//                    {
//                        string query = "SELECT xstdid FROM amstudentvwextacc WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass and xtype='Seat Conf'";
...
//                        {
//                            ViewState["xseatconf"] = amexamd;
//                        }
//                        else
//                        {
//                            ViewState["xseatconf"] = null;
//                        }
//                    }
//                }
...
//                    using (DataSet dts1 = new DataSet())
//                    {
////                        string query = @"select zid,xsession,xclass,xstdid,xpromoted from
////                                        (
////                                        select h.zid as zid,xsessionpro as xsession,case when xpromoted=1 then xclasspro else xclass end as xclass, 
////                                        (select xstdid from amadmis where zid=h.zid and xrow=d.xsrow) as xstdid,xpromoted
////                                        from ampromotionh as h inner join ampromotiond as d on h.zid=d.zid and h.xrow=d.xrow
////                                        )tbl
////                                        WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass and xpromoted=1";

//                        string query = "SELECT xstdid FROM amstudentvwext WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass and xtype='Promoted'";
...
//                        if (amexamd.Rows.Count > 0)
//                        {
//                            ViewState["xpromoted"] = amexamd;
//                        }
//                        else
//                        {
//                            ViewState["xpromoted"] = null;
//                        }
//                    }
//                }
                ...
                string str2 = "";                
                    ...
                    //TemplateField tfield7 = new TemplateField();
                    //tfield7.HeaderText = "Seat Confirmation";
                    //tfield7.ItemStyle.HorizontalAlign = HorizontalAlign.Center;
                    ...
                    bfield = new BoundField();
                    bfield.HeaderText = "Mobile No.";
                    //bfield.SortExpression = "xname";
                    bfield.DataField = "xcellno";
            ...
            else
            {
                ...
                GridView1.DataBind();
                message.InnerHtml = "No Student found in hold list.";
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    //    lbl.Controls.Add(spn);
                        ...
                        //if (ViewState["xseatconf"] != null)
                        //{
                        //    DataTable ampromotiond = (DataTable)ViewState["xseatconf"];
                        //    DataRow[] result =
                        //        ampromotiond.Select("xstdid=" +
                        //                            (e.Row.DataItem as DataRowView).Row["xstdid"].ToString());
                        //    if (result.Length > 0)
                        //    {
                        ...
                        //if (ViewState["xpromoted"] != null)
                        //{
                        //    DataTable ampromotiond = (DataTable)ViewState["xpromoted"];
                        //    DataRow[] result =
                        //        ampromotiond.Select("xstdid=" +
                        //                            (e.Row.DataItem as DataRowView).Row["xstdid"].ToString());
                        //    if (result.Length > 0)
