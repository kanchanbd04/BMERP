...
{
    public partial class amtfcconfig : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ViewState["xrow"] != null)
                {
                    xstatus2 = zglobal.fnGetValue("xstatus", "amtfcconfig",
                          "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
                    ...
                    //{
                        if (ViewState["xrow"] == null)
                        {
                            ...
                            using (SqlConnection con = new SqlConnection(zglobal.constring))
                            {
                                using (DataSet dts = new DataSet())
                                {
                                    con.Open();
                                    string query =
                                        "SELECT * FROM amtfcconfig WHERE zid=@zid AND xtfccode=@xtfccode AND xclass=@xclass AND xgroup=@xgroup AND xsection=@xsection AND xstdid=@xstdid AND xeffdate=@xeffdate ";
                            ...
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query =
                                    "INSERT INTO amtfcconfig (ztime,zid,xrow,xtfccode,xclass,xgroup,xsection,xstdid,xamount,xdisfixed,xdisperc,xvatfixed,xvatperc,xsign,xeffdate,zactive,xstatus,xremarks,zemail,xsrow,xtype1) " +
                                ...
                                xrow = zglobal.GetMaximumIdInt("xrow", "amtfcconfig", " zid=" + _zid, 1, 1);
                                ViewState["xrow"] = xrow;
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            if (xrow == 0)
                            {
                                ...
                                query = "UPDATE amtfcconfig SET zutime=@zutime,xemail=@xemail, " +
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                         ...
                         "FROM amtfcconfig WHERE zid=@zid  AND xstatus='New' and coalesce(xstdid,'')='' order by xtfccode,xeffdate desc";
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                         ...
                         " (select xdescdet from mscodesam where zid=amtfcconfig.zid and xcode=amtfcconfig.xtfccode and xtype='TFC Code') as xtfccodetitle, " +
                         "(select xname from amadmis where zid=amtfcconfig.zid and xstdid=amtfcconfig.xstdid) as xname, xamount,zactive " +
                         "FROM amtfcconfig WHERE zid=@zid  AND xstatus='Submited' and coalesce(xstdid,'')='' order by xtfccode,xeffdate desc";
