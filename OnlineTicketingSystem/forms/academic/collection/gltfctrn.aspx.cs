...
{
    public partial class gltfctrn : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    xtype.Items.Remove("Advance Received Adjustment");
                    ...
                    //btnPost1.Enabled = false;

                    ViewState["xmodule"] = "Collection";
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //    //    isValid = false;
            //    //}
            //    //if (xtype1.Text == "" || xtype1.Text == string.Empty || xtype1.Text == "[Select]")
            //    //{
            //    //    rtnMessage = rtnMessage + "<li>Payment Type</li>";
            //    //    isValid = false;
            //    //}
            ...
            //    //if (ViewState["xrow"] != null)
            //    //{
            //    //    xstatus2 = zglobal.fnGetValue("xstatus", "amtfcspecialdisconf",
            //    //          "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
            ...
            //                        string query =
            //                           "SELECT * FROM glinterface WHERE zid=@zid AND xkey=@xkey AND xtype=@xtype and xmodule=@xmodule "; //AND xbank=@xbank AND xprefix=@xprefix 


            //                        SqlDataAdapter da = new SqlDataAdapter(query, con);

            //                        da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
            //                        da.SelectCommand.Parameters.AddWithValue("@xtype", xtype.Text.ToString().Trim());
            ...
            //                        da.SelectCommand.Parameters.AddWithValue("@xkey", xkey.Text.ToString().Trim());
            ...
            //                    string query =
            //                        "INSERT INTO glinterface (ztime,zid,xrow,xmodule,xkey,xtype,xaccdr,xsubdr,xacccr,xsubcr,xbank,xprefix,zemail) " +
            //                        "VALUES(@ztime,@zid,@xrow,@xmodule,@xkey,@xtype,@xaccdr,@xsubdr,@xacccr,@xsubcr,@xbank,@xprefix,@zemail) ";
            ...
            //                    xkey1 = xkey.Text.Trim().ToString();
            //                    xtype12 = xtype.Text.Trim().ToString();
            ...
            //                    cmd.Parameters.AddWithValue("@xkey", xkey1);
            //                    cmd.Parameters.AddWithValue("@xtype", xtype12);
            ...
            //                           "xmodule=@xmodule,xkey=@xkey,xtype=@xtype, xaccdr=@xaccdr, xsubdr=@xsubdr,xacccr=@xacccr, " +
            //                           "xsubcr=@xsubcr, xbank=@xbank, xprefix=@xprefix " +
            //                           "WHERE zid=@zid AND xrow=@xrow ";
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            ...
            //    xkey.Text = dttemp.Rows[0]["xkey"].ToString();
        ...
        protected void btnProcess_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DateTime xdatefrom = xfdate.Text.ToString().Trim() != string.Empty
                              ? DateTime.ParseExact(xfdate.Text.ToString().Trim(), "dd/MM/yyyy",
                                  CultureInfo.InvariantCulture)
                              : DateTime.Now;

                DateTime xdateto = xtdate.Text.ToString().Trim() != string.Empty
                            ? DateTime.ParseExact(xtdate.Text.ToString().Trim(), "dd/MM/yyyy",
                                CultureInfo.InvariantCulture)
                            : DateTime.Now;
                int xfperyear = xdatefrom.Month + xdatefrom.Year;
                int xtperyear = xdateto.Month + xdateto.Year;

                if (xfperyear != xtperyear)
                {
                    message.InnerHtml = "Date range isn't in Same Month and Year ";
                    message.Style.Value = zglobal.warningmsg;
                    return;
                }
                ...
                DateTime xtdate123 = new DateTime(tempyear + 1, offset + 1, 1);
                ...
                string xbank1 = xbank.Text.Trim().ToString();
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts = new DataSet())
                    {
                        ...
                        //    query = "SELECT * " +
                        //            "FROM amtfcduecalvw2 " +
                        //            "WHERE zid=@zid AND xtitle='Revenue' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and coalesce(xvoucher,'')=''";
                        //}
                        //else if (xtype1 == "Discount")
                        //{
                        //    query = "SELECT * " +
                        //            "FROM amtfcduecalvw2 " +
                        //            "WHERE zid=@zid AND xtitle='Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and coalesce(xvoucher,'')=''";
                        //}
                        //else if (xtype1 == "Special Discount")
                        //{
                        //    query = "SELECT * " +
                        //            "FROM amtfcduecalvw2 " +
                        //            "WHERE zid=@zid AND xtitle='Special Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and coalesce(xvoucher,'')=''";
                        //}
                        //else
                        //{
                        //    query = "SELECT * " +
                        //            "FROM amtfcduecalvw2 " +
                        //            "WHERE zid=@zid AND xtitle='Received' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and xbank=@xbank and coalesce(xvoucher,'')=''";
                        //}
                        ...
                        if (xtype1 == "Revenue")
                        {
                            query = "SELECT * " +
                                    "FROM amtfcduecalvw2 " +
                                    "WHERE zid=@zid AND xtitle='Revenue' AND xtdate>=@xdatefrom AND xtdate<=@xdateto ";
                        }
                        else if (xtype1 == "Discount")
                        {
                            query = "SELECT * " +
                                    "FROM amtfcduecalvw2 " +
                                    "WHERE zid=@zid AND xtitle='Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto ";
                        }
                        else if (xtype1 == "Special Discount")
                        {
                            query = "SELECT * " +
                                    "FROM amtfcduecalvw2 " +
                                    "WHERE zid=@zid AND xtitle='Special Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto ";
                        }
                        else if (xtype1 == "Received")
                        {
                            query = "SELECT * " +
                                    "FROM amtfcduecalvw2 " +
                                    "WHERE zid=@zid AND xtitle='Received' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and xbank=@xbank and coalesce(xvoucher,'')='' and xadvreceive='No'";
                        }
                        else
                        {
                            query = "SELECT * " +
                                    "FROM amtfcduecalvw2 " +
                                    "WHERE zid=@zid AND xtitle='Received' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and xbank=@xbank and coalesce(xvoucher,'')='' and xadvreceive='Yes'";
                        }
                        ...
                        da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                        da.SelectCommand.Parameters.AddWithValue("@xdatefrom", xdatefrom);
                        da.SelectCommand.Parameters.AddWithValue("@xdateto", xdateto);
                        da.SelectCommand.Parameters.AddWithValue("@xbank", xbank1);
                        ...
                        if (tempTable.Rows.Count > 0)
                        {
                            ViewState["gltfctrn"] = tempTable;
                        }
                        else
                        {
                            ViewState["gltfctrn"] = null;
                        }
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts = new DataSet())
                    {
                        ...
                        if (xtype1 == "Revenue")
                        {
                            query = "SELECT * " +
                                       "FROM glinterface " +
                                       "WHERE zid=@zid AND xmodule='Collection' AND xtype='Revenue'";
                        }
                        else if (xtype1 == "Discount")
                        {
                            query = "SELECT * " +
                                        "FROM glinterface " +
                                        "WHERE zid=@zid AND xmodule='Collection' AND xtype='Discount'";
                        }
                        else if (xtype1 == "Special Discount")
                        {
                            query = "SELECT * " +
                                        "FROM glinterface " +
                                        "WHERE zid=@zid AND xmodule='Collection' AND xtype='Special Discount'";
                        }
                        else if (xtype1 == "Received")
                        {
                            query = "SELECT * " +
                                      "FROM glinterface " +
                                      "WHERE zid=@zid AND xmodule='Collection' AND xtype='Received' and xbank=@xbank";
                        }
                        else
                        {
                            query = "SELECT * " +
                                      "FROM glinterface " +
                                      "WHERE zid=@zid AND xmodule='Collection' AND xtype='Advance Received' and xbank=@xbank";
                        }
                        ...
                        if (tempTable.Rows.Count > 0)
                        {
                            ViewState["glinterface"] = tempTable;

                        }
                        else
                        {
                            ViewState["glinterface"] = null;
                        }
                    ...
                    using (SqlConnection con = new SqlConnection(zglobal.constring))
                    {
                        using (DataSet dts = new DataSet())
                        {
                            ...
                            string query =
                                          "SELECT * " +
                                          "FROM glinterface " +
                                          "WHERE zid=@zid AND xmodule='Collection' AND xtype='Advance Received Adjustment' ";
                            ...
                            if (tempTable.Rows.Count > 0)
                            {
                                ViewState["glinterface1"] = tempTable;

                            }
                            else
                            {
                                ViewState["glinterface1"] = null;
                            }
                ...
                if (ViewState["gltfctrn"] != null)
                {
                    using (TransactionScope tran = new TransactionScope())
                    {
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            if (xtype1 == "Revenue")
                            {
                                query = "UPDATE amtfcdued SET xvoucher='' " +
                                         "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Revenue' AND xtdate>=@xdatefrom AND xtdate<=@xdateto )";

                                cmd.CommandText = query;
                                cmd.Parameters.Clear();
                                cmd.Parameters.AddWithValue("@zid", _zid);
                                cmd.Parameters.AddWithValue("@xdatefrom", xdatefrom);
                                cmd.Parameters.AddWithValue("@xdateto", xdateto);
                                cmd.ExecuteNonQuery();

                                query = "DELETE FROM gldetail " +
                                        "WHERE zid=@zid AND xvoucher in (select xvoucher from gltfctrn where zid=@zid AND xtitle='Revenue' AND xtdate>=@xdatefrom AND xtdate<=@xdateto )";
                            ...
                            {
                                query = "UPDATE amtfcdued SET xvoucher='' " +
                                        "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto  )";
                                ...
                                query = "DELETE FROM gldetail " +
                                        "WHERE zid=@zid AND xvoucher in (select xvoucher from gltfctrn where zid=@zid AND xtitle='Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto )";
                            ...
                            {
                                query = "UPDATE amtfcdued SET xvoucher='' " +
                                        "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Special Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto  )";
                                ...
                                query = "DELETE FROM gldetail " +
                                        "WHERE zid=@zid AND xvoucher in (select xvoucher from gltfctrn where zid=@zid AND xtitle='Special Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto )";
                            ...
                            {
                                query = "DELETE FROM gltfctrn " +
                                        "WHERE zid=@zid AND xtitle='Received' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and xbank=@xbank and coalesce(xvoucher,'')='' and coalesce(xadvreceive,'No')='No'";
                            }
                            else
                            {
                                query = "DELETE FROM gltfctrn " +
                                        "WHERE zid=@zid AND xtitle='Received' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and xbank=@xbank and coalesce(xvoucher,'')='' and coalesce(xadvreceive,'No')='Yes'";
                            }
                            ...
                            {
                                query = "DELETE FROM gltfctrn " +
                                      "WHERE zid=@zid AND xtype='Advance Received Adjustment' AND xtdate>=@xdatefrom AND xtdate<=@xdateto  and coalesce(xvoucher,'')='' and coalesce(xadvreceive,'No')='Yes'";
                            ...
                            foreach (DataRow row in ((DataTable)ViewState["gltfctrn"]).Rows)
                            {
                                if (ViewState["glinterface"] != null)
                                {
                                    DataRow[] rowglint =
                                        ((DataTable)ViewState["glinterface"]).Select("zid=" + row["zid"] +
                                                                                      " and xkey='" +
                                                                                      row["xtfccode"] + "'");

                                    if (rowglint.Length > 0)
                                    {
                                        foreach (var row1 in rowglint)
                                        {
                                            query = "INSERT INTO gltfctrn (ztime,zid,xrow,xkey,xrow_tbl,xline,xsrow,xtitle,xbank,xamount,xtdate,xmodule,xrow_int,xtype,xprefix,xaccdr,xsubdr,xacccr,xsubcr,zemail,xadvreceive) " +
                                                    "VALUES(@ztime,@zid,@xrow,@xkey,@xrow_tbl,@xline,@xsrow,@xtitle,@xbank,@xamount,@xtdate,@xmodule,@xrow_int,@xtype,@xprefix,@xaccdr,@xsubdr,@xacccr,@xsubcr,@zemail,@xadvreceive)";

                                            cmd.CommandText = query;
                                            cmd.Parameters.Clear();
                                            cmd.Parameters.AddWithValue("@ztime", DateTime.Now);
                                            cmd.Parameters.AddWithValue("@zid", _zid);
                                            cmd.Parameters.AddWithValue("@xrow", zglobal.GetMaximumIdInt("xrow", "gltfctrn", " zid=" + _zid, 1, 1));
                                            cmd.Parameters.AddWithValue("@xkey", row1["xkey"].ToString());
                                            cmd.Parameters.AddWithValue("@xrow_tbl", Convert.ToInt32(row["xrow"]));
                                            cmd.Parameters.AddWithValue("@xline", Convert.ToInt32(row["xline"]));
                                            cmd.Parameters.AddWithValue("@xsrow", Convert.ToInt64(row["xsrow"]));
                                            cmd.Parameters.AddWithValue("@xtitle", row["xtitle"].ToString());
                                            cmd.Parameters.AddWithValue("@xbank", row["xbank"].ToString());
                                            cmd.Parameters.AddWithValue("@xamount", Convert.ToDecimal(row["xamount"]));
                                            cmd.Parameters.AddWithValue("@xtdate", Convert.ToDateTime(row["xtdate"]));
                                            cmd.Parameters.AddWithValue("@xmodule", ViewState["xmodule"].ToString());
                                            cmd.Parameters.AddWithValue("@xrow_int", Convert.ToInt32(row1["xrow"]));
                                            cmd.Parameters.AddWithValue("@xtype", row1["xtype"].ToString());
                                            cmd.Parameters.AddWithValue("@xprefix", row1["xprefix"].ToString());
                                            cmd.Parameters.AddWithValue("@xaccdr", row1["xaccdr"].ToString());
                                            cmd.Parameters.AddWithValue("@xsubdr", row1["xsubdr"].ToString());
                                            cmd.Parameters.AddWithValue("@xacccr", row1["xacccr"].ToString());
                                            cmd.Parameters.AddWithValue("@xsubcr", row1["xsubcr"].ToString());
                                            cmd.Parameters.AddWithValue("@zemail", Convert.ToString(HttpContext.Current.Session["curuser"]));
                                            cmd.Parameters.AddWithValue("@xadvreceive", row["xadvreceive"].ToString());
                                            cmd.ExecuteNonQuery();
                                ...
                                if (ViewState["glinterface1"] != null)
                                {
                                    DataRow[] rowglint =
                                        ((DataTable)ViewState["glinterface1"]).Select("zid=" + row["zid"] +
                                                                                      " and xkey='" +
                                                                                      row["xtfccode"] + "'");
                                    ...
                                    if (rowglint.Length > 0)
                                    {
                                        foreach (var row1 in rowglint)
                                        {
                                            ...
                                            cmd.Parameters.AddWithValue("@xsrow", Convert.ToInt64(row["xsrow"]));
                                            cmd.Parameters.AddWithValue("@xtitle", row["xtitle"].ToString());
                                            cmd.Parameters.AddWithValue("@xbank", "");
                                            cmd.Parameters.AddWithValue("@xamount", Convert.ToDecimal(row["xamount"]));
                                            cmd.Parameters.AddWithValue("@xtdate", Convert.ToDateTime(row["xtdate"]));
                                            cmd.Parameters.AddWithValue("@xmodule", ViewState["xmodule"].ToString());
                                            cmd.Parameters.AddWithValue("@xrow_int", Convert.ToInt32(row1["xrow"]));
                ...
                //message.InnerHtml = zglobal.savesuccmsg;
                message.InnerHtml = "Successfully Processed <b>" + count.ToString() + "</b> Row(s).";
                message.Style.Value = zglobal.successmsg;
            }
            catch (Exception exp)
            {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts = new DataSet())
                    {
                        ...
                        query = @"select zid,xtype,xbank,xmodule,xprefix,xaccdr as xacc,xsubdr as xsub,1 as xsign,sum(xamount) as xamount from gltfctrn
                                  where zid=@zid and xtdate>=@xdatefrom and xtdate<=@xdateto and xtype=@xtype and xbank=@xbank and xmodule=@xmodule and coalesce(xvoucher,'') =''
                                  group by zid,xtype,xbank,xmodule,xprefix,xaccdr,xsubdr,coalesce(xadvreceive,'No')
                                  union
                                  select zid,xtype,xbank,xmodule,xprefix,xacccr,xsubcr,-1 as xsign,sum(xamount) from gltfctrn
                                  where zid=@zid and xtdate>=@xdatefrom and xtdate<=@xdateto and xtype=@xtype and xbank=@xbank and xmodule=@xmodule and coalesce(xvoucher,'') =''
                                  group by zid,xtype,xbank,xmodule,xprefix,xacccr,xsubcr,coalesce(xadvreceive,'No')";
                        ...
                        if (tempTable.Rows.Count > 0)
                        {
                            ...
                            {
                                if (Convert.ToInt32(row["xsign"]) < 0)
                                {
                                    cramt = cramt + Convert.ToDecimal(row["xamount"]);
                                }
                                else
                                {
                                    dramt = dramt + Convert.ToDecimal(row["xamount"]);
                                }
                            }
                            ...
                            if (dramt == 0 && cramt == 0)
                            {
                                message.InnerHtml = "Amount Zero. Can't Post.";
                                message.Style.Value = zglobal.warningmsg;
                                return;
                            }

                            string xlong1 = "";
                            string xpostflag1 = "Posted";
                            ...
                            using (TransactionScope tran = new TransactionScope())
                            {
                                using (SqlConnection conn = new SqlConnection(zglobal.constring))
                                {
                                    ...
                                    _zid = Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"]));
                                    string xvoucher_1 = "";
                                    xdate1 = xtdate.Text.ToString().Trim() != string.Empty
                                        ? DateTime.ParseExact(xtdate.Text.ToString().Trim(), "dd/MM/yyyy",
                                            CultureInfo.InvariantCulture)
                                            : DateTime.Now;

                                    //if (xtype.Text == "Advance Received")
                                    //{
                                    //    xdate1 = xtdate123;
                                    //}
                                    ...
                                    foreach (DataRow row in tempTable.Rows)
                                    {
                                        if (chk == 0)
                                        {
                                            ...
                                            xaction1 = zglobal.fnGetValue("xaction", "mstrn",
                                            "zid=" + _zid + " and xtypetrn='GL Voucher' " +
                                            "and xtrn='" + row["xprefix"].ToString() + "'");

                                            query =
                                      "INSERT INTO glheader (ztime,zid,xvoucher,xref,xdate,xlong,xpostflag,xyear,xper,xstatusjv,xdesc03,zemail,xnumofper,xtrngl,xnote,xaction,xflag,xstatus,xemailc,xdatepost) " +
                                            "VALUES(@ztime,@zid,@xvoucher,@xref,@xdate,@xlong,@xpostflag,@xyear,@xper,@xstatusjv,@xdesc03,@zemail,@xnumofper,@xtrngl,@xnote,@xaction,'','0000',@xemailc,@xdatepost )";

                                            xvoucher_1 = zglobal.GetMaximumIDWithTRNAcc(row["xprefix"].ToString(), "xvoucher", "glheader", xdate1, "xdate");
                                            ViewState["xvoucher"] = xvoucher_1;

                                            xtrngl1 = row["xprefix"].ToString();
                                            xref1 = "Transfered From TFC.";
                                            xdesc031 = "";
                                            xnote1 = "**System Generated Voucher Transfered From Collection**";

                                            cmd.CommandText = query;
                                            cmd.Parameters.AddWithValue("@ztime", ztime);
                                            cmd.Parameters.AddWithValue("@xdatepost", ztime);
                                            cmd.Parameters.AddWithValue("@zid", _zid);
                                            ...
                                            cmd.Parameters.AddWithValue("@zemail", zemail1);
                                            cmd.Parameters.AddWithValue("@xemailc", zemail1);
                                            cmd.Parameters.AddWithValue("@xnumofper", xnumofper1);
                                            cmd.Parameters.AddWithValue("@xtrngl", xtrngl1);
                                        ...
                                        int xrow11 = zglobal.GetMaximumIdInt("xrow", "gldetail",
                                  " zid=" + _zid + " and xvoucher='" + Convert.ToString(ViewState["xvoucher"] + "'"), "row");
                                        ...
                                        if (Convert.ToInt32(row["xsign"]) > 0)
                                        {
                                            xprime1 = Convert.ToDecimal(row["xamount"]);
                                            xacc1 = row["xacc"].ToString();
                                            xsub1 = row["xsub"].ToString();
                                        }
                                        else
                                        {
                                            xprime1 = -Convert.ToDecimal(row["xamount"]);
                                        ...
                                        string xcheque1 = "System";
                                        string xpaytype1 = "";

                                        if (row["xprefix"].ToString() == "BRV-")
                                        {
                                            xpaytype1 = "Cheque";
                                        }
                                        else
                                        ...
                                        DateTime xdateclr1 = new DateTime(2999, 12, 31);

                                        if (Math.Abs(xprime1) != 0)
                                        {
                                            using (DataSet dtsChkStatusDataSet = new DataSet())
                                            {
                                                string query1 =
                                                    "SELECT xaccusage,xaccsource,xacctype FROM glmst WHERE zid=@zid and xacc=@xacc";

                                                SqlDataAdapter daChkStatuSqlDataAdapter = new SqlDataAdapter(query1,
                                                    conn);
                                                daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@zid",
                                                    _zid);
                                                daChkStatuSqlDataAdapter.SelectCommand.Parameters.AddWithValue("@xacc",
                                                    xacc1);
                                                ...
                                                {
                                                    xaccusage1 =
                                                        tempTableChkStatusDataTable.Rows[0]["xaccusage"].ToString();
                                                    xaccsource1 =
                                                        tempTableChkStatusDataTable.Rows[0]["xaccsource"].ToString();
                                                    xacctype1 =
                                                        tempTableChkStatusDataTable.Rows[0]["xacctype"].ToString();

                                                    if (xaccsource1 == "None")
                                                    {
                                                        xsub1 = row["xacc"].ToString();
                                                    }
                                                }
                                            ...
                                            cmd.Parameters.Clear();
                                            cmd.Parameters.AddWithValue("@zid", _zid);
                                            cmd.Parameters.AddWithValue("@xvoucher",
                                                Convert.ToString(ViewState["xvoucher"]));
                                    ...
                                    if (xtype1 == "Revenue")
                                    {
                                        query = "UPDATE gltfctrn SET xvoucher=@xvoucher " +
                                                "WHERE zid=@zid AND xtitle='Revenue' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and coalesce(xvoucher,'')=''";
                                    }
                                    else if (xtype1 == "Discount")
                                    {
                                    ...
                                    if (xtype1 == "Revenue")
                                    {
                                        query = "UPDATE amtfcdued SET xvoucher=@xvoucher " +
                                                 "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Revenue' AND xtdate>=@xdatefrom AND xtdate<=@xdateto  and coalesce(xvoucher,'')=@xvoucher)";
                                    }
                                    else if (xtype1 == "Discount")
                                    {
                                        query = "UPDATE amtfcdued SET xvoucherdis=@xvoucher " +
                                                "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto  and coalesce(xvoucher,'')=@xvoucher)";
                                    }
                                    else if (xtype1 == "Special Discount")
                                    {
                                        query = "UPDATE amtfcdued SET xvoucherspdis=@xvoucher " +
                                                "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Special Discount' AND xtdate>=@xdatefrom AND xtdate<=@xdateto  and coalesce(xvoucher,'')=@xvoucher)";
                                    }
                                    else if (xtype1 == "Received")
                                    {
                                        query = "UPDATE amtfch SET xstatus='Paid',xvoucher=@xvoucher " +
                                                "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Received' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and xbank=@xbank and coalesce(xvoucher,'')=@xvoucher and coalesce(xadvreceive,'No')='No')";
                                    }
                                    else
                                    {
                                        query = "UPDATE amtfch SET xstatus='Paid',xvoucher=@xvoucher " +
                                                "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Received' AND xtdate>=@xdatefrom AND xtdate<=@xdateto and xbank=@xbank and coalesce(xvoucher,'')=@xvoucher and coalesce(xadvreceive,'No')='Yes')";
                                    }
                                    ...
                                    //if (xtype1 == "Revenue")
                                    //{
                                    //    query = "UPDATE amtfcdueh SET xstatus='Confirmed' " +
                                    //             "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtitle='Revenue' AND xtdate>=@xdatefrom AND xtdate<=@xdateto  and coalesce(xvoucher,'')=@xvoucher)";
                                ...
                                if (xtype1 == "Advance Received")
                                {
                                    string advadj = fnAdvancedReceivedAdjustment();
                                    if (advadj == "0")
                                    {
                                        return;
                                    }
                                    ...
                                    {
                                        ViewState["xvoucher"] = ViewState["xvoucher"].ToString() + ", " + advadj;
                                    }
                                }
                        ...
                        {
                            message.InnerText = "No Data Found for Post.";
                            message.Style.Value = zglobal.warningmsg;
                            return;
                        }
                    }
                ...
                message.InnerHtml = "Successfully Posted Transaction With Voucher No. <b>" + ViewState["xvoucher"].ToString() + "</b>.";
                message.Style.Value = zglobal.successmsg;
        ...
        public string  fnAdvancedReceivedAdjustment()
        {
            int _zid = Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"]));
            ...
            //string xtype1 = xtype.Text.Trim().ToString();
            string xtype1 = "Advance Received Adjustment";
            string xbank1 = "";
                            ...
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                foreach (DataRow row in tempTable.Rows)
                                {
                                  ...
                                  "INSERT INTO glheader (ztime,zid,xvoucher,xref,xdate,xlong,xpostflag,xyear,xper,xstatusjv,xdesc03,zemail,xnumofper,xtrngl,xnote,xaction,xflag,xstatus,xemailc,xdatepost) " +
                                        ...
                                        xvoucher_1 = zglobal.GetMaximumIDWithTRNAcc(row["xprefix"].ToString(), "xvoucher", "glheader", xdate1, "xdate");
                                        ViewState["xvoucher1"] = xvoucher_1;

                                        xtrngl1 = row["xprefix"].ToString();
                                        xref1 = "Transfered From TFC.";
                                        xdesc031 = "";
                                        xnote1 = "**System Generated Voucher Transfered From Collection (Advance Received Adjustment)**";
                                        ...
                                        cmd.CommandText = query;
                                        cmd.Parameters.AddWithValue("@zid", _zid);
                                        cmd.Parameters.AddWithValue("@xvoucher", ViewState["xvoucher1"].ToString());
                                        cmd.ExecuteNonQuery();
                                    ...
                                    int xrow11 = zglobal.GetMaximumIdInt("xrow", "gldetail",
                              " zid=" + _zid + " and xvoucher='" + Convert.ToString(ViewState["xvoucher1"] + "'"), "row");
                                    ...
                                    if (Math.Abs(xprime1) != 0)
                                    {
                                        ...
                                        cmd.Parameters.AddWithValue("@zid", _zid);
                                        cmd.Parameters.AddWithValue("@xvoucher",
                                            Convert.ToString(ViewState["xvoucher1"]));
                                        cmd.Parameters.AddWithValue("@xrow", xrow11);
                                        cmd.Parameters.AddWithValue("@xacc", xacc1);
                                        cmd.Parameters.AddWithValue("@xaccusage", xaccusage1);
                                        cmd.Parameters.AddWithValue("@xaccsource", xaccsource1);
                                ...
                                query = "UPDATE amtfch SET xstatus='Paid',xvoucher=@xvoucher " +
                                            "WHERE zid=@zid AND xrow in (select xrow_tbl from gltfctrn where zid=@zid AND xtype='Advance Received Adjustment' AND xtdate>=@xdatefrom AND xtdate<=@xdateto  and coalesce(xvoucher,'')=@xvoucher and coalesce(xadvreceive,'No')='Yes')";
            ...
            return ViewState["xvoucher1"].ToString();
        }

        protected void xtype_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                message.InnerHtml = "";
                ...
                if (xtype.Text.Trim().ToString() == "Received" || xtype.Text.Trim().ToString() == "Advance Received")
                {
                    xbnk.Visible = true;
                    //xbank.Text = "";
                }
