...
{
    public partial class hrgrowthh_ctrpttemp : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            str = "select ROW_NUMBER() OVER (ORDER BY xcodealt,xcat,xline) AS xid,xrow,xline,xdesc,xcat FROM hrgrowthset as a inner join mscodesam as b " +
                  ...
                  "WHERE a.zid=@zid AND a.xtype='Class Teacher' and a.zactive=1 order by xcodealt,xcat,xline ";
            ...
            if (dtmarks.Rows.Count > 0)
            {
                ...
                tfield119.ItemStyle.Width = 200;
                GridView1.Columns.Add(tfield119);



                tfield = new TemplateField();
                tfield.HeaderText = "Teacher's Name and Subject";
                tfield.ItemStyle.HorizontalAlign = HorizontalAlign.Center;
                tfield.ItemStyle.Width = 350;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //    message.InnerHtml = message.InnerHtml + txtTeacher.Text.ToString() +
                //                        "  " + txtClassSub.Text.ToString() +
                //                        "  " + txtAction.Text.ToString() +
                    ...
                    if (GridView1.Rows.Count > 0)
                    {
                        if (ViewState["xrow"] == null)
                        {
                            ...
                            using (SqlConnection con = new SqlConnection(zglobal.constring))
                            {
                                using (DataSet dts = new DataSet())
                                {
                                    ...
                                    string query =
                                        ...
                                        "SELECT * FROM hrgrowthh WHERE zid=@zid AND xsession=@xsession AND xdate=@xdate AND xtype='Class Teacher'";
                            ...
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query =
                                    "INSERT INTO hrgrowthh (ztime,zid,xrow,xsession,xtype,xstatus,zemail,xdate,xclass,xsection) " +
                                    "VALUES(@ztime,@zid,@xrow,@xsession,@xtype,@xstatus,@zemail,@xdate,@xclass,@xsection) ";
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            if (xrow == 0)
                            {
                                ...
                                query = "UPDATE hrgrowthh SET zutime=@zutime,xemail=@xemail,xsession=@xsession,xdate=@xdate,xclass=@xclass,xsection=@xsection " +
                            ...
                            foreach (GridViewRow row in GridView1.Rows)
                            {
                                ...
                                cmd.Parameters.AddWithValue("@xdesc", lblxdesc.Text.ToString().Trim() == String.Empty ? "" : lblxdesc.Text.Trim().ToString());
                                cmd.Parameters.AddWithValue("@xcat", row.Cells[7].Text.ToString());
                                cmd.Parameters.AddWithValue("@xremarks", txtComments.Text.Trim().ToString() == String.Empty ? "" : txtComments.Text.Trim().ToString());
                                cmd.Parameters.AddWithValue("@xrow1", row.Cells[8].Text.ToString());
                                cmd.Parameters.AddWithValue("@xteacher1", txtTeacher.Text.Trim().ToString() == String.Empty ? "" : txtTeacher.Text.Trim().ToString());
                                cmd.Parameters.AddWithValue("@xclasssub", txtClassSub.Text.Trim().ToString() == String.Empty ? "" : txtClassSub.Text.Trim().ToString());
                                cmd.Parameters.AddWithValue("@xaction", txtAction.Text.Trim().ToString() == String.Empty ? "" : txtAction.Text.Trim().ToString());
                                cmd.ExecuteNonQuery();
                            }
                ...
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    if (!xshowall)
                    {
                        query =
                            ...
                            " xrow,xsession,xdate " +
                            "FROM hrgrowthh WHERE zid=@zid AND xtype='Class Teacher' AND xstatus='New' and zemail=@zemail order by xrow desc";
                    }
                    else
                    {
                        query =
                            "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) +
                            " xrow,xsession,xdate " +
                            "FROM hrgrowthh WHERE zid=@zid AND xtype='Class Teacher' AND xstatus='New' order by xrow desc";
                    }
                ...
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    if (!xshowall)
                    {
                        query =
                         ...
                         "FROM hrgrowthh WHERE zid=@zid AND xtype='Class Teacher' AND xstatus='Submited' and zemail=@zemail order by xrow desc";
                    }
                    else
                    {
                        query =
                         "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " xrow,xsession,xdate " +
                         "FROM hrgrowthh WHERE zid=@zid AND xtype='Class Teacher' AND xstatus='Submited'  order by xrow desc";
                    }
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                string str = "SELECT  xsession,xdate,xclass,xsection " +
