...
{
    public partial class hrgrowthh_corpttemp : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            str = "select ROW_NUMBER() OVER (ORDER BY xcodealt,xcat,xline) AS xid,xrow,xline,xdesc,xcat FROM hrgrowthset as a inner join mscodesam as b " +
                  ...
                  "WHERE a.zid=@zid AND a.xtype='Coordinator' and a.zactive=1 order by xcodealt,xcat,xline ";
                    ...
                    if (GridView1.Rows.Count > 0)
                    {
                        if (ViewState["xrow"] == null)
                        {
                            ...
                            using (SqlConnection con = new SqlConnection(zglobal.constring))
                            {
                                using (DataSet dts = new DataSet())
                                {
                                    ...
                                    string query =
                                        ...
                                        "SELECT * FROM hrgrowthh WHERE zid=@zid AND xsession=@xsession AND xdate=@xdate AND xtype='Coordinator'";
                            ...
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query =
                                    "INSERT INTO hrgrowthh (ztime,zid,xrow,xsession,xtype,xstatus,zemail,xdate,xclass) " +
                                    "VALUES(@ztime,@zid,@xrow,@xsession,@xtype,@xstatus,@zemail,@xdate,@xclass) ";
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            if (xrow == 0)
                            {
                                ...
                                query = "UPDATE hrgrowthh SET zutime=@zutime,xemail=@xemail,xsession=@xsession,xdate=@xdate,xclass=@xclass " +
                ...
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    if (!xshowall)
                    {
                        query =
                            ...
                            " xrow,xsession,xdate " +
                            "FROM hrgrowthh WHERE zid=@zid AND xtype='Coordinator' AND xstatus='New' and zemail=@zemail order by xrow desc";
                    }
                    else
                    {
                        query =
                            "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) +
                            " xrow,xsession,xdate " +
                            "FROM hrgrowthh WHERE zid=@zid AND xtype='Coordinator' AND xstatus='New' order by xrow desc";
                    }
                ...
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    if (!xshowall)
                    {
                        query =
                         ...
                         "FROM hrgrowthh WHERE zid=@zid AND xtype='Coordinator' AND xstatus='Submited' and zemail=@zemail order by xrow desc";
                    }
                    else
                    {
                        query =
                         "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " xrow,xsession,xdate " +
                         "FROM hrgrowthh WHERE zid=@zid AND xtype='Coordinator' AND xstatus='Submited'  order by xrow desc";
                    }
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                string str = "SELECT  xsession,xdate,xclass " +
