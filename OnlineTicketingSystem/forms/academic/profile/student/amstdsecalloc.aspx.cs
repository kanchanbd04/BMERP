...
{
    public partial class amstdsecalloc : System.Web.UI.Page
    {
...
//                   ORDER BY xname";
            ...
            str = @"select xrow, xname, xstdid,
                   case when (select count(*) from amexamvwnew as a where zid=a.zid and xsession=amstudentvw.xsession and xsrow=amstudentvw.xrow) > 0 then 'Locked' else 'Open' end
                   as xstatus, xsection, xgroup, xprow as xrow1,xtype,(SELECT xcodealt FROM mscodesam WHERE zid=amstudentvw.zid AND xtype='Section' AND zactive=1 and xcode=amstudentvw.xsection) as xcodealt
                   from amstudentvw
                    ...
                    zglobal.fnGetComboDataCD("Section", dlistxsection);
                        ...
                        if (ViewState["ampromotiond"] != null)
                        {
                            ...
                            if (result.Length > 0)
                            {
                                ...
                                //}
                                dlistxsection.Text = result[0]["xsection"].ToString();
                                dlistxgroup.Text = result[0]["xgroup"].ToString();
                            }
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    if (GridView1.Rows.Count > 0)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            foreach (GridViewRow row in GridView1.Rows)
                            {
                                if (row.CssClass == "titleval")
                                {
                                    continue;
                                }


                                DropDownList xsection123 = row.FindControl("xsection") as DropDownList;
                                DropDownList xgroup123 = row.FindControl("xgroup") as DropDownList;

                                if (row.Cells[row.Cells.Count - 3].Text.ToString().ToLower() == "new" ||
                                    row.Cells[row.Cells.Count - 3].Text.ToString().ToLower() == "old")
                                {
                                    query = "UPDATE amadmis SET xsection=@xsection,xgroup=@xgroup " +
                                        "WHERE zid=@zid and xrow=@xsrow";
                                }
                                else
                                {
                                    query = "UPDATE ampromotiond SET xsection=@xsection,xgroup=@xgroup " +
                                ...
                                cmd.Parameters.AddWithValue("@xrow", Int64.Parse(row.Cells[row.Cells.Count - 2].Text.ToString()));
                                ...
                                cmd.Parameters.AddWithValue("@xsection", xsection123.Text.ToString());
                                cmd.Parameters.AddWithValue("@xgroup", xgroup123.Text.ToString());
                        ...
                        zglobal.fnRecordLog("Student Section Allocation","Save","amstdsecalloc","","",0,DateTime.Now);
                    }
                    else
                    {
