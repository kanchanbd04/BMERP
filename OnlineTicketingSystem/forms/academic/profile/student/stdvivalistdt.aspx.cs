...
{
    public partial class stdvivalistdt : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    xprintlistfor.Items.Add("Qualiifed");
        ...
        private void BindGrid()
        {
            ...
            DateTime xvivadate1 = xvivadate.Text.Trim() != string.Empty ? DateTime.ParseExact(xvivadate.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture) : DateTime.ParseExact("01/01/1999", "dd/MM/yyyy", CultureInfo.InvariantCulture);
            if (xstatus2 == "Qualiifed")
            {
                xstatus2 = "Approved";
            }
            ...
            string str = "SELECT   xrow,ROW_NUMBER() OVER (ORDER BY xrow) AS xid, xname, xexamroll, coalesce(xstatus,'n') AS xstatus,xfname,xremarks FROM amadmis WHERE zid=@zid AND xsession=@xsession AND xnumexam=@xnumexam AND xclass=@xclass AND xgroup=@xgroup AND xstatus=@xstatus AND xvivadate=@xvivadate";
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    for (i = 4; i < 4 + (int)ViewState["numrow"]; i++)
                    {
                        ...
                        //txTextBox.TextChanged += new System.EventHandler(this.TxTextBoxOnTextChanged);
                        e.Row.Cells[i].Controls.Add(txTextBox);
                        //txTextBox.AutoPostBack = true;
                        if (xst != "n")
                        {
        ...
        protected void TxTextBoxOnTextChanged(object sender, EventArgs eventArgs)
        {
            try
            {
                TextBox txt = (TextBox) sender;
                //int RowIndex = Convert.ToInt32(EventArgs.CommandArgument.ToString());
                Label lbl = GridView1.Rows[1].FindControl("xstotmarks") as Label;
                lbl.Text = txt.Text;
            }
            catch (Exception exp)
            {
                    ...
                    foreach (GridViewRow row in GridView1.Rows)
                    {
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                xrow = Int32.Parse(row.Cells[6 + (int)ViewState["numrow"]].Text.ToString());
            ...
            try
            {
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        DateTime zutime = DateTime.Now;
                        _zid = Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"]));
                        int xrow = Convert.ToInt32(_row.Value != "" ? _row.Value : "0");
                        ...
                        if (TabContainer1.ActiveTabIndex == 1)
                        {
                            ...
                            if (GridView1.Rows.Count > 0)
                            {
                                ...
                                foreach (GridViewRow row in GridView1.Rows)
                                {
                                   ...
                                   // CheckBox chkBox = row.FindControl("promoted") as CheckBox;
                                    ...
                                    if (xstatus == "n")
                                    {
                                        ...
                                        int passmarks = Int32.Parse(xpassmarks.Text.TrimEnd('%'));
                                        CheckBox chkPromoted = row.FindControl("promoted") as CheckBox;
                                        if (Int32.Parse(lblPass.Text) >= passmarks)
                                        {
            ...
            try
            {
                ...
                try
                {
                    if (TabContainer1.ActiveTabIndex == 1)
                    {
                        ...
                        //crystalReport.Load(Server.MapPath("~/reports/stdresult.rpt")); // path of report 
                        ...
                        //string fileName = "Result_" + xsession1 + "_" + xclass1 + "_" + xgroup1 + "_" + xnumexam1;
        ...
        private void fnPrintList(string format)
        {
            int _zid = Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"]));
            string xsession1 = xsession_res.Text.Trim().ToString();
            string xclass1 = xclass_res.Text.Trim().ToString();
