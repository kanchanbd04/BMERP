...
{
    public partial class stdresvivaapp : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    getActiveTabIndex.Value = "1";
                    type.Value = "out";
                    TabContainer1.Height = 165;

                    //xvivadate.Text = DateTime.Now.ToString("dd/MM/yyyy");
        ...
        private void fnGetVivaDate()
        {
            using (SqlConnection conn = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts = new DataSet())
                {
                    ...
                    string query;
                    if (xschfor.Text == "Qualified")
                    {
                        query = "select distinct xvivadate from amadmis where zid=@zid and xsession=@xsession and xclass=@xclass and xgroup=@xgroup and xnumexam=@xnumexam and xstatus='Approved'";
                    }
                    else if (xschfor.Text == "Waiting")
                    {
                        query = "select distinct xvivadate from amadmis where zid=@zid and xsession=@xsession and xclass=@xclass and xgroup=@xgroup and xnumexam=@xnumexam and xstatus='Waiting'";
                    }
                    else
                    {
                        query = "";
                    ...
                    dtexam = dts.Tables[0];
                    xvivadate.Items.Clear();
                    xvivadate.Items.Add("");
                    if (dtexam.Rows.Count > 0)
                    {
                        foreach (DataRow row in dtexam.Rows)
                        {
                            if (row["xvivadate"].Equals(DBNull.Value) == false)
                            {
                                xvivadate.Items.Add(Convert.ToDateTime(row["xvivadate"]).ToString("dd/MM/yyyy"));
                            }
                        }
                    }
                    xvivadate.Text = "";
        ...
        private void BindGrid()
        {
            ...
            string xclass1 = xclass_res.Text.Trim().ToString();
            string xgroup1 = xgroup_res.Text.Trim().ToString();
            string xstatus1 = xschfor.Text.Trim().ToString();
            DateTime xvivadate1 = xvivadate.Text.Trim() != string.Empty ? DateTime.ParseExact(xvivadate.Text.Trim().ToString(), "dd/MM/yyyy", CultureInfo.InvariantCulture) : DateTime.ParseExact("31/12/2999", "dd/MM/yyyy", CultureInfo.InvariantCulture);
            ...
            string str = "SELECT   xrow,ROW_NUMBER() OVER (ORDER BY xrow) AS xid, xname, xexamroll, coalesce(xstatus,'n') AS xstatus,xfname,xremarks,coalesce(xstatus1,'n') AS xstatus1,xvivares,xremarks1,xnumexam FROM amadmis WHERE zid=@zid AND xsession=@xsession AND xnumexam=@xnumexam AND xclass=@xclass AND xgroup=@xgroup AND xstatus=@xstatus AND xvivadate=@xvivadate";
            ...
            da.SelectCommand.Parameters.AddWithValue("@xclass", xclass1);
            da.SelectCommand.Parameters.AddWithValue("@xgroup", xgroup1);
            da.SelectCommand.Parameters.AddWithValue("@xstatus", xstatus1);
            da.SelectCommand.Parameters.AddWithValue("@xvivadate", xvivadate1);
            da.Fill(dts, "tempztcode");
            ...
            if (dtmarks.Rows.Count > 0)
            {
                ...
                //if (dtmarks.Rows[0]["xstatus1"].ToString() != "n")
                //{
                ...
                if (dtmarks.Rows[0]["xstatus1"].ToString() != "n")
                {
                    if (dtmarks.Rows[0]["xnumexam"].ToString() == "Special" || xclass_res.Text.ToString() == "Playgroup")
                    {
                        btnEdit.Enabled = true;
                        btnApproved.Enabled = true;
                    }
                ...
                if (dtmarks2.Rows.Count > 0)
                {
                    ...
                    tfield5.HeaderText = "Viva Result";
        ...
        protected void OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    int i = 0;
                    string xst = (e.Row.DataItem as DataRowView).Row["xstatus1"].ToString();
                    ...
                    DropDownList dwVivaRes = new DropDownList();
                    dwVivaRes.ID = "xvivares";
                    dwVivaRes.CssClass = "bm_academic_dropdown_50_50 bm_academic_ctl_global bm_academic_dropdown";
                    dwVivaRes.Items.Add("");
                    dwVivaRes.Items.Add("A*");
                    dwVivaRes.Items.Add("A");
                    dwVivaRes.Items.Add("B");
                    dwVivaRes.Items.Add("C");
                    dwVivaRes.Items.Add("D");
                    dwVivaRes.Text = "";
                    e.Row.Cells[i + 5].Controls.Add(dwVivaRes);
                    dwVivaRes.Text = (e.Row.DataItem as DataRowView).Row["xvivares"].ToString();

                    txTextBox = new TextBox();
                    txTextBox.ID = "txtRemarks1";
                    txTextBox.CssClass = "bm_academic_textbox_grid bm_academic_ctl_global bm_academic_textbox";
                    txTextBox.TextMode = TextBoxMode.MultiLine;
                    e.Row.Cells[i + 6].Controls.Add(txTextBox);
                    txTextBox.Text = (e.Row.DataItem as DataRowView).Row["xremarks1"].ToString();
                    if (xst != "n")
                    {
                        txTextBox.Enabled = false;
                        dwVivaRes.Enabled = false;
                    }

                    CheckBox chkCheckBox2 = new CheckBox();
                    chkCheckBox2.ID = "promoted";
                    e.Row.Cells[i + 7].Controls.Add(chkCheckBox2);

                    CheckBox chkCheckBox1 = new CheckBox();
                    chkCheckBox1.ID = "waiting";
                    e.Row.Cells[i + 8].Controls.Add(chkCheckBox1);
                    ...
                    else if (xst == "Approved")
                    {
                        chkCheckBox2.Checked = true;
                        chkCheckBox1.Checked = false;
                        
                    }
                    else  if (xst == "Canceled")
                    {
                        chkCheckBox2.Checked = false;
                        chkCheckBox1.Checked = false;
                    }
                        ...
                        foreach (GridViewRow row in GridView1.Rows)
                        {
                            using (TransactionScope tran = new TransactionScope())
                            {
                                using (SqlConnection conn = new SqlConnection(zglobal.constring))
                                {
                                    ...
                                    CheckBox chkWaiting = row.FindControl("waiting") as CheckBox;
                                    if (chkPromoted.Checked)
                                    {
                                        xstatus = "Approved";
                                    }
                                    else if (chkWaiting.Checked)
                                    {
                                        xstatus = "Waiting";
                                    }
                                    else
                                    ...
                                    TextBox txtTextBox1 = row.FindControl("txtRemarks1") as TextBox;
                                    string xremarks = txtTextBox1.Text.Trim().ToString();

                                    DropDownList xvivares = row.FindControl("xvivares") as DropDownList;
                                    string xvivares1 = xvivares.Text.Trim().ToString();

                                    string query =
                                        "UPDATE amadmis SET xvivares=@xvivares,xremarks1=@xremarks1,xstatus1=@xstatus1,xapprovedby1=@xapprovedby1,xapproveddt1=@xapproveddt1 " +
                                    ...
                                    cmd.Parameters.AddWithValue("@zid", _zid);
                                    cmd.Parameters.AddWithValue("@xrow", xrow);
                                    cmd.Parameters.AddWithValue("@xvivares", xvivares1);
                                    cmd.Parameters.AddWithValue("@xremarks1", xremarks);
                                    cmd.Parameters.AddWithValue("@xstatus1", xstatus);
                                    cmd.Parameters.AddWithValue("@xapprovedby1", xapprovedby);
                                    cmd.Parameters.AddWithValue("@xapproveddt1", xapproveddt);
                                    cmd.ExecuteNonQuery();
                                }
                        ...
                        if (TabContainer1.ActiveTabIndex == 1)
                        {
                            ...
                            if (GridView1.Rows.Count > 0)
                            {
                                ...
                                foreach (GridViewRow row in GridView1.Rows)
                                {
                                    ...
                                    totmark = 0;
                                    xrow = Int64.Parse(row.Cells[6 + (int)ViewState["numrow"]].Text.ToString());
                                    xstatus = row.Cells[13 + (int) ViewState["numrow"]].Text.ToString();
                                    TextBox txtTextBox1 = row.FindControl("txtRemarks1") as TextBox;
                                    xremarks = txtTextBox1.Text.Trim().ToString();

                                    DropDownList xvivares = row.FindControl("xvivares") as DropDownList;
                                    xvivares1 = xvivares.Text.Trim().ToString();

                                    if (xstatus == "n")
                                    {
                                        query = "UPDATE amadmis SET xvivares=@xvivares,xremarks1=@xremarks1,zutime=@zutime,xemail=@xemail " +
                                                "WHERE zid=@zid AND xrow=@xrow ";
                                        cmd.Parameters.Clear();
