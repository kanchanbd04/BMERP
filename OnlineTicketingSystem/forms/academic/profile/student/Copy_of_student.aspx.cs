...
{
    public partial class Copy_of_student  : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if ((xclass_res.Text != "" || xclass_res.Text != string.Empty || xclass_res.Text != "[Select]") &&
                        (_row.Value != "" || _row.Value != string.Empty))
                    {
                        int row = Convert.ToInt32(_row.Value);
                        fnCreateConteolMarks(xclass_res.Text.ToString(), row);
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (TabContainer1.ActiveTabIndex == 1)
                        {
                            ...
                            dts.Reset();
                            string str = "select amadmis.zid as zid,  amadmis.xrow as xrow,ammarks.xrow as xrowmarks, ammarks.xsubject as xsubject, coalesce(ammarks.xmark,0) as xmark, coalesce(ammarks.xpassmark,0) as xpassmark from amadmis inner join  amadmisd on amadmis.zid=amadmisd.zid and amadmis.xrow=amadmisd.xrow and amadmis.xrow=@xrow " +
                                            " right outer join ammarks on amadmisd.zid=ammarks.zid and amadmisd.xrowmarks=ammarks.xrow " +
                                            " where ammarks.zid=@zid and ammarks.xclass=@xclass and xtype='admistest' and " +
                                            "xeffdate=(select MAX(xeffdate)from ammarks where zid=@zid and ammarks.xclass=@xclass and xeffdate<=coalesce(xexamdate,getdate()) and xtype='admistest') ";
                            ...
                            DataTable dtmarks = new DataTable();
                            dtmarks = dts.Tables[0];
                            //message.InnerHtml = dtmarks.Rows.Count.ToString() +" - " +  _zid.ToString() + " - " + xrow.ToString() + " - " + xclass1;
                            if (dtmarks.Rows.Count > 0)
                            {
                                ...
                                {
                                    cmd.Parameters.Clear();
                                    int xline = zglobal.GetMaximumIdInt("xline", "amadmisd", " zid=" + _zid + " and xrow=" + xrow, "line");
                                    int xgetmark;
                                    string marks2;
                                    string marks1;
                                    ...
                                    query = "INSERT INTO amadmisd (zid,xrow,xline,xrowmarks,xsubject,xmark,xpassmark,xgetmark) " +
                                        "VALUES(@zid,@xrow,@xline,@xrowmarks,@xsubject,@xmark,@xpassmark,@xgetmark) ";
                                    ...
                                    cmd.Parameters.AddWithValue("@xmark", Convert.ToInt32(row["xmark"]));
                                    cmd.Parameters.AddWithValue("@xpassmark", Convert.ToInt32(row["xpassmark"]));
