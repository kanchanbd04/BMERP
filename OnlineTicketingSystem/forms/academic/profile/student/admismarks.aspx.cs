...
{
    public partial class admismarks : System.Web.UI.Page
    {
                ...
                if (TabContainer1.ActiveTabIndex == 0)
                {
                    if (grid_xnumexam.Text == "All")
                    {
                        str = "SELECT TOP " + _gridrow.Text.Trim().ToString() +
                              " xrow,xname,xexamroll,xexamdate,xexamvenue,(xcellno +', '+ xcellno1) as xcontact,xgender FROM amadmis where zid=@zid and xsession=@xsession and xclass=@xclass and xgroup=@xgroup AND coalesce(xstatus3,'')<>'Revised'  ORDER BY xrow DESC";
                    }
                    else
                    {
                        str = "SELECT TOP " + _gridrow.Text.Trim().ToString() +
        ...
        private void BindGrid()
        {
            ...
            string str = "SELECT   xrow,ROW_NUMBER() OVER (ORDER BY xrow) AS xid, xname, xexamroll, coalesce(xstatus,'n') AS xstatus,xfname,xremarks,xnumexam, coalesce(xstatus1,'n') AS xstatus1 " +
                         "FROM amadmis WHERE zid=@zid AND xsession=@xsession AND xnumexam=@xnumexam AND xclass=@xclass AND xgroup=@xgroup AND xnumexam<>'' AND " +
                         "coalesce(xstatus3,'')<>'Revised' AND coalesce(xstatus4,'')<>'old' ";
            ...
            //foreach (DataRow row1 in dtmarks.Rows)
            //{
            //    message.InnerHtml = message.InnerHtml + row1["xexamroll"].ToString() + " " + row1["xname"].ToString() + "<br>";
            //}
            //return;
            ...
            if (dtmarks.Rows.Count > 0)
            {
                ...
                //{
                //    //if (dtmarks.Rows[0]["xnumexam"].ToString() != "Special" && xclass.Text.ToString() != "Playgroup")
                //    //{
                ...
                string str2 = "SELECT xsubject,xmark, xpassmark FROM  ammarks " +
                              ...
                              "and xeffdate<=@xexamdate and xtype='admistest' AND zactive=1 AND xstatus='Approved' ) AND zactive=1 AND xstatus='Approved' " +
                              "AND xgroup=@xgroup";
                ...
                //message.InnerHtml = _zid.ToString() + " " + xsession1 + " " + xnumexam1 + " " + xclass1 + " " + xgroup1 + xexamdate1;
                //return;
                ...
                {
                    //foreach (DataRow row1 in dtmarks2.Rows)
                    //{
                    //    message.InnerHtml = message.InnerHtml + row1["xsubject"].ToString() + " " + row1["xmark"].ToString() + "<br>";
                    //}
                    //return;
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    for (i = 4; i < 4 + (int)ViewState["numrow"]; i++)
                    {
                        ...
                        //    if (xclass_res.Text.ToString() == "Playgroup")
                        //    {
                        //        if (xst != "Canceled")
                        //        {
                        //            txTextBox.Enabled = false;
        ...
        protected void FillControls(object sender, EventArgs e)
        {
            try
            {
                ...
                int _zid = Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"]));
                Int64 xrow = Convert.ToInt64(((LinkButton)sender).Text);
        ...
        protected void btnEdit_Click(object sender, EventArgs e)
        {
            try
            {
                        ...
                        if (TabContainer1.ActiveTabIndex == 1)
                        {
                            ...
                            if (GridView1.Rows.Count > 0)
                            {
                                ...
                                foreach (GridViewRow row in GridView1.Rows)
                                {
                                    ...
                                    //if (xstatus == "n" || (xclass_res.Text == "Playgroup" && xstatus == "Canceled") || (xnumexam_res.Text == "Special" && xstatus == "Canceled"))
                                    {
                ...
                _row_global.Value = _row.Value != ""  ? _row.Value : "0";
            }
            catch (Exception exp)
            {
                message.InnerHtml = exp.Message;
