...
{
    public partial class amsecinterchange : System.Web.UI.Page
    {
        private void fnButtonState()
        {
            ...
            else
            {
                ...
                string xstatus1 = zglobal.fnGetValue("xstatus", "amsecinterchange",
                               "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
                hxstatus.Value = xstatus1;
                xstatus.InnerHtml = xstatus1;
                zemail.InnerHtml = zglobal.fnGetValue("coalesce(zemail,'') as zemail", "amsecinterchange",
                               "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
                xapprovedby.InnerHtml = zglobal.fnGetValue("coalesce(xapprovedby,'') as xapprovedby", "amsecinterchange",
                               "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    zglobal.fnGetComboDataCD("Section", xsectionnew);
                    zglobal.fnGetComboDataCD("Education Group", xgroupnew);
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xsectionnew.Text == "" || xsectionnew.Text == string.Empty || xsectionnew.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>New Section</li>";
                    isValid = false;
                }
                ...
                if (ViewState["xrow"] != null)
                {
                    xstatus2 = zglobal.fnGetValue("xstatus", "amsecinterchange",
                          "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
                        ...
                        string query =
                            "select * from amexamhh where zid=@zid and xsession=@xsession and xclass=@xclass and coalesce(xterm,'')=@xterm and xgroup=@xgroup and xtype=@xtype and xstatus='Approved'";
                        ...
                        da1.SelectCommand.Parameters.AddWithValue("@xterm", "");
                        da1.SelectCommand.Parameters.AddWithValue("@xgroup", xgroup.Text.ToString().Trim());
                        da1.SelectCommand.Parameters.AddWithValue("@xtype", "Average");
                        ...
                        {
                            message.InnerHtml = "Final Result already approved. Cann't change section.";
                ...
                //        string query =
                //            "select * from amsecinterchange where zid=@zid and xsession=@xsession  and xsection=@xsection and xgroup=@xgroup ";
                ...
                //        if (dtamexamh.Rows.Count > 0)
                //        {
                //            message.InnerHtml = "Duplicate record.";
                //            message.Style.Value = zglobal.am_warningmsg;
                //            return;
                //        }
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    if (ViewState["xrow"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                                "INSERT INTO amsecinterchange (ztime,zid,xrow,xsession,xsrow,xsection,xsectionnew,xstatus,xremarks,zemail,xgroupnew) " +
                                "VALUES(@ztime,@zid,@xrow,@xsession,(select xrow from amadmis where zid=@zid and xstdid=@xstdid),@xsection,@xsectionnew,@xstatus,@xremarks,@zemail,@xgroupnew) ";

                            xrow = zglobal.GetMaximumIdInt("xrow", "amsecinterchange", " zid=" + _zid, 1, 1);
                            ViewState["xrow"] = xrow;
                            xsession1 = xsession.Text.Trim().ToString();
                            xstdid1 = xstdid.Text.Trim().ToString();
                            xsection1 = xsection.Text.Trim().ToString();
                            xsectionnew1 = xsectionnew.Text.Trim().ToString();
                            xgroupnew1 = xgroupnew.Text.Trim().ToString();
                            ...
                            cmd.Parameters.AddWithValue("@xsectionnew", xsectionnew1);
                            cmd.Parameters.AddWithValue("@xgroupnew", xgroupnew.Text.ToString());
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xrow == 0)
                        {
                            ...
                            query = "UPDATE amsecinterchange SET zutime=@zutime,xemail=@xemail, " +
                                   "xsection=@xsection,xsectionnew=@xsectionnew,xremarks=@xremarks,xgroupnew=@xgroupnew " +
        ...
        private void fnCheckRef(DataTable tblPrevSecData, DataTable tblNewSecData, string xrefcttype, string xrefctno, string xterm, string xsubject, 
            string xpaper, string xext, string xtype, SqlCommand cmd, int _zid)
        {
            
            DataRow[] resPrevSecData;
            resPrevSecData =
                tblPrevSecData.Select(
                    "xterm='" + xterm +
                    "' and xsubject='" + xsubject +
                    "' and xpaper='" + xpaper +
                    "' and xext='" + xext +
                    "' and xtype='" + xtype +
                    "' and xrefcttype='" + xrefcttype +
                    "' and xrefctno='" + xrefctno + "'");
          
            if (resPrevSecData.Length > 0)
            {

                string query = "";


                foreach (DataRow row in resPrevSecData)
                {
                    DataRow[] result;
                    result =
                        tblNewSecData.Select(
                            "xterm='" + row["xterm"].ToString() +
                            "' and xsubject='" + row["xsubject"].ToString() +
                            "' and xpaper='" + row["xpaper"].ToString() +
                            "' and xext='" + row["xext"].ToString() +
                            "' and xtype='" + row["xtype"].ToString() +
                            "' and xrefcttype='" + row["xrefcttype"].ToString() +
                            "' and xrefctno='" + row["xrefctno"].ToString() + "'");
                    if (result.Length > 0)
                    {
                        query =
                    "UPDATE amexamd SET xline=@xline,xrow=@xrow,xctno1=@xctno1,xgetmark=@xgetmark*@xmarks1/@xmarks,xinterchange=1, " +
                    "xinterchangedate=@xinterchangedate, xinterchangeexamdate=@xinterchangeexamdate  " +
                    "WHERE zid=@zid AND xrow=@xrow1 AND xsrow=@xsrow";

                        cmd.CommandText = query;
                        cmd.Parameters.Clear();
                        cmd.Parameters.AddWithValue("@zid", _zid);
                        cmd.Parameters.AddWithValue("@xrow1",
                            Convert.ToInt32(row["xrow"].ToString()));
                        cmd.Parameters.AddWithValue("@xrow",
                            Convert.ToInt32(result[0]["xrow"].ToString()));
                        cmd.Parameters.AddWithValue("@xsrow",
                            Convert.ToInt64(row["xsrow"].ToString()));
                        cmd.Parameters.AddWithValue("@xline",
                            zglobal.GetMaximumIdInt("xline", "amexamd",
                                " zid=" + _zid + " and xrow=" +
                                Convert.ToInt32(result[0]["xrow"].ToString()), "line"));
                        cmd.Parameters.AddWithValue("@xctno1", row["xctno"].ToString());
                        cmd.Parameters.AddWithValue("@xgetmark",
                            Convert.ToDecimal(row["xgetmark"].ToString()));
                        cmd.Parameters.AddWithValue("@xmarks",
                            Convert.ToDecimal(row["xmarks"].ToString()));
                        cmd.Parameters.AddWithValue("@xmarks1",
                            Convert.ToDecimal(result[0]["xmarks"].ToString()));
                        cmd.Parameters.AddWithValue("@xinterchangedate", DateTime.Now);
                        cmd.Parameters.AddWithValue("@xinterchangeexamdate", Convert.ToDateTime(row["xdate"].ToString()));
                        cmd.ExecuteNonQuery();
                    }
                    else
                    {
        ...
        private decimal fnMaxGetMarkRef(DataTable tblPrevSecData, string xrefcttype, string xrefctno, string xterm, string xsubject,
            string xpaper, string xext, string xtype, decimal xgetmark, SqlCommand cmd, int _zid)
        {
            decimal maxmarks = xgetmark;
            //decimal tempmarks = marks;
            DataRow[] resPrevSecData;
            resPrevSecData =
                tblPrevSecData.Select(
                    "xterm='" + xterm +
            ...
            if (resPrevSecData.Length > 0)
            {
                ...
                foreach (DataRow row in resPrevSecData)
                {
                    if (maxmarks < Convert.ToDecimal(row["xgetmark"].ToString()))
                    {
                        maxmarks = Convert.ToDecimal(row["xgetmark"].ToString());
                    }

                    string query = "DELETE FROM amexamd WHERE zid=@zid AND xrow=@xrow AND xsrow=@xsrow";
                    cmd.Parameters.Clear();

                    cmd.CommandText = query;
                    cmd.Parameters.AddWithValue("@zid", _zid);
                    cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(row["xrow"].ToString()));
                    cmd.Parameters.AddWithValue("@xsrow", Convert.ToInt64(row["xsrow"].ToString()));
                    cmd.ExecuteNonQuery();
                }
            }
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                         ...
                         "(select xname from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xname, " +
                         "(select xstdid from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xstdid, " +
                         "(select xclass from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xclass, " +
                         "(select xgroup from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xgroup, " +
                        //"(select xsection from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xsection, " +
                         "xstatus, xsectionnew,xsection " +
                         "FROM amsecinterchange WHERE zid=@zid AND xstatus='New' order by xrow desc";
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                        ...
                        //"(select xsection from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xsection, " +
                         "xstatus, xsectionnew,xsection " +
                         "FROM amsecinterchange WHERE zid=@zid AND xstatus='Approved' order by xrow desc";
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                string str = "SELECT  xsession, " +
                             ...
                             "(select xgroup from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xgroup, " +
                    //"(select xsection from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xsection, " +
                             "(select xfname from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xfname, " +
                             "(select xmname from amstudentvw where zid=amsecinterchange.zid and xsession=amsecinterchange.xsession and xrow=amsecinterchange.xsrow) as xmname, " +
                             "xsectionnew,xstatus,zemail,xapprovedby,xremarks,xsection,xgroupnew " +
                             " FROM amsecinterchange where zid=@zid  and xrow=@xrow ";
                ...
                xsectionnew.Text = dttemp.Rows[0]["xsectionnew"].ToString();
                xgroupnew.Text = dttemp.Rows[0]["xgroupnew"].ToString();
        ...
        private void fnSave()
        {
            ...
            //{
                ...
                //        string query =
                //            "select * from amexamhh where zid=@zid and xsession=@xsession and xclass=@xclass and coalesce(xterm,'')=@xterm and xgroup=@xgroup ";
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xrow"] == null)
                    {
                        ...
                        //        con.Open();
                        //        string query =
                        //            "SELECT * FROM amsecinterchange WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where xstdid=@xstdid) ";
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        using (DataSet dts = new DataSet())
                        {
                            ...
                            query =
                                ...
                                "WHERE h.zid=@zid AND  d.xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid) AND h.xsessionpro=@xsession and coalesce(xpromoted,0) = 1";
                            ...
                            if (tempTable.Rows.Count > 0)
                            {
                                //message.InnerHtml = "inside promotion";
                                query =
                                    "UPDATE ampromotiond SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xrow=@xrow AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                                ...
                                cmd.Parameters.AddWithValue("@xsection", xsectionnew.Text.ToString().Trim());
                                cmd.Parameters.AddWithValue("@xgroup", xgroupnew.Text.ToString().Trim());
                                cmd.ExecuteNonQuery();
                            }
                            else
                            {
                                //message.InnerHtml = "inside admis";
                                query =
                                   "UPDATE amadmis SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                                    "UPDATE amexamph_sumext_exam SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                                   "UPDATE amexamph_sumext_exam2_sub SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                                   "UPDATE amexamph_sumext_exam2_wt1 SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                                   "UPDATE amexamph_sumext_exam_vw SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                                   "UPDATE amexamph_sumext_exam2_sub_vw SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                                   "UPDATE amexamph_sumext_exam2_wt1_vw SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                                   "UPDATE amexamph_result_avg1 SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                                  "UPDATE amexamph_result_avg_sub SET xsection=@xsection,xgroup=@xgroup WHERE zid=@zid AND xsession=@xsession AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                        ...
                        using (DataSet dts = new DataSet())
                        {
                            //conn.Open();
                            //query =
                            //    "SELECT * FROM amexamh as h inner join amexamd as d on h.zid=d.zid and h.xrow=d.xrow  " +
                            //    "WHERE h.zid=@zid AND  d.xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid) " +
                            //    "AND h.xsession=@xsession and h.xclass=@xclass and h.xgroup=@xgroup and xsection=@xsection";

                            //query =
                            //    "SELECT * FROM amexamvwnew " +
                            //    "WHERE zid=@zid AND  xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid) " +
                            //    "AND xsession=@xsession and xclass=@xclass and xgroup=@xgroup and xsection=@xsection AND " +
                            //    "xcttype not in (SELECT xcode FROM mscodesam WHERE zid=amexamvwnew.zid AND xtype='Test Type' AND coalesce(xcodealt1,'') in ('sub'))";

                            query =
                                "SELECT * FROM amexamvwnew " +
                                "WHERE zid=@zid AND  xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid) " +
                                "AND xsession=@xsession and xclass=@xclass and xgroup=@xgroup and xsection=@xsection ";
                            ...
                            if (tempTable.Rows.Count > 0)
                            {
                                //int transferredCount = 1;

                                query =
                                    "SELECT * FROM amexamvwnew  " +
                                    "WHERE zid=@zid  AND xsession=@xsession  AND xclass=@xclass AND xgroup=@xgroup AND " +
                                    "xsection=@xsection  ";
                                //"xcttype in (SELECT xcode FROM mscodesam WHERE zid=h.zid AND xtype='Test Type' AND coalesce(xcodealt1,'') in ('main','hidden'))";
                                // "(xcttype in (SELECT xcode FROM mscodesam WHERE zid=amexamvwnew.zid AND xtype='Test Type' AND coalesce(xcodealt1,'') in ('main','hidden')) or xcttype='')";

                                SqlDataAdapter da1 = new SqlDataAdapter(query, conn);
                                da1.SelectCommand.CommandTimeout = 0;
                                da1.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                //da1.SelectCommand.Parameters.AddWithValue("@xsrow", Convert.ToInt64(tempTable.Rows[i]["xsrow"].ToString()));
                                da1.SelectCommand.Parameters.AddWithValue("@xsession", tempTable.Rows[0]["xsession"].ToString());
                                //da1.SelectCommand.Parameters.AddWithValue("@xterm", tempTable.Rows[i]["xterm"].ToString());
                                da1.SelectCommand.Parameters.AddWithValue("@xclass", tempTable.Rows[0]["xclass"].ToString());
                                //da1.SelectCommand.Parameters.AddWithValue("@xgroup", tempTable.Rows[0]["xgroup"].ToString());
                                da1.SelectCommand.Parameters.AddWithValue("@xsection", xsectionnew.Text.Trim().ToString());
                                da1.SelectCommand.Parameters.AddWithValue("@xgroup", xgroupnew.Text.Trim().ToString());
                                // da1.SelectCommand.Parameters.AddWithValue("@xsubject", tempTable.Rows[i]["xsubject"].ToString());
                                //da1.SelectCommand.Parameters.AddWithValue("@xpaper", tempTable.Rows[i]["xpaper"].ToString());
                                //da1.SelectCommand.Parameters.AddWithValue("@xext", tempTable.Rows[i]["xext"].ToString());
                                //da1.SelectCommand.Parameters.AddWithValue("@xctno", row["xcttype"].ToString() == "Transferred" 
                                //                                                    ? row["xctno1"].ToString() 
                                //                                                    : row["xctno"].ToString());
                                //da1.SelectCommand.Parameters.AddWithValue("@xctno", tempTable.Rows[i]["xid"].ToString());
                                //da1.SelectCommand.Parameters.AddWithValue("@xtype", tempTable.Rows[i]["xtype"].ToString());
                                ...
                                query =
                                "select xrow,xsubject,xpaper,xext,xtype, " +
                                "row_number() over(partition by zid,xsession,xterm,xclass,xgroup,xsection,xtype,xsubject,xpaper,xext " +
                                "order by zid,xsession,xterm,xclass,xgroup,xsection,xtype,xsubject,xpaper,xext) as xctno " +
                                "from amexamvwnew where zid=@zid and xstdid=@xstdid and xsession=@xsession ";

                                SqlDataAdapter da1234 = new SqlDataAdapter(query, conn);

                                da1234.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                da1234.SelectCommand.Parameters.AddWithValue("@xsession", xsession.Text.ToString().Trim());
                                da1234.SelectCommand.Parameters.AddWithValue("@xstdid", xstdid.Text.ToString().Trim()); ;

                                DataTable tempTable1234 = new DataTable();
                                //dts.Reset();
                                da1234.Fill(dts, "tempTable1234");
                                tempTable1234 = dts.Tables["tempTable1234"];
                                ...
                                for (int i = 0; i < tempTable.Rows.Count; i++)
                                {
                                    //query =
                                    //    "SELECT * FROM amexamh as h inner join amexamd as d on h.zid=d.zid and h.xrow=d.xrow  " +
                                    //    "WHERE zid=@zid AND xsrow=@xsrow AND xsession=@xsession AND xterm=@xterm AND xclass=@xclass AND xgroup=@xgroup AND " +
                                    //    "xsection=@xsection AND xsubject=@xsubject AND xpaper=@xpaper AND xext=@xext AND xctno=@xctno AND h.xtype=@xtype AND " +
                                    //    //"xcttype in (SELECT xcode FROM mscodesam WHERE zid=h.zid AND xtype='Test Type' AND coalesce(xcodealt1,'') in ('main','hidden'))";
                                    //    "(xcttype in (SELECT xcode FROM mscodesam WHERE zid=h.zid AND xtype='Test Type' AND coalesce(xcodealt1,'') in ('main','hidden')) or xcttype='')";

                                    //query =
                                    //  "SELECT * FROM amexamvwnew  " +
                                    //  "WHERE zid=@zid  AND xsession=@xsession AND xterm=@xterm AND xclass=@xclass AND xgroup=@xgroup AND " +
                                    //  "xsection=@xsection AND xsubject=@xsubject AND xpaper=@xpaper AND xext=@xext AND xctno=@xctno AND xtype=@xtype AND " +
                                    //    //"xcttype in (SELECT xcode FROM mscodesam WHERE zid=h.zid AND xtype='Test Type' AND coalesce(xcodealt1,'') in ('main','hidden'))";
                                    //  "(xcttype in (SELECT xcode FROM mscodesam WHERE zid=amexamvwnew.zid AND xtype='Test Type' AND coalesce(xcodealt1,'') in ('main','hidden')) or xcttype='')";
                                    ...
                                    //da1.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                    //da1.SelectCommand.Parameters.AddWithValue("@xsrow", Convert.ToInt64(tempTable.Rows[i]["xsrow"].ToString()));
                                    //da1.SelectCommand.Parameters.AddWithValue("@xsession", tempTable.Rows[i]["xsession"].ToString());
                                    //da1.SelectCommand.Parameters.AddWithValue("@xterm", tempTable.Rows[i]["xterm"].ToString());
                                    //da1.SelectCommand.Parameters.AddWithValue("@xclass", tempTable.Rows[i]["xclass"].ToString());
                                    //da1.SelectCommand.Parameters.AddWithValue("@xgroup", tempTable.Rows[i]["xgroup"].ToString());
                                    //da1.SelectCommand.Parameters.AddWithValue("@xsection", xsectionnew.Text.Trim().ToString());
                                    //da1.SelectCommand.Parameters.AddWithValue("@xsubject", tempTable.Rows[i]["xsubject"].ToString());
                                    //da1.SelectCommand.Parameters.AddWithValue("@xpaper", tempTable.Rows[i]["xpaper"].ToString());
                                    //da1.SelectCommand.Parameters.AddWithValue("@xext", tempTable.Rows[i]["xext"].ToString());
                                    ...
                                    //string xctno1 = tempTable.Rows[i]["xid"].ToString();

                                    //if (tempTable.Rows[i]["xtype"].ToString() == "Term Exam" ||
                                    //    tempTable.Rows[i]["xtype"].ToString() == "Mock Exam")
                                    //{
                                    //    xctno1 = "";
                                    //}
                                    if (tempTable.Rows[i]["xrefcttype"].ToString() == "")
                                    {
                                        decimal xgetmark = fnMaxGetMarkRef(tempTable,
                                            tempTable.Rows[i]["xcttype"].ToString(),
                                            tempTable.Rows[i]["xctno"].ToString(),
                                            tempTable.Rows[i]["xterm"].ToString(),
                                            tempTable.Rows[i]["xsubject"].ToString(),
                                            tempTable.Rows[i]["xpaper"].ToString(),
                                            tempTable.Rows[i]["xext"].ToString(), tempTable.Rows[i]["xtype"].ToString(),
                                            Convert.ToDecimal(tempTable.Rows[i]["xgetmark"].ToString()), cmd, _zid);

                                        DataRow[] result;
                                        result =
                                            tempTable1.Select(
                                                "xterm='" + tempTable.Rows[i]["xterm"].ToString() +
                                                "' and xsubject='" + tempTable.Rows[i]["xsubject"].ToString() +
                                                "' and xpaper='" + tempTable.Rows[i]["xpaper"].ToString() +
                                                "' and xext='" + tempTable.Rows[i]["xext"].ToString() +
                                                "' and xtype='" + tempTable.Rows[i]["xtype"].ToString() +
                                                "' and xcttype='" + tempTable.Rows[i]["xcttype"].ToString() +
                                                "' and xctno='" + tempTable.Rows[i]["xctno"].ToString() + "'");
                                        ...
                                        if (result.Length > 0)
                                        {
                                            ...
                                            cmd.Parameters.AddWithValue("@zid", _zid);
                                            cmd.Parameters.AddWithValue("@xrow1",
                                                Convert.ToInt32(tempTable.Rows[i]["xrow"].ToString()));
                                            cmd.Parameters.AddWithValue("@xrow",
                                                Convert.ToInt32(result[0]["xrow"].ToString()));
                                            cmd.Parameters.AddWithValue("@xsrow",
                                                Convert.ToInt64(tempTable.Rows[i]["xsrow"].ToString()));
                                            cmd.Parameters.AddWithValue("@xline",
                                                zglobal.GetMaximumIdInt("xline", "amexamd",
                                                    " zid=" + _zid + " and xrow=" +
                                                    Convert.ToInt32(result[0]["xrow"].ToString()), "line"));
                                            cmd.Parameters.AddWithValue("@xctno1", tempTable.Rows[i]["xctno"].ToString());
                                            //cmd.Parameters.AddWithValue("@xgetmark",
                                            //    Convert.ToDecimal(tempTable.Rows[i]["xgetmark"].ToString()));
                                            cmd.Parameters.AddWithValue("@xgetmark", xgetmark);
                                            cmd.Parameters.AddWithValue("@xmarks",
                                                Convert.ToDecimal(tempTable.Rows[i]["xmarks"].ToString()));
                                            cmd.Parameters.AddWithValue("@xmarks1",
                                                Convert.ToDecimal(result[0]["xmarks"].ToString()));
                                            cmd.Parameters.AddWithValue("@xinterchangedate", DateTime.Now);
                                            cmd.Parameters.AddWithValue("@xinterchangeexamdate", Convert.ToDateTime(tempTable.Rows[i]["xdate"].ToString()));
                                            cmd.ExecuteNonQuery();

                                            //fnCheckRef(tempTable, tempTable1, tempTable.Rows[i]["xcttype"].ToString(), tempTable.Rows[i]["xctno"].ToString(),
                                            //    tempTable.Rows[i]["xterm"].ToString(), tempTable.Rows[i]["xsubject"].ToString(), tempTable.Rows[i]["xpaper"].ToString(),
                                            //    tempTable.Rows[i]["xext"].ToString(), tempTable.Rows[i]["xtype"].ToString(),cmd,_zid);
                                        }
                                        else
                                        {
                                            //try
                                            //{
                                            //    query =
                                            //        "INSERT INTO amexamhnew (ztime,zid,xrow,xsession,xterm,xclass,xgroup,xtype,xstatus,zemail,xdate,xsection,xsubject,xpaper,xext,xcttype,xctno,xmarks,xcteacher,xsteacher,xtopic,xdetails,xrefcttype,xrefctno,xreason,xretestfor,xschdate) " +
                                            ...
                                            //        "SELECT ztime,zid,xrow,xsession,xterm,xclass,xgroup,xtype,xstatus,zemail,xdate,xsection,xsubject,xpaper,xext,xcttype,xctno,xmarks,xcteacher,xsteacher,xtopic,xdetails,xrefcttype,xrefctno,xreason,xretestfor,xschdate FROM amexamh " +
                                            //        "WHERE zid=@zid AND xrow=@xrow";

                                            //    cmd.CommandText = query;
                                            //    cmd.Parameters.Clear();
                                            //    cmd.Parameters.AddWithValue("@zid", _zid);
                                            //    cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(tempTable.Rows[i]["xrow"].ToString()));
                                            //    cmd.ExecuteNonQuery();

                                            //    query =
                                            //        "INSERT INTO amexamdnew (zid,xrow,xline,xstdid,xsrow,xgetmark,xremarks,xna) " +
                                            //        "SELECT zid,xrow,xline,xstdid,xsrow,xgetmark,xremarks,xna FROM amexamd " +
                                            //        "WHERE zid=@zid AND xrow=@xrow";
                                            ...
                                            //query = "DELETE FROM amexamd WHERE zid=@zid AND xrow=@xrow AND xsrow=@xsrow";
                                            //cmd.Parameters.Clear();

                                            //cmd.CommandText = query;
                                            //cmd.Parameters.AddWithValue("@zid", _zid);
                                            //cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(tempTable.Rows[i]["xrow"].ToString()));
                                            //cmd.Parameters.AddWithValue("@xsrow", Convert.ToInt64(tempTable.Rows[i]["xsrow"].ToString()));
                                            //cmd.ExecuteNonQuery();
                                            //---------
                                            ...
                                            if (hxrow == 0)
                                            {
                                                hxrow = zglobal.GetMaximumIdInt("xrow", "amexamh", " zid=" + _zid, 1, 1);
                                            }
                                            else
                                            {
                                                hxrow = hxrow + 1;
                                            ...
                                            //            dts1.Reset();
                                            //            query =
                                            //                "SELECT coalesce(max(convert(int,xctno)),1) FROM amexamh WHERE zid=@zid AND xsession=@xsession AND " +
                                            //                "xclass=@xclass AND xterm=@xterm AND xgroup=@xgroup AND xtype='Class Test'  AND xsection=@xsection " +
                                            //                "AND xsubject=@xsubject AND xpaper=@xpaper AND coalesce(xext,'')=@xext AND xcttype=@xcttype  ";

                                            //            SqlDataAdapter da11 = new SqlDataAdapter(query, con);

                                            //            da11.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xsession", tempTable.Rows[i]["xsession"].ToString());
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xclass", tempTable.Rows[i]["xclass"].ToString());
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xterm", tempTable.Rows[i]["xterm"].ToString());
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xgroup", tempTable.Rows[i]["xgroup"].ToString());
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xsection", xsectionnew.Text.ToString().Trim());
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xsubject", tempTable.Rows[i]["xsubject"].ToString());
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xpaper", tempTable.Rows[i]["xpaper"].ToString());
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xext", tempTable.Rows[i]["xext"].ToString());
                                            //            da11.SelectCommand.Parameters.AddWithValue("@xcttype", "Test (WS)");


                                            //            DataTable tempTable11 = new DataTable();
                                            //            da11.Fill(dts1, "tempTable");
                                            //            tempTable11 = dts1.Tables["tempTable"];

                                            //            hxctno = Convert.ToInt32(tempTable11.Rows[0][0].ToString());
                                            //        }
                                            //    }
                                            //}
                                            //else
                                            ...
                                            //message.InnerHtml = message.InnerHtml + hxrow + " " + hxctno + "<br>";

                                            hxctno =
                                                Convert.ToInt32(
                                                    tempTable1234.Select("xrow=" +
                                                                         Convert.ToInt32(
                                                                             tempTable.Rows[i]["xrow"].ToString()))[0][
                                                                                 "xctno"]);

                                            query =
                                                "INSERT INTO amexamh (ztime, zutime, zid, xrow, xsession, xterm, xclass, xgroup, xsection, xsubject, xpaper, xdate, " +
                                                "xcttype, xctno, xmarks, xtype, xstatus, zemail, xemail, xsubmitedby, xsubmitdt, xcteacher, xsteacher, xtopic, xdetails, " +
                                                " xretestfor, xrefcttype, xrefctno, xreason, xschdate, xundoby, xundodt, xapproved1by, xapproved1dt, xflag10, " +
                                                "xapproved2by, xapproved2dt, xflag, xapproved3by, xapproved3dt, xcttypebackup, xctnobackup, xpaperbackup, " +
                                                "xpaperstat, xext, xext1,xinterchange,xinterchangedate ) " +
                                                //"VALUES(@ztime,@zid,@xrow,@xsession,@xterm,@xclass,@xgroup,@xtype,@xstatus,@zemail,@xdate,@xsection,@xsubject,@xpaper,@xext,@xcttype,@xctno,@xmarks,@xcteacher,@xsteacher,@xtopic,@xdetails,@xrefcttype,@xrefctno,@xreason,@xretestfor,@xschdate) ";
                                                "SELECT getdate(), zutime, zid, @xrow1, xsession, xterm, xclass, @xgroup, @xsection, xsubject, xpaper, xdate, @xcttype, " +
                                                "@xctno, " +
                                                "xmarks, xtype, " +
                                                "case when (select top 1 xstatus from amexamhh where zid=b.zid and xsession=b.xsession and xclass=b.xclass and coalesce(xterm,'')= " +
"b.xterm and xgroup=b.xgroup and xtype=b.xtype) = 'Approved3'  then 'Approved3' when (select top 1 xstatus from amexamhh where zid=b.zid and xsession=b.xsession and xclass=b.xclass and coalesce(xterm,'')= " +
"b.xterm and xgroup=b.xgroup and xtype=b.xtype) = 'Approved2'  then 'Approved2'else 'New' end as xstatus, " +
                                                "zemail, xemail, xsubmitedby, xsubmitdt, xcteacher, xsteacher, xtopic, xdetails, " +
                                                "xretestfor, " +
                                                "xrefcttype, xrefctno, xreason, xschdate, xundoby, xundodt, xapproved1by, xapproved1dt, xflag10, xapproved2by, " +
                                                "xapproved2dt, xflag, xapproved3by, xapproved3dt, xcttypebackup, xctnobackup, xpaperbackup, xpaperstat, xext, xext1,1,getdate() " +
                                                "FROM amexamh as b " +
                                                "WHERE zid=@zid AND xrow=@xrow";
                                            ...
                                            //if (
                                            //    amexamhh1234.Select("xtype='" + tempTable.Rows[i]["xtype"].ToString() +
                                            //                        "'")[0]["xstatus"] == "Approved3")
                                            //{
                                            //    xstatus4 = "Approved3";
                                            //}
                                            ...
                                            cmd.Parameters.Clear();
                                            cmd.Parameters.AddWithValue("@zid", _zid);
                                            cmd.Parameters.AddWithValue("@xrow1", hxrow);
                                            cmd.Parameters.AddWithValue("@xsection", xsectionnew.Text.Trim().ToString());
                                            cmd.Parameters.AddWithValue("@xgroup", xgroupnew.Text.Trim().ToString());
                                            cmd.Parameters.AddWithValue("@xcttype", "Test (WS)");
                                            cmd.Parameters.AddWithValue("@xctno", hxctno);
                                            //cmd.Parameters.AddWithValue("@xstatus", xstatus4);

                                            cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(tempTable.Rows[i]["xrow"].ToString()));
                                            cmd.ExecuteNonQuery();

                                            query =
                                                "INSERT INTO amexamd (zid, xrow, xline, xstdid, xsrow, xgetmark, xremarks, zemail, xemail, xflag1, xflag2, xundoby, xundodt, xremarks1, xbflag, xna, " +
                                                "xadjmark, xctno1, xstatus, xinterchange, xinterchangedate, xinterchangeexamdate) " +
                                                "SELECT zid, @xrow1, @xline, xstdid, xsrow, @xgetmark, xremarks + ' **transferred**', zemail, xemail, xflag1, xflag2, xundoby, xundodt, xremarks1, xbflag, xna, " +
                                                "xadjmark, xctno1, xstatus, 1, getdate(), @xinterchangeexamdate " +
                                                "FROM amexamd " +
                                                "WHERE zid=@zid AND xrow=@xrow and xsrow=@xsrow";


                                            //int xline = zglobal.GetMaximumIdInt("xline", "amexamd", " zid=" + _zid + " and xrow=" + xrow, "line");
                                            cmd.CommandText = query;
                                            cmd.Parameters.Clear();
                                            cmd.Parameters.AddWithValue("@zid", _zid);
                                            cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(tempTable.Rows[i]["xrow"].ToString()));
                                            cmd.Parameters.AddWithValue("@xrow1", hxrow);
                                            cmd.Parameters.AddWithValue("@xline", 1);
                                            cmd.Parameters.AddWithValue("@xsrow", Convert.ToInt64(tempTable.Rows[i]["xsrow"].ToString()));
                                            cmd.Parameters.AddWithValue("@xgetmark", xgetmark);
                                            cmd.Parameters.AddWithValue("@xinterchangeexamdate", Convert.ToDateTime(tempTable.Rows[i]["xdate"].ToString()));
                                            cmd.ExecuteNonQuery();
            ...
            try
            {
                ...
                if (ViewState["xrow"] != null)
                {
                    if (txtconformmessageValue.Value == "Yes")
                    {
                        ...
                        //        string query =
                        //            "select * from amsecinterchange where zid=@zid and xsession=@xsession and xclass=@xclass and xsection=@xsection and xgroup=@xgroup ";
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                query =
                                    "UPDATE amsecinterchange SET xstatus=@xstatus,xapprovedby=@xapprovedby,xapproveddt=@xapproveddt " +
                                    "WHERE zid=@zid AND xrow=@xrow ";
                                cmd.Parameters.Clear();
                    ...
                    if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM amsecinterchange WHERE zid=@zid AND xrow=@xrow";
                                cmd.Parameters.Clear();
