...
{
    public partial class stdresapp : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            string xclass1 = xclass_res.Text.Trim().ToString();
            string xgroup1 = xgroup_res.Text.Trim().ToString();
            string str = "SELECT   xrow,ROW_NUMBER() OVER (ORDER BY xrow) AS xid, xname, xexamroll, coalesce(xstatus,'n') AS xstatus,xfname,xremarks,xnumexam,coalesce(xstatus1,'n') AS xstatus1 FROM amadmis WHERE zid=@zid AND xsession=@xsession AND xnumexam=@xnumexam AND xclass=@xclass AND xgroup=@xgroup AND xnumexam<>'' AND coalesce(xstatus3,'')<>'Revised' AND coalesce(xstatus4,'')<>'old'";
        ...
        protected void OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    string xnumexam1 = (e.Row.DataItem as DataRowView).Row["xnumexam"].ToString();
                    ...
                    e.Row.Cells[i + 5].Controls.Add(chkCheckBox2);

                    CheckBox chkCheckBox1 = new CheckBox();
                    chkCheckBox1.ID = "waiting";
                    e.Row.Cells[i + 6].Controls.Add(chkCheckBox1);
                        ...
                        foreach (GridViewRow row in GridView1.Rows)
                        {
                            using (TransactionScope tran = new TransactionScope())
                            {
                                using (SqlConnection conn = new SqlConnection(zglobal.constring))
                                {
                                    ...
                                    string query =
                                        "UPDATE amadmis SET xremarks=@xremarks,xstatus=@xstatus,xapprovedby=@xapprovedby,xapproveddt=@xapproveddt " +
