...
{
    public partial class amnotice : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xtitle.Text == "" || xtitle.Text == string.Empty || xtitle.Text == "[Select]")
                {
                ...
                if (ViewState["xrow"] != null)
                {
                    xstatus2 = zglobal.fnGetValue("xstatus", "amnotice",
                          "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
                ...
                string fileName = xtitle.Text.Trim().ToString() + "_"  + Path.GetFileNameWithoutExtension(xlink.PostedFile.FileName) + "_" + DateTime.Now.ToString("ddMMyyyy_hhmmss") + extension;
                string xfile = xtitle.Text.Trim().ToString() + "_"  + Path.GetFileNameWithoutExtension(xlink.PostedFile.FileName) + extension;
                ...
                string serverName = "http://103.111.225.75:909/";
                string folder = "Uploads/";
                // Upload to Hosted Server.
                WebClient webClient = new WebClient();
                webClient.Credentials = new NetworkCredential("Administrator", "12!@abAB"); ;
                Uri uri = new Uri(serverName + folder + fileName);
                webClient.UploadFile(uri, "POST", fileName);


                //string xlink1 = _xlink;
                ////xremarks1 = xremarks.Text.Trim().ToString();
                ////xremarks1 = "";
                //string xteacher1 = Convert.ToString(HttpContext.Current.Session["curuser"]);
