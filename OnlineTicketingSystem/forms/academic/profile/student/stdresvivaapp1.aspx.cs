...
{
    public partial class stdresvivaapp1 : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            string xclass1 = xclass_res.Text.Trim().ToString();
            string xgroup1 = xgroup_res.Text.Trim().ToString();
            string str = "SELECT   xrow,ROW_NUMBER() OVER (ORDER BY xrow) AS xid, xname, xexamroll, coalesce(xstatus,'n') AS xstatus,xfname,xremarks FROM amadmis WHERE zid=@zid AND xsession=@xsession AND xnumexam=@xnumexam AND xclass=@xclass AND xgroup=@xgroup";
