...
{
    public partial class amdoublepro_approve : System.Web.UI.Page
    {
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        using (DataSet dts = new DataSet())
                        {
                            ...
                            da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                            da.SelectCommand.Parameters.AddWithValue("@xsession", hstdsession.Value.ToString().Trim());
                            da.SelectCommand.Parameters.AddWithValue("@xstdid", hstdid.Value.ToString().Trim());
                            ...
                            {
                                query =
                                    "DELETE FROM ampromotiond WHERE zid=@zid AND xrow=@xrow AND xsrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";

                                cmd.CommandText = query;
                                cmd.Parameters.Clear();
                                cmd.Parameters.AddWithValue("@zid", _zid);
                                cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(tempTable.Rows[0]["xrow"].ToString()));
                                cmd.Parameters.AddWithValue("@xstdid", hstdid.Value.ToString().Trim());
                                cmd.ExecuteNonQuery();
                                ...
                                using (DataSet dts1 = new DataSet())
                                {
                                   ...
                                   query =
                                        ...
                                        "SELECT * FROM ampromotionh WHERE zid=@zid AND " +
                                        "xsession=@xsession AND " +
                                        "xclasspro=(select xclasspro from amdoublepro where zid=@zid AND xrow=@xrow) " +
                                        "AND xgroup=(select xgrouppro from amdoublepro where zid=@zid AND xrow=@xrow) " +
                                        "AND xsection=(select xsectionpro from amdoublepro where zid=@zid AND xrow=@xrow)";


                                    SqlDataAdapter da1 = new SqlDataAdapter(query, conn);

                                    da1.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                    da1.SelectCommand.Parameters.AddWithValue("@xrow", Convert.ToInt32(hstdrow.Value));
                                    ...
                                    {
                                        int xline = zglobal.GetMaximumIdInt("xline", "ampromotiond", " zid=" + _zid + " and xrow=" + Convert.ToInt32(tempTable1.Rows[0]["xrow"].ToString()), "line");

                                        query = "INSERT INTO ampromotiond (zid,xrow,xline,xsrow,xpromoted,xstatus1,xsection,xgroup) " +
                                        "VALUES(@zid,@xrow,@xline,(select xrow from amadmis where zid=@zid AND xstdid=@xstdid),@xpromoted,@xstatus1,@xsection,@xgroup) ";

                                        cmd.CommandText = query;
                                        cmd.Parameters.Clear();
                                        cmd.Parameters.AddWithValue("@zid", _zid);
                                        cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(tempTable1.Rows[0]["xrow"].ToString()));
                                        cmd.Parameters.AddWithValue("@xline", xline);
                                        cmd.Parameters.AddWithValue("@xstdid", hstdid.Value.ToString().Trim());
                                        cmd.Parameters.AddWithValue("@xpromoted", 1);
                                        cmd.Parameters.AddWithValue("@xstatus1", "");
                                        cmd.Parameters.AddWithValue("@xsection", tempTable1.Rows[0]["xsection"].ToString());
                                        cmd.Parameters.AddWithValue("@xgroup", tempTable1.Rows[0]["xgroup"].ToString());
                                        cmd.ExecuteNonQuery();
                                                                            
                                    }
                                    else
                                    {
                                        message.InnerHtml = "Result not processed yet.";
                                        message.Style.Value = zglobal.warningmsg;
                                        return;
                                    }
                            ...
                            {
                                query =
                                   "UPDATE amadmis SET xclass=(select xclasspro from amdoublepro where zid=@zid AND xrow=@xrow)," +
                                   "xsection=(select xsectionpro from amdoublepro where zid=@zid AND xrow=@xrow)," +
                                   "xgroup=(select xgrouppro from amdoublepro where zid=@zid AND xrow=@xrow) " +
                                   "WHERE zid=@zid AND xrow=(select xrow from amadmis where zid=@zid AND xstdid=@xstdid)";
                            ...
                            query =
                               ...
                               "WHERE zid=@zid AND  xstdid=@xstdid AND xsession=(select xsession from amdoublepro where zid=@zid AND xrow=@xrow)";

                            da = new SqlDataAdapter(query, conn);

                            da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                            da.SelectCommand.Parameters.AddWithValue("@xrow", Convert.ToInt32(hstdrow.Value));
                            da.SelectCommand.Parameters.AddWithValue("@xstdid", hstdid.Value.ToString().Trim());
                            ...
                            {
                                query =
                                    "UPDATE amtfcseatconf SET xclass=(select xclasspro from amdoublepro where zid=@zid AND xrow=@xrow1), " +
                                    "xsection=(select xsectionpro from amdoublepro where zid=@zid AND xrow=@xrow1)," +
                                   "xgroup=(select xgrouppro from amdoublepro where zid=@zid AND xrow=@xrow1) " +
                                    "WHERE zid=@zid AND xrow=@xrow AND xstdid=@xstdid";

                                cmd.CommandText = query;
                                cmd.Parameters.Clear();
                                cmd.Parameters.AddWithValue("@zid", _zid);
                                cmd.Parameters.AddWithValue("@xrow", Convert.ToInt32(tempTable.Rows[0]["xrow"].ToString()));
                                cmd.Parameters.AddWithValue("@xrow1", Convert.ToInt32(hstdrow.Value));
                                cmd.Parameters.AddWithValue("@xstdid", hstdid.Value.ToString().Trim());
                                cmd.ExecuteNonQuery();
                            }
