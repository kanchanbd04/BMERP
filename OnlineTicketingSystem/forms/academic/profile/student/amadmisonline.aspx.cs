...
{
    public partial class amadmisonline : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    zglobal.fnGetComboDataCD("Session", xsession_check);
                    ...
                    xsession_check.Text = zglobal.fnDefaults("xsessiononline", "Student");
                    ...
                    grid_xstatus.Items.Add("All");
                    grid_xstatus.Items.Add("New");
                    grid_xstatus.Items.Add("Confirmed");
                    grid_xstatus.Items.Add("Canceled");

                    grid_xstatus.Text = "New";

                    fnFillDataGrid6(sender, e);
        ...
        public void fnRegisterPostBack()
        {
            if (TabContainer1.ActiveTabIndex == 1)
            {
                if (_grid_admis_test.Rows.Count > 0)
                {
                    ...
                    {
                        LinkButton lnkFull1 = row.FindControl("xrow1") as LinkButton;
        ...
        public void fnFillDataGrid(object sender, EventArgs e)
        {
            try
            {
                ...
                if (TabContainer1.ActiveTabIndex == 1)
                {
                    if (grid_xstatus.Text == "All")
                    {
                        str = "SELECT TOP " + _gridrow.Text.Trim().ToString() +
                              " right(xrow,4) as xrow1, xrow,xname,(xemail1 + '<br/>' + xcellno +', '+ xcellno1) as xcontact,xgender,xmname,xfname,xdob,xstatus,ztime FROM amadmison where zid=@zid and xsession=@xsession and xclass=@xclass ORDER BY xrow";
                    }
                    else
                    {
                        str = "SELECT TOP " + _gridrow.Text.Trim().ToString() +
                              " right(xrow,4) as xrow1, xrow,xname,(xemail1 + '<br/>' + xcellno +', '+ xcellno1) as xcontact,xgender,xmname,xfname,xdob,xstatus,ztime FROM amadmison where zid=@zid and xsession=@xsession and xclass=@xclass and xstatus=@xstatus ORDER BY xrow";
                    }
                ...
                da.SelectCommand.Parameters.AddWithValue("@xstatus", grid_xstatus.Text.Trim().ToString());
        ...
        protected void FillControls(object sender, EventArgs e)
        {
            try
            {
                ...
                xrow = Convert.ToInt32(_grid_admis_test.Rows[row.RowIndex].Cells[10].Text);

                //message.InnerHtml = xrow_.ToString();
                ...
                string str = "SELECT  right(xrow,4) as xref, *, coalesce(xreasoncancel,'') as xreasoncancel1," +
                             "coalesce((select xshifted from amadmisonvw1 where zid=amadmison.zid and xreference=amadmison.xrow),'') as xshifted " +
                             "FROM amadmison where zid=@zid  and xrow=@xrow ";
                ...
                if (TabContainer1.ActiveTabIndex == 1)
                {
                    if (dtztcode.Rows[0]["xstatus"].ToString() != "New")
                    {
                    ...
                    xref.Text = dtztcode.Rows[0]["xref"].ToString();
                    ...
                    xauthen.Text = dtztcode.Rows[0]["xauthen"].ToString();

                    if (dtztcode.Rows[0]["xreasoncancel1"].ToString() != "")
                    {
                        xstatus.Text = dtztcode.Rows[0]["xstatus"].ToString() + " (" + dtztcode.Rows[0]["xreasoncancel1"].ToString() + ")";
                    }
                    else if (dtztcode.Rows[0]["xshifted"].ToString() != "")
                    {
                        xstatus.Text = dtztcode.Rows[0]["xstatus"].ToString() + " ( Shifted to " + dtztcode.Rows[0]["xshifted"].ToString() + ")";
                    }
                    else
                    {
                    ...
                    xbcimagelink.ImageUrl = dtztcode.Rows[0]["xbcimagelink"].ToString();

                    if (dtztcode.Rows[0]["ztime"].Equals(DBNull.Value) == false)
                    {
                        ztime.Text = Convert.ToDateTime(dtztcode.Rows[0]["ztime"]).ToString("dd/MM/yyyy");
                    }

                    ////////////////////////////////
                    dts.Reset();

                    str = "SELECT *,coalesce((select xname from amadmis where zid=@zid and xstdid=amadmisonsib.xstdid),'') as xname1 FROM amadmisonsib WHERE zid=@zid AND xrow=@xrow";
                    ...
                    if (dttemp.Rows.Count > 0)
                    {
                        ...
                        for (int i = 1; i <= howMany; i++)
                        {
                            ...
                            ((TextBox)placeholder9.FindControl("xsibname" + i)).Text =
                               dttemp.Rows[i - 1]["xname1"].ToString();
                            ((TextBox)placeholder9.FindControl("xsibclass" + i)).Text =
                ...
                if (TabContainer1.ActiveTabIndex != 2)
                {
                    ...
                    _row.Value = xrow.ToString();
                    _row_global.Value = xrow.ToString();
                }
                message.InnerHtml = "";
        ...
        protected void FillControls2(object sender, EventArgs e)
        {
            try
            {
                ...
                if (String.IsNullOrEmpty(_student.Value) || _student.Value == "" || _student.Value == string.Empty)
                {
                    return;
                }
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
                        ...
                        string query =
                            ...
                            "SELECT xrow FROM amadmison WHERE zid=@zid AND xsession=@xsession AND right(xrow,4)=@xref ";
                        ...
                        da1.SelectCommand.Parameters.AddWithValue("@xref", xref.Text.ToString().Trim());
                        ...
                        {
                            _student.Value = tempTable1.Rows[0][0].ToString();

                        }
                        else
                        {
                            message.InnerHtml = "Invalid Reference.";
        ...
        private void fnCreateControl9(int k)
        {
            try
            {
                ...
                xsibclass.CssClass = "bm_academic_textbox_dynamic_100_80 bm_academic_ctl_global bm_academic_textbox";
                ...
                xsibid.Enabled = false;
                xsibname.Enabled = false;
                xsibclass.Enabled = false;

                xsibid.BackColor = Color.White;
                xsibname.BackColor = Color.White;
                xsibclass.BackColor = Color.White;
                ...
                //{
                    ...
                    using (TransactionScope tran = new TransactionScope())
                    {
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            xreference = Convert.ToInt32(_row.Value.ToString());

                            query = "UPDATE amadmison SET xstatus='Canceled',xreasoncancel=@xreasoncancel,xremarks=@xremarks WHERE zid=@zid and xrow=@xrow";
                            cmd.Parameters.Clear();
                            cmd.CommandText = query;
                            cmd.Parameters.AddWithValue("@zid", _zid);
                            cmd.Parameters.AddWithValue("@xrow", xreference);
                            cmd.Parameters.AddWithValue("@xreasoncancel", xreasoncancel.Text.ToString().Trim());
                    ...
                    _student.Value = _row.Value;

                    FillControls2(null, null);
                    fnFillDataGrid(null, null);

                    message.InnerHtml = "Cancel Sucessful";
                    message.Style.Value = zglobal.successmsg;
                //}
            }
            catch (Exception exp)
        ...
        protected void btnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                if (_xconfirm.Value.ToString() == "Yes")
                {
                    int _zid = Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"]));
                    ...
                    using (TransactionScope tran = new TransactionScope())
                    {
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            if (TabContainer1.ActiveTabIndex == 1)
                            {
                                query = "INSERT INTO amadmis (ztime,zid,xrow,xsession,xname,xclass,xpschool,xmname,xprofession,xfname,xprofession1,xpadd,xperadd,xemail1,xnation,xdob,xreligion,xgender,ximagelink,zemail,xcellno,xcellno1,zactive,xreference,xgroup,xexamroll,xsection) " +
                                        "SELECT @ztime,zid,@xrow,xsession,xname,xclass,xpschool,xmname,xprofession,xfname,xprofession1,xpadd,xperadd,xemail1,xnation,xdob,xreligion,xgender,ximagelink,@zemail,xcellno,xcellno1,1,@xreference,'',@xexamroll,'' FROM amadmison WHERE zid=@zid AND xrow= @xreference ";
                            ...
                            cmd.Parameters.AddWithValue("@xrow", xrow);
                            cmd.Parameters.AddWithValue("@zemail", zemail);
                            cmd.Parameters.AddWithValue("@xreference", xreference);
                            cmd.Parameters.AddWithValue("@xexamroll", xexamroll1);
                            //cmd.Parameters.AddWithValue("@xsection", xsection1);
                            cmd.ExecuteNonQuery();

                            query = "UPDATE amadmison SET xstatus='Confirmed' WHERE zid=@zid and xrow=@xrow";
                            cmd.Parameters.Clear();
                            cmd.CommandText = query;
                            cmd.Parameters.AddWithValue("@zid", _zid);
                            cmd.Parameters.AddWithValue("@xrow", xreference);
                            ...
                            if (TabContainer1.ActiveTabIndex == 1)
                            {
                                if (ViewState["ctladded9"] != null)
                                {
                                    ...
                                    string str = "SELECT *,coalesce((select xname from amadmis where zid=@zid and xstdid=amadmisonsib.xstdid),'') as xname1 FROM amadmisonsib WHERE zid=@zid AND xrow=@xrow";

                                    SqlDataAdapter da2 = new SqlDataAdapter(str, conn);

                                    da2.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                                    da2.SelectCommand.Parameters.AddWithValue("@xrow", xreference);
                                    dts.Reset();
                                    da2.Fill(dts, "temp");
                                    DataTable dttemp = new DataTable();
                                    dttemp = dts.Tables[0];
                                    ...
                                    if (dttemp.Rows.Count > 0)
                                    {
                                        ...
                                        for (int i = 1; i <= dttemp.Rows.Count; i++)
                                        {
                                            ...
                                            cmd.Parameters.AddWithValue("@xrow", xrow);
                                            cmd.Parameters.AddWithValue("@xline", xline);
                                            cmd.Parameters.AddWithValue("@xstdid",  dttemp.Rows[i - 1]["xstdid"].ToString());
                                            cmd.Parameters.AddWithValue("@xname", dttemp.Rows[i - 1]["xname1"].ToString());
                                            cmd.Parameters.AddWithValue("@xclass", dttemp.Rows[i - 1]["xclass"].ToString());
                                            cmd.ExecuteNonQuery();
                                    ...
                                    //    string xsibid = ((TextBox)placeholder9.FindControl("xsibid" + i)).Text.ToString().Trim();
                                    //    string xsibname = ((TextBox)placeholder9.FindControl("xsibname" + i)).Text.ToString().Trim();
                                    //    string xsibclass = ((TextBox)placeholder9.FindControl("xsibclass" + i)).Text.ToString().Trim();
                    ...
                    message.InnerHtml = "Confirm Successful.";
                    message.Style.Value = zglobal.successmsg;
                }
            }
            catch (Exception exp)
        ...
        public void fnFillDataGrid6(object sender, EventArgs e)
        {
            try
            {
                SqlConnection conn1;
                ...
                //string str = "SELECT ROW_NUMBER() OVER (ORDER BY xclass) AS xno,xcodealt,xclass,SUM(CASE WHEN xstatus='New' THEN 1 ELSE 0 END) AS xnew, " +
                //             "SUM(CASE WHEN xstatus='Confirmed' THEN 1 ELSE 0 END) AS xconfirmed, SUM(CASE WHEN xstatus='Canceled' THEN 1 ELSE 0 END) AS xcanceled," +
                //             "COUNT(xrow) AS xtotal " +
                //             "FROM amadmison INNER JOIN mscodesam on amadmison.zid=mscodesam.zid and mscodesam.xtype='Class' and xclass=xcode WHERE amadmison.zid=@zid AND xsession=@xsession " +
                //             "GROUP BY xcodealt,xclass " +
                //             "ORDER BY xcodealt ";

                string str = "SELECT ROW_NUMBER() OVER (ORDER BY xclass) AS xno,xcodealt,xclass,SUM(CASE WHEN xstatus='New' THEN 1 ELSE 0 END) AS xnew, " +
                             "SUM(CASE WHEN xstatus='Confirmed' THEN 1 ELSE 0 END) AS xconfirmed, SUM(CASE WHEN xstatus='Canceled' THEN 1 ELSE 0 END) AS xcanceled," +
                             "COUNT(xrow) AS xtotal, sum(case when xshifted <> '' then 1 else 0 end ) as xshifted " +
                             "From " +
                             "(select xclass,xstatus,xrow,xcodealt, " +
                             "coalesce((select xshifted from amadmisonvw1 where zid=amadmison.zid and xreference=amadmison.xrow),'') as xshifted " +
                             "FROM amadmison INNER JOIN mscodesam on amadmison.zid=mscodesam.zid and mscodesam.xtype='Class' and xclass=xcode WHERE amadmison.zid=@zid AND xsession=@xsession ) tbl " +
                             "GROUP BY xcodealt,xclass " +
                             "ORDER BY xcodealt ";
                ...
                da.SelectCommand.Parameters.Add("@xsession", xsession_check.Text.Trim().ToString());
                da.Fill(dts, "tempztcode");
                //DataView dv = new DataView(dts.Tables[0]);
                DataTable dtztcode = new DataTable();
                dtztcode = dts.Tables[0];
                ...
                using (SqlConnection conn = new SqlConnection(zglobal.constring))
                {
                    ...
                    {
                        //int _zid = Convert.ToInt32(Convert.ToString(HttpContext.Current.Session["business"]));
                        string xsession1 = xsession_check.Text.Trim().ToString();

                        string query = "select * from amadmisonvw1 where zid=@zid and xsession=@xsession ";
                        ...
                        dtexam1 = dts1.Tables[0];
                        ViewState["amadmisonvw1"] = dtexam1;
                    }
                }
        ...
        private int xnew1 = 0;
        private int xconfirmed1 = 0;
        private int xtotal1 = 0;
        private int xcanceled1 = 0;
        private int xshifted1 = 0;
        ...
        protected void _grid_emp_OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    xconfirmed1 += Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "xconfirmed"));
                    xcanceled1 += Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "xcanceled"));
                    xtotal1 += Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "xtotal"));
                    xshifted1 += Convert.ToInt32(DataBinder.Eval(e.Row.DataItem, "xshifted"));
                    ...
                    result3 =
                            ((DataTable)ViewState["amadmisonvw1"]).Select("xclass='" + e.Row.Cells[1].Text + "'");

                    if(result3.Length>0)
                    {
                        string title="";
                        foreach (DataRow row60 in result3)
                        {
                            title += row60["xname"] + " shifted to " + row60["xshifted"] + "\n";
                ...
                if (e.Row.RowType == DataControlRowType.Footer)
                {
                    ...
                    e.Row.Cells[2].Text = xnew1.ToString();
                    e.Row.Cells[2].Font.Bold = true;
                    e.Row.Cells[2].HorizontalAlign = HorizontalAlign.Center;
                    e.Row.Cells[2].ForeColor = Color.White;

                    e.Row.Cells[3].Text = xconfirmed1.ToString();
                    e.Row.Cells[3].Font.Bold = true;
                    e.Row.Cells[3].HorizontalAlign = HorizontalAlign.Center;
                    e.Row.Cells[3].ForeColor = Color.White;

                    e.Row.Cells[4].Text = xcanceled1.ToString();
                    ...
                    e.Row.Cells[6].Text = xshifted1.ToString();
