...
{
    public partial class hrteacherquickcon : System.Web.UI.Page
    {
            ...
            if (TabContainer1.ActiveTabIndex == 0)
            {
                if (_grid_teacherinfo_breakup.Rows.Count > 0)
                {
                    foreach (GridViewRow row in _grid_teacherinfo_breakup.Rows)
                    {
                        ...
                        //HyperLink lnkFull1 = row.FindControl("xemp") as HyperLink;
        ...
        public void fnFillDataGrid(object sender, EventArgs e)
        {
            try
            {
                ...
                if (TabContainer1.ActiveTabIndex == 0)
                {
                    ...
                    str = "SELECT  xemp,xname +'<br/>' + xcontact + '<br/>' + xemail1 as xname,xemail1,xfather + '<br/>' + xmother as xparents,xspouse,xeconname + '<br/>' + xecontact + '<br/>' + xeconrelation as xecontact,xblood,xpadd  FROM hrmst where zid=@zid and ( xname like '%" + _searchbox.Text.Trim().ToString() + "%' or xemp like '%" + _searchbox.Text.Trim().ToString() + "%' or xcontact like '%" + _searchbox.Text.Trim().ToString() + "%' ) and xtype='teacher' and xstatusemp='A-Active' ORDER BY xemp ";
                ...
                if (TabContainer1.ActiveTabIndex == 0)
                {
                    ...
                    //{
                    //    Label lbl = (Label)row.FindControl("xserial");
                    //    lbl.Text = i.ToString();
        ...
        protected void GridView1_Sorting(object sender, GridViewSortEventArgs e)
        {
            ...
            if (dtrslt.Rows.Count > 0)
            {
                ...
                _grid_teacherinfo_breakup.DataSource = dtrslt;
