...
{
    public partial class hrmststaff : System.Web.UI.Page
    {
        ...
        public void fnFillDataGrid(object sender, EventArgs e)
        {
            try
            {
                ...
                if (TabContainer1.ActiveTabIndex == 0)
                {
                    str = "SELECT TOP " + _gridrow.Text.Trim().ToString() + " xemp,xname,xcontact,xemail1,xblood,xfather,xstatusemp FROM hrmst where zid=@zid and ( xname like '%" + _searchbox.Text.Trim().ToString() + "%' or xemp like '%" + _searchbox.Text.Trim().ToString() + "%' or xcontact like '%" + _searchbox.Text.Trim().ToString() + "%' ) and xtype='staff' ORDER BY xemp ";
            ...
            try
            {
                ...
                if (TabContainer1.ActiveTabIndex == 0)
                {
                    ...
                    if (dtztcode.Rows.Count <= 0)
                    {
                        if (sender is TextBox)
                        {
                            if (ViewState["xemp"] != null || sender is ImageButton)
                            {
                                btnSave.Visible = true;
                                btnEdit.Visible = false;
                            }
            ...
            try
            {
               ...
               // _gridrow_h.Text = zglobal._grid_row_value;
                ...
                if (TabContainer1.ActiveTabIndex == 0)
                {
                    ...
                    list.Visible = true;
                    position.Visible = false;
                    _grid_header.InnerHtml = "Employee's Personal Information (Quick Contact) :";
                    fnGridVisibleProp("_grid_teacherinfo_breakup");
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (TabContainer1.ActiveTabIndex == 0)
                        {
                            query = "INSERT INTO hrmst (ztime,zid,xemp,xname,xdob,xnationality,xnid,xblood,xsex,xfather,xmother,xnosibling,xreligion,xpadd,xperadd,xcontact,xecontact,xemail1,xmstatus,xmday,xlink,xspouse,xsocupation,xnochild,zemail,xfbid,xhobbies,xexactivity,xeconname,xeconrelation,xstatusemp,xtype) " +
                                    "VALUES(@ztime,@zid,@xemp,@xname,@xdob,@xnationality,@xnid,@xblood,@xsex,@xfather,@xmother,@xnosibling,@xreligion,@xpadd,@xperadd,@xcontact,@xecontact,@xemail1,@xmstatus,@xmday,@xlink,@xspouse,@xsocupation,@xnochild,@zemail,@xfbid,@xhobbies,@xexactivity,@xeconname,@xeconrelation,'A-Active','staff') ";
