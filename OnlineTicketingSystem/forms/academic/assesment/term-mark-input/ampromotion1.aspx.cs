...
{
    public partial class ampromotion1 : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            str = @"select d.xsrow as xrow,ROW_NUMBER() OVER (ORDER BY (select xstdid from amadmis where xrow=d.xsrow and zid=h.zid)) AS xid, 
                   ...
                   case when (coalesce(d.xstatus1,'')='') and coalesce(d.xpromoted,0)=1 then 'Approved' 
                   when coalesce(d.xstatus1,'')='meetvp' and coalesce(d.xpromoted,0)=1 and coalesce(d.xstatus2,'')='Approved' then 'Approved' 
                   else '' end as xstatus
                   from ampromotionh as h inner join ampromotiond as d on h.zid=d.zid and h.xrow=d.xrow 
                   WHERE h.zid=@zid AND h.xsession=@xsession  AND h.xclass=@xclass AND h.xgroup=@xgroup AND h.xsection=@xsection and h.xstatus='Approved'
                   ORDER BY xname";
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    if (ViewState["amexamph_result_avg1"] != null)
                    {
                        ...
                        if (ViewState["ampromotiond"] != null)
                        {
                            ...
                            //if ((e.Row.DataItem as DataRowView).Row["xstatus"].ToString() == "Approved")
                            //{
                            //    chkCheckBox.InputAttributes.Add("disabled", "disabled");
                            //}
                            //else
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xsessionpro.Text == "" || xsessionpro.Text == string.Empty || xsessionpro.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Session (Promoted)</li>";
                    isValid = false;
                }
                if (xclasspro.Text == "" || xclasspro.Text == string.Empty || xclasspro.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Class (Promoted)</li>";
                    isValid = false;
                }
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    if (GridView1.Rows.Count > 0)
                    {
                        if (ViewState["xrow"] == null)
                        {
                            ...
                            //        string query =
                            //            //"SELECT * FROM hrgrowthh WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass  AND xgroup=@xgroup   AND xsection=@xsection AND xsubject=@xsubject AND xobserno=@xobserno AND xteacher=@xteacher";
                            //            "SELECT * FROM amstudentvw WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass AND xgroup=@xgroup ";


                            //        SqlDataAdapter da = new SqlDataAdapter(query, con);

                            //        da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                            //        da.SelectCommand.Parameters.AddWithValue("@xsession", xsessionpro.Text.ToString().Trim());
                            //        da.SelectCommand.Parameters.AddWithValue("@xclass", xclasspro.Text.ToString().Trim());
                            //        da.SelectCommand.Parameters.AddWithValue("@xgroup", xgrouppro.Text.ToString().Trim());
                            ...
                            //    string query =
                            //        "INSERT INTO ampromotionh (ztime,zid,xrow,xsession,xclass,xgroup,xsection,xsessionpro,xclasspro,xgrouppro,xstatus,zemail) " +
                            //        "VALUES(@ztime,@zid,@xrow,@xsession,@xclass,@xgroup,@xsection,@xsessionpro,@xclasspro,@xgrouppro,@xstatus,@zemail) ";
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            foreach (GridViewRow row in GridView1.Rows)
                            {
                                ...
                                CheckBox xpromotion = row.FindControl("xpromotion") as CheckBox;

                                int xpromotion1;
                                //string xstatus11 = "";
                                if (xpromotion.Checked == true)
                                {
                                    xpromotion1 = 1;
                                }
                                else
                                ...
                                //query = "INSERT INTO ampromotiond (zid,xrow,xline,xsrow,xpromoted,xstatus1) " +
                                //        "VALUES(@zid,@xrow,@xline,@xsrow,@xpromoted,@xstatus1) ";

                                //query = "UPDATE ampromotiond SET xpromoted=@xpromoted " +
                                //        "WHERE zid=@zid and xrow=@xrow and xsrow=@xsrow and coalesce(xstatus1,'')='meetvp'";
                                ...
                                cmd.Parameters.AddWithValue("@xpromoted", xpromotion1);
                                //cmd.Parameters.AddWithValue("@xstatus1", xstatus11);
                                cmd.ExecuteNonQuery();
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                foreach (GridViewRow row in GridView1.Rows)
                                {
                                    ...
                                    query = "UPDATE amtfcseatconf SET xstatusconf=1 " +
                                          "WHERE zid=@zid AND xsession=@xsession and xstdid=(select xstdid from amadmis where zid=@zid and xrow=@xsrow) ";
                                    cmd.Parameters.Clear();

                                    cmd.CommandText = query;
                                    cmd.Parameters.AddWithValue("@zid", _zid);
                                    cmd.Parameters.AddWithValue("@xsession", xsessionpro.Text.Trim().ToString());
                                    cmd.Parameters.AddWithValue("@xsrow", Int64.Parse(row.Cells[row.Cells.Count - 1].Text.ToString()));
                                    cmd.ExecuteNonQuery();
