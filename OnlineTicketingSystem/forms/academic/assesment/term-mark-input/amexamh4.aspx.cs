...
{
    public partial class amexamh4 : System.Web.UI.Page
    {
        private void fnButtonState()
        {
            ...
            else if (xstatus1 == "Approved1" || xstatus1 == "Approved2" || xstatus1 == "Approved3")
            {
                btnUndo.Enabled = false;
                btnUndo1.Enabled = false;
                btnDelete.Enabled = false;
        ...
        protected void OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (ViewState["xrow"] != null)
                    {
                        //if (Convert.ToString(ViewState["xstatus"]) != "New" && Convert.ToString(ViewState["xstatus"]) != "Undo" && Convert.ToString(ViewState["xstatus"]) != "Undo1" && Convert.ToString(ViewState["xstatus"]) != "Submited" )
                        if (Convert.ToString(ViewState["xstatus"]) != "Submited" )
                        {
                            txtComments.Enabled = false;
                            txtMarks.Enabled = false;
                            chkCheckBox.Enabled = false;
        ...
        private void fnCheckRow()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts = new DataSet())
                {
                    ...
                    // {
                    query =
                    "SELECT * FROM amexamh WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass AND xterm=@xterm AND xgroup=@xgroup AND xtype='Term Exam'  AND xsection=@xsection AND xsubject=@xsubject AND xpaper=@xpaper AND coalesce(xext,'')=@xext ";
                    // }
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts = new DataSet())
                    {
                        ...
                        string query =
                            ...
                            "SELECT count(*) FROM amexamhh WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass AND xterm=@xterm AND xgroup=@xgroup AND xtype='Term Exam' AND xstatus='Approved3' ";
                        ...
                        {
                            message.InnerHtml = "Result already approved. Cann't insert new record for this term.";
                ...
                if (ViewState["xrow"] != null)
                {
                    xstatus1 = zglobal.fnGetValue("xstatus", "amexamh",
                          "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
                    if (xstatus1 != "New" && xstatus1 != "Undo" && xstatus1 != "Undo1" && xstatus1 != "Submited")
                    {
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    if (GridView1.Rows.Count > 0)
                    {
                        ...
                        //        string xrefctno1 = "";
                        //        string xreason1 = "";
                        //        DateTime xschdate1 = DateTime.ParseExact("31/12/2999", "dd/MM/yyyy", CultureInfo.InvariantCulture);
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            //    cmd.Parameters.AddWithValue("@xtopic", "");
                            //    cmd.Parameters.AddWithValue("@xdetails", xdetails.Value.Trim().ToString());
                            //    cmd.Parameters.AddWithValue("@xdate", xdate1);
                            //    cmd.Parameters.AddWithValue("@xrefcttype", xrefcttype1);
                            //    cmd.Parameters.AddWithValue("@xrefctno", xrefctno1);
                            //    cmd.Parameters.AddWithValue("@xreason", xreason1);
                            //    cmd.Parameters.AddWithValue("@xretestfor", "");
                            //    cmd.Parameters.AddWithValue("@xschdate", DateTime.ParseExact("31/12/2999", "dd/MM/yyyy", CultureInfo.InvariantCulture));
                            //    cmd.ExecuteNonQuery();
                            ...
                            foreach (GridViewRow row in GridView1.Rows)
                            {
                                ...
                                //    {
                                //        query = "INSERT INTO amexamd (zid,xrow,xline,xstdid,xsrow,xgetmark,xremarks,xna,xbflag) " +
                                //                "VALUES(@zid,@xrow,@xline,@xstdid,@xsrow,@xgetmark,@xremarks,@xna,@xbflag) ";
                                ...
                                //        cmd.Parameters.AddWithValue("@xbflag", "1");
                                //        cmd.ExecuteNonQuery();
                                //    }
                                //    else
                                //    {
                                //        query =
                                //            "UPDATE amexamd SET zutime=@zutime,xemail=@xemail,xgetmark=@xgetmark,xremarks=@xremarks,xna=@xna,xbflag=@xbflag " +
                                ...
                                //query = "INSERT INTO amexamd (zid,xrow,xline,xstdid,xsrow,xgetmark,xremarks,xna,xbflag,xabsent) " +
                                //    "VALUES(@zid,@xrow,@xline,@xstdid,@xsrow,@xgetmark,@xremarks,@xna,@xbflag,@xabsent) ";

                                query = "UPDATE amexamd SET xremarks=@xremarks, xna=@xna, xabsent=@xabsent,xemail=@xemail,zutime=@zutime,xbflag=@xbflag " +
            ...
            try
            {
                ...
                if (ViewState["xrow"] != null)
                {
                    if (txtconformmessageValue3.Value == "Yes")
                    {
                        ...
                        btnUndo.Enabled = false;
                        btnUndo1.Enabled = false;
                        ViewState["xstatus"] = "Approved1";
                        hxstatus.Value = "Approved1";
                        hiddenxstatus.Value = "Approved1";
                        fnButtonState();
                        BindGrid();
                        fnFillGrid2();
                    }
        ...
        protected void fnEventValue(string evnt, object sender, EventArgs e)
        {
            ...
            //                "SELECT * FROM amexamh WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass AND xterm=@xterm AND xgroup=@xgroup AND xtype='Class Test'  AND xsection=@xsection AND xsubject=@xsubject AND xpaper=@xpaper AND xcttype=@xcttype AND xstatus NOT IN ('New','Undo','Undo1')";
            //                // }
