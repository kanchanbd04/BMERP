...
{
    public partial class amexamh : System.Web.UI.Page
    {
            ...
            try
            {
                if (!IsPostBack)
                {
                    ...
                    if (xsession1 != "" && xterm1 != "" && xclass1 != "" && xsection1 != "" && xsubject1 != "" && xcttype1 != "" && xctno1 != "")
                    {
                        ...
                        //message.InnerHtml = "Session : " + xsession1 + ", Term : " + xterm1 + ", Class : " + xclass1 +
                        //                    ", Group : " + xgroup1 +
                        //                    ", Section : " + xsection1 + ", Subject : " +
                        //                    xsubject1 + ", Ext. : " + xext1 + ", Paper : " + xpaper1 + ", Test Type : " +
                        //                    xcttype1 + ", Test No : " + xctno1;
                    }
                    else
                    {
                        _xext.Visible = false;
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts = new DataSet())
                {
                    ...
                    if (tempTable.Rows.Count > 0)
                    {
                        ...
                        string xflag1 = DBNull.Value.Equals(tempTable.Rows[0]["xflag123"]) == true ? "" : tempTable.Rows[0]["xflag123"].ToString();
                        ViewState["xflag"] = xflag1;
                    }
                    else
                    {
                        ViewState["xrow"] = null;
                    ...
                    if (GridView1.Rows.Count > 0)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            if (xrow == 0)
                            {
                                ...
                                //    xreason1 = xreason.Text.Trim().ToString();
                                //}
                                query = "UPDATE amexamh SET zutime=@zutime,xemail=@xemail,xmarks=@xmarks,xcteacher=@xcteacher,xsteacher=@xsteacher,xtopic=@xtopic,xdetails=@xdetails,xretestfor=@xretestfor,xdate=@xdate,xrefcttype=@xrefcttype, xrefctno=@xrefctno, xreason=@xreason, xflag123=@xflag " +
                                ...
                                cmd.Parameters.AddWithValue("@xrefctno", xrefctno1);
                                cmd.Parameters.AddWithValue("@xreason", xreason1);
                                cmd.Parameters.AddWithValue("@xflag", xflag1);
        ...
        protected void fnAutoTestNo()
        {
            ...
            using (SqlConnection conn = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts = new DataSet())
                {
                    ...
                    string xpaper1 = xpaper.Text.Trim().ToString();
                    string xext1 = xext.Text.Trim().ToString();
                    string xcttype1 = xcttype.Text.Trim().ToString();

                    string query =
                        "SELECT coalesce(max(cast(xctno as int)) + 1,1) as xctno " +
                        "FROM amexamh " +
                        "WHERE zid=@zid AND xsession=@xsession AND xterm=@xterm AND xclass=@xclass AND xgroup=@xgroup AND xsection=@xsection AND xsubject=@xsubject AND xpaper=@xpaper AND coalesce(xext,'')=@xext AND xtype='Class Test'  AND xcttype=@xcttype";
                    ...
                    da.SelectCommand.Parameters.AddWithValue("@xpaper", xpaper1);
                    da.SelectCommand.Parameters.AddWithValue("@xext", xext1);
                    da.SelectCommand.Parameters.AddWithValue("@xcttype", xcttype1);
                    da.Fill(dts, "tempztcode");
                    DataTable dtexam = new DataTable();
                    dtexam = dts.Tables[0];
                    ...
                    if (dtexam.Rows.Count > 0)
                    {
                        try
                        {
                            xctno.Text = dtexam.Rows[0][0].ToString();
                        }
                        catch (Exception exp)
                        {
