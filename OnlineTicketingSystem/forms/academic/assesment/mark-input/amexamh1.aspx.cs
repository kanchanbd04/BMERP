...
{
    public partial class amexamh1 : System.Web.UI.Page
    {
        private void fnButtonState()
        {
            ...
            else if (xstatus1 == "Approved2" || xstatus1 == "Approved3" || xstatus1 == "Approved1")
            {
                btnUndo.Enabled = false;
                btnUndo1.Enabled = false;
                btnDelete.Enabled = false;
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (ViewState["xrow"] != null)
                    {
                        ...
                        //if (Convert.ToString(ViewState["xstatus"]) == "Approved1")
                        {
                            txtComments.Enabled = false;
                            txtMarks.Enabled = false;
                            chkCheckBox.Enabled = false;
        ...
        private void fnCheckRow()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts = new DataSet())
                {
                    ...
                    // {
                    query =
                    "SELECT * FROM amexamh WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass AND xterm=@xterm AND xgroup=@xgroup AND xtype='Class Test'  AND xsection=@xsection AND xsubject=@xsubject AND xpaper=@xpaper AND coalesce(xext,'')=@xext AND xcttype=@xcttype AND xctno=@xctno and xstatus in ('Submited','Approved1','Approved2','Approved3')";
                    // }
            ...
            try
            {
                ...
                if (ViewState["xrow"] != null)
                {
                    xstatus1 = zglobal.fnGetValue("xstatus", "amexamh",
                          "zid=" + _zid + " AND xrow=" + Convert.ToInt32(ViewState["xrow"]));
                    if (xstatus1 != "Submited" && xstatus1 != "Undo" && xstatus1 != "Undo1")
                    {
