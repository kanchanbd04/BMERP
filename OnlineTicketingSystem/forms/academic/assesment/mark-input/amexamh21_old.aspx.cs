...
{
    public partial class amexamh21_old : System.Web.UI.Page
    {
            ...
            try
            {
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (ViewState["xrow"] != null)
                    {
                        ...
                        if (Convert.ToString(ViewState["xstatus"]) == "Approved3")
                        {
                            chkCheckBox.Enabled = false;
                            //image.Enabled = false;
                            //Page.ClientScript.RegisterHiddenField("hxstatus", ViewState["xstatus"].ToString());
                ...
                if (xcttype.Text != "")
                {
                    ...
                    using (SqlConnection con = new SqlConnection(zglobal.constring))
                    {
                        using (DataSet dts = new DataSet())
                        {
                            ...
                            "SELECT * FROM amexamh WHERE zid=@zid AND xsession=@xsession AND xclass=@xclass AND xterm=@xterm AND xgroup=@xgroup AND xtype='Class Test'  AND xsection=@xsection AND xsubject=@xsubject AND xpaper=@xpaper AND coalesce(xext,'')=@xext AND xcttype=@xcttype AND xstatus IN ('Approved1','Approved2','Approved3')";
                            // }
