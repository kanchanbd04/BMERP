...
{
    public partial class pratdt : System.Web.UI.Page
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xshift.Text == "" || xshift.Text == string.Empty || xshift.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Shift Code</li>";
                    isValid = false;
                }
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xsup"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                               "INSERT INTO pratdt (ztime,zid,xdate,xshift,xlotime,xintime,xpnlttime,zemail,xouttime,xmrgntime,xremark,xstatus,xdayab,xatdstat,xinc,xnum,xscore,xqtycount," +
                               "xstr01,xrowauth,xrowappt,xrowprob,xint01,xint02) " +
                               "VALUES(@ztime,@zid,@xdate,@xshift,@xlotime,@xintime,@xpnlttime,@zemail,@xouttime,@xmrgntime,@xremark,@xstatus,@xdayab,@xatdstat,@xinc,@xnum,@xscore,@xqtycount," +
                               "@xstr01,@xrowauth,@xrowappt,@xrowprob,@xint01,@xint02) ";
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xsupp == "")
                        {
                            ...
                            query = "UPDATE pratdt SET zutime=@zutime,xemail=@xemail, xlotime=@xlotime, " +
        ...
        private void fnFillControl(string xsup1,DateTime xdate1)
        {
            ...
            string str = "SELECT  * " +
                         "FROM pratdt where zid=@zid  and xshift=@xshift and xdate=@xdate ";
                    ...
                    if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM pratdt WHERE zid=" + _zid + " AND xshift='" + xshift.Text.ToString().Trim() + "' AND xdate='" + DateTime.ParseExact(xdate.Text.ToString().Trim(), "dd/MM/yyyy",
                                    CultureInfo.InvariantCulture) + "'";
                                cmd.Parameters.Clear();
