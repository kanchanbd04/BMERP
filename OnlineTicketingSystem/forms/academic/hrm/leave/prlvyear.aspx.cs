...
{
    public partial class prlvyear : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    string xempcat1 = Request.QueryString["xempcat"] != null ? Request.QueryString["xempcat"].ToString().Trim() : "";
                    string xyear1 = Request.QueryString["xyear"] != null ? Request.QueryString["xyear"].ToString().Trim() : "";
                    string xdateeff1 = Request.QueryString["xdateeff"] != null ? Request.QueryString["xdateeff"].ToString().Trim() : "";
                    string xdateexp1 = Request.QueryString["xdateexp"] != null ? Request.QueryString["xdateexp"].ToString().Trim() : "";

                    if (xempcat1 != "" && xyear1 != "" && xdateexp1 != "" && xdateeff1 != "")
                    {
                        xempcat.Text = xempcat1;
                        xyear.Text = xyear1;
                        xdateeff.Text = xdateeff1;
                        xdateexp.Text = xdateexp1;
        ...
        public void fnRegisterPostBack()
        {
            ...
            {
                LinkButton lnkFull1 = row.FindControl("xyear") as LinkButton;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xempcat.Text == "" || xempcat.Text == string.Empty || xempcat.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Employee Category</li>";
                    isValid = false;
                }
                ...
                if (xdateeff.Text == "" || xdateeff.Text == string.Empty || xdateeff.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Effectivity Date</li>";
                    isValid = false;
                }
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xsup"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                               "INSERT INTO prlvyear (zid,ztime,xempcat,xyear,xdateeff,xdateexp) " +
                               "VALUES(@zid,@ztime,@xempcat,@xyear,@xdateeff,@xdateexp) ";
                            ...
                            cmd.Parameters.AddWithValue("@xdateeff", xdateeff1);
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xsupp == "")
                        {
                            ...
                            query = "UPDATE prlvyear SET zutime=@zutime,xdateeff=@xdateeff, xdateexp=@xdateexp " +
                                   "WHERE zid=@zid AND xempcat=@xempcat AND xyear=@xyear ";
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " xempcat,xyear,xdateeff,xdateexp " +
                       "FROM prlvyear WHERE zid=@zid " +
                       "order by xempcat,xyear ";
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                string xyear1 = ((LinkButton)sender).Text;

                fnFillControl(xempcat1, xyear1);
        ...
        private void fnFillControl(string xempcat1, string xyear1)
        {
            message.InnerHtml = "";
            ...
            string str = "SELECT  * " +
                         "FROM prlvyear where zid=@zid  and xempcat=@xempcat and xyear=@xyear ";
            ...
            xempcat.Text = dttemp.Rows[0]["xempcat"].ToString();
                    ...
                    if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM prlvpolicy WHERE zid=" + _zid + " AND xempcat='" + xempcat.Text.ToString().Trim() + "' AND xyear='" + xyear.Text.ToString().Trim() + "'";
                                cmd.Parameters.Clear();
        ...
        protected void btnDetails_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ViewState["xsup"] != null)
                {
                    string xempcat1 = xempcat.Text.Trim().ToString();
                    string xyear1 = xyear.Text.Trim().ToString();
                    string xdateeff1 = xdateeff.Text.Trim().ToString();
                    string xdateexp1 = xdateexp.Text.Trim().ToString();

                    string url =
                        string.Format(
                            "~/forms/academic/hrm/leave/prlvpolicy.aspx?subnav=myDropdown20&link=LinkButton20&xid=200001010&xempcat={0}&xyear={1}&xdateeff={2}&xdateexp={3}",
