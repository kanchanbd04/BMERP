...
{
    public partial class prleave_u : System.Web.UI.Page
    {
        ...
        //        }
                ...
                if (!IsPostBack)
                {
                    ...
                    zglobal.fnGetComboDataCD("Other Leave Type", xleave);
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xsupp == "")
                        {
                            ...
                            query = "UPDATE prleave SET zutime=@zutime,xemail=@xemail, xdate=@xdate, " +
                                   ...
                                   "xstatleave=@xstatleave,xintime=@xintime,xouttime=@xouttime " +
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                        "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " xemp,xrow,xleave,xfdate,xtdate,xleavetkn,xpflag,zemail,xemail " +
                      "FROM prleave WHERE zid=@zid and xstatuslv like 'Unallocated%' and xleave<>'' and xpflag='New' " +
                      "order by xdate desc";
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    //string query =
                    //     "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " xemp,xrow,xleave,xfdate,xtdate,xleavetkn,xpflag,zemail,xemail " +
                    ...
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " xemp,xrow,xleave,xfdate,xtdate,xleavetkn,xpflag,zemail,xemail " +
                       "FROM prleave WHERE zid=@zid and xstatuslv like 'Unallocated%' and xleave='" + xleave.Text.ToString() + "' and xyear='" + curyear + "' and xemp='" + xemp.Text.Trim().ToString() + "' and xpflag='Approved' " +
                       "order by xemp,xrow desc";
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(TextBox2.Text.Trim().ToString()) + " xemp,xrow,xleave,xfdate,xtdate,xleavetkn,xpflag,zemail,xemail " +
                       "FROM prleave WHERE zid=@zid and xstatuslv like 'Unallocated%'  and  xpflag='Approved' " +
                       "order by xdate desc";
                    ...
                    if (txtconformmessageValue.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            ...
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "UPDATE prleave set xpflag = 'Approved',xapprover=@xcheck,xdateapp=@xdatechk " +
