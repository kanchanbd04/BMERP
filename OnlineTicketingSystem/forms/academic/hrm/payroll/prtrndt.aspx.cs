...
{
    public partial class prtrndt : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts = new DataSet())
                    {
                        con.Open();
                        string query =
                            "SELECT * FROM prtrndt WHERE zid=@zid AND xpaycode=@xpaycode and xpaydate=@xpaydate and xemp=@xemp ";
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xpaycode"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                               "INSERT INTO prtrndt (zid,ztime,xpaydate,xemp,xpaycode,xamount,zemail,xtype,xoption,xrem) " +
                               "VALUES(@zid,@ztime,@xpaydate,@xemp,@xpaycode,@xamount,@zemail,'','',@xrem) ";
                            ...
                            cmd.Parameters.AddWithValue("@xamount", Convert.ToDecimal(xamount.Text.Trim()));
                            cmd.Parameters.AddWithValue("@xrem", xrem.Text.Trim().ToString());
                            cmd.ExecuteNonQuery();
                        }
                    }
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xsupp == "")
                        {
                            ...
                            query = "UPDATE prtrndt SET zutime=@zutime,xamount=@xamount, xrem=@xrem, xemail=@xemail " +
                                    "WHERE zid=@zid AND xpaydate=@xpaydate AND xpaycode=@xpaycode and xemp=@xemp";
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " *,(select xdesc from prpayhead where zid=prtrndt.zid and xpaycode=prtrndt.xpaycode) as xdesc " +
                       "FROM prtrndt WHERE zid=@zid and xpaydate=@xpaydate and xemp=@xemp " +
                       "order by xpaycode ";
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                fnFillControl(xpaydate1, xemp1,xpaycode1);
        ...
        private void fnFillControl(string xpaydate1, string xemp1, string xpaycode1)
        {
            message.InnerHtml = "";
            ...
            string str = "SELECT  * " +
                         "FROM prtrndt WHERE zid=@zid and xpaydate=@xpaydate and xemp=@xemp  and xpaycode=@xpaycode ";
            ...
            xrem.Text = dttemp.Rows[0]["xrem"].ToString();
                      ...
                      "zid=" + _zid + " AND xpaydate>='" + DateTime.ParseExact(xpaydate1, "dd/MM/yyyy", CultureInfo.InvariantCulture) + "' and xflags='1'");
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM prtrndt WHERE zid=" + _zid + " AND xemp='" + xemp1
                                    + "' and xpaydate='" + DateTime.ParseExact(xpaydate1, "dd/MM/yyyy", CultureInfo.InvariantCulture) + "' and xpaycode='"+ xpaycode.Text.Trim().ToString() +"'";
                                cmd.Parameters.Clear();
        ...
        protected void btnBack_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string url =
                     string.Format(
                         "~/forms/academic/hrm/payroll/prmsg.aspx?subnav=myDropdown21&link=LinkButton21&xid=205005001&xpaydate={0}&xemp={1}",
                         xpaydate1, xemp1);
