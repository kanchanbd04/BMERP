...
{
    public partial class prmsgacc : System.Web.UI.Page
    {
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                       "SELECT Top " + Int32.Parse(TextBox1.Text.Trim().ToString()) + " *,(select xdesc from prpayhead where zid=practrndt.zid and xpaycode=practrndt.xpaycode) as xdesc, " +
                       "(select xemp + '- ' + xname from hrmst where zid=practrndt.zid and xemp=practrndt.xemp) as xname " +
                       "FROM practrndt WHERE zid=@zid and xpaydate=@xpaydate and xemp=@xemp " +
                       "order by xpaycode ";
                    ...
                    if (tempTable.Rows.Count > 0)
                    {
                        ...
                        GridView3.DataBind();
                        xempid.Text = tempTable.Rows[0]["xname"].ToString();
                    }
                    else
                    {
        ...
        private void fnFillControl(string xpaydate1, string xemp1)
        {
            ...
            //xemail.Text = dttemp.Rows[0]["xemail"].ToString();
            //zemail.Text = dttemp.Rows[0]["zemail"].ToString();
            xadvflag.Text = dttemp.Rows[0]["xadvflag"].ToString();
            xdept.Text = dttemp.Rows[0]["xdept"].ToString();
            xstatusemp.Text = dttemp.Rows[0]["xstatusemp"].ToString();
        ...
        protected void btnBack_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string url =
                         string.Format(
                             "~/forms/academic/hrm/payroll/pracc.aspx?subnav=myDropdown21&link=LinkButton21&xid=205005010&xpaydate={0}",
                             xpaydate1);
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
                        ...
                        if (tempTable.Rows.Count > 0)
                        {
                           fnFillControl(xpaydate1,tempTable.Rows[0]["xemp"].ToString());
                        }
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
                        ...
                        string query =
                             "SELECT top 1 xemp " +
                           "FROM prmsg WHERE zid=@zid and xpaydate=@xpaydate and xemp < '"+xemp.Text.Trim().ToString() +"'" +
                           "order by xemp desc";
