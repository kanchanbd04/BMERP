...
{
    public partial class prpayheaddt : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    string xdesc1 = Request.QueryString["xdesc"] != null ? Request.QueryString["xdesc"].ToString().Trim() : "";

                    spanxpaycode.InnerHtml = xpaycode1;
                    spanxdesc.InnerHtml = xdesc1;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    rtnMessage = rtnMessage + "<li>Pay scale Code</li>";
                    isValid = false;
                }
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts = new DataSet())
                    {
                        con.Open();
                        string query =
                            "SELECT * FROM prpayheaddt WHERE zid=@zid AND xpaycode=@xpaycode and xpsclcode=@xpsclcode ";

                        SqlDataAdapter da = new SqlDataAdapter(query, con);

                        da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                        da.SelectCommand.Parameters.AddWithValue("@xpaycode", xpaycode1);
                        da.SelectCommand.Parameters.AddWithValue("@xpsclcode", xpsclcode.Text.Trim().ToString());
                        ...
                        if (tempTable.Rows.Count > 0)
                        {
                            ViewState["xpsclcode"] = xpsclcode.Text.Trim().ToString();
                        }
                        else
                        {
                            ViewState["xpsclcode"] = null;
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xpsclcode"] == null)
                    {
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                               "INSERT INTO prpayheaddt (zid,ztime,xpsclcode,xpaycode,xpcode,xpercent,xamount,xminpay,xmaxpay,zemail) " +
                               "VALUES(@zid,@ztime,@xpsclcode,@xpaycode,@xpcode,@xpercent,@xamount,@xminpay,@xmaxpay,@zemail) ";
                            ...
                            ViewState["xpsclcode"] = xpsclcode.Text.Trim().ToString();
                            xsupp = xpsclcode.Text.Trim().ToString();
                            ...
                            xpsclcode1 = xpsclcode.Text.ToString();
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xsupp == "")
                        {
                            ...
                            query = "UPDATE prpayheaddt SET zutime=@zutime,xpcode=@xpcode, xpercent=@xpercent, xamount=@xamount, xminpay=@xminpay, xmaxpay=@xmaxpay, xemail=@xemail " +
                                    "WHERE zid=@zid AND xpsclcode=@xpsclcode AND xpaycode=@xpaycode ";
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    ...
                    string query =
                       ...
                       "FROM prpayheaddt WHERE zid=@zid and xpaycode=@xpaycode " +
                       "order by xpaycode,xpsclcode ";
        ...
        protected void FillControl(object sender, EventArgs e)
        {
            try
            {
                ...
                fnFillControl(xpaycode1, xpsclcode1);
        ...
        private void fnFillControl(string xpaycode1, string xpsclcode1)
        {
            message.InnerHtml = "";
            ...
            string str = "SELECT  * " +
                         "FROM prpayheaddt where zid=@zid  and xpaycode=@xpaycode and xpsclcode=@xpsclcode ";
            ...
            xpcode.Text = dttemp.Rows[0]["xpcode"].ToString();
            xpercent.Text = dttemp.Rows[0]["xpercent"].ToString();
            xamount.Text = dttemp.Rows[0]["xamount"].ToString();
            xminpay.Text = dttemp.Rows[0]["xminpay"].ToString();
            xmaxpay.Text = dttemp.Rows[0]["xmaxpay"].ToString();
                    ...
                    if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM prpayheaddt WHERE zid=" + _zid + " AND xpaycode='" + xpaycode1 + "'  and xpsclcode='" + xpsclcode.Text.Trim().ToString() + "'";
                                cmd.Parameters.Clear();
        ...
        protected void btnBack_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string url =
                         string.Format(
                             "~/forms/academic/hrm/payroll/prpayhead.aspx?subnav=myDropdown21&link=LinkButton21&xid=205001005&xpaycode={0}",
                             xpaycode1);
