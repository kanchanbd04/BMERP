...
{
    public partial class lbauthor : System.Web.UI.Page
    {
        private void fnButtonState()
        {
            if (ViewState["xlbauthr"] == null)
            {
            ...
            {
                xlbauthr.Text = ViewState["xlbauthr"].ToString();
        ...
        public void fnRegisterPostBack()
        {
            ...
            {
                LinkButton lnkFull1 = row.FindControl("xlbauthr") as LinkButton;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xtrnaut.Text == "" || xtrnaut.Text == string.Empty || xtrnaut.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Author Prefix</li>";
                    isValid = false;
                }
                ...
                //          "zid=" + _zid + " AND xsup=" + ViewState["xlbauthr"].ToString());
                //    if (xstatus2 != "1-Open")
                //    {
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xlbauthr"] == null)
                    {
                        ...
                        //        con.Open();
                        //        string query =
                        //            "SELECT * FROM lbauthor WHERE zid=@zid AND xname=@xname ";
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                               "INSERT INTO lbauthor (ztime,zid,xtrnaut,xlbauthr,xname,xorg,xmobile,xphone,xfax,xemail1,xcountry,xadd,xdesc02,xrem,zemail) " +
                               "VALUES(@ztime,@zid,@xtrnaut,@xlbauthr,@xname,@xorg,@xmobile,@xphone,@xfax,@xemail1,@xcountry,@xadd,@xdesc02,@xrem,@zemail) ";


                            //xrow = zglobal.GetMaximumIDWithTRN("xcdate", "xrow", "amtfch", xcdate1);
                            //ViewState["xrow"] = xrow;
                            //hiddenxrow.Value = xrow.ToString();
                            xtrnaut1 = xtrnaut.Text.Trim().ToString();
                            xlbauthr1 = zglobal.GetMaximumIDWithTRN(xtrnaut1, "xlbauthr", "lbauthor");
                            ViewState["xlbauthr"] = xlbauthr1;
                            xlbauthrr = xlbauthr1;
                            ...
                            cmd.Parameters.AddWithValue("@xtrnaut", xtrnaut1);
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xlbauthrr == "")
                        {
                            ...
                            query = "UPDATE lbauthor SET zutime=@zutime,xemail=@xemail, xorg=@xorg, " +
                                   ...
                                   "WHERE zid=@zid AND xlbauthr=@xlbauthr ";

                            cmd.Parameters.Clear();

                            cmd.CommandText = query;
                            cmd.Parameters.AddWithValue("@zid", _zid);
                            cmd.Parameters.AddWithValue("@xlbauthr", ViewState["xlbauthr"].ToString());
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " xlbauthr,xname,xorg,xadd " +
                         "FROM lbauthor WHERE zid=@zid  " +
                         "order by xlbauthr asc";
        ...
        private void fnFillControl(string xlbauthr1)
        {
            message.InnerHtml = "";
            ...
            string str = "SELECT  * " +
                         "FROM lbauthor where zid=@zid  and xlbauthr=@xlbauthr ";
            ...
            SqlDataAdapter da = new SqlDataAdapter(str, conn1);
            da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
            da.SelectCommand.Parameters.AddWithValue("@xlbauthr", xlbauthr1);
            da.Fill(dts, "tempztcode");
            ...
            {
                message.InnerHtml = "Wrong author code.";
                message.Style.Value = zglobal.am_warningmsg;
                return;
            }

            ViewState["xlbauthr"] = xlbauthr1.ToString();
            //hiddenxrow.Value = ViewState["xrow"].ToString();

            xlbauthr.Text = dttemp.Rows[0]["xlbauthr"].ToString();
            xtrnaut.Text = dttemp.Rows[0]["xtrnaut"].ToString();
                    ...
                    if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM lbauthor WHERE zid=@zid AND xlbauthr=@xlbauthr";
                                cmd.Parameters.Clear();
