...
{
    public partial class lbpublisher : System.Web.UI.Page
    {
        private void fnButtonState()
        {
            if (ViewState["xlbpubcode"] == null)
            {
            ...
            {
                xlbpubcode.Text = ViewState["xlbpubcode"].ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    zglobal.fnGetComboDataCDTRN("'Publisher Prefix'", xtrnpub);
        ...
        public void fnRegisterPostBack()
        {
            ...
            {
                LinkButton lnkFull1 = row.FindControl("xlbpubcode") as LinkButton;
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (xtrnpub.Text == "" || xtrnpub.Text == string.Empty || xtrnpub.Text == "[Select]")
                {
                    rtnMessage = rtnMessage + "<li>Publisher Prefix</li>";
                    isValid = false;
                }
                if (xname1.Text == "" || xname1.Text == string.Empty || xname1.Text == "[Select]")
                {
                ...
                //          "zid=" + _zid + " AND xsup=" + ViewState["xlbpubcode"].ToString());
                //    if (xstatus2 != "1-Open")
                //    {
                ...
                using (TransactionScope tran = new TransactionScope())
                {
                    ...
                    if (ViewState["xlbpubcode"] == null)
                    {
                        ...
                        //        con.Open();
                        //        string query =
                        //            "SELECT * FROM lbpublisher WHERE zid=@zid AND xname=@xname ";

                        //        SqlDataAdapter da = new SqlDataAdapter(query, con);

                        //        da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                        //        da.SelectCommand.Parameters.AddWithValue("@xname", xname.Text.ToString().Trim());
                        //        //da.SelectCommand.Parameters.AddWithValue("@xtrnpub", xtrnpub.Text.ToString().Trim());
                        ...
                        //        if (tempTable.Rows.Count > 0)
                        //        {
                        //            message.InnerHtml = "Cann't insert duplicate record. Publisher already exists.";
                        //            message.Style.Value = zglobal.warningmsg;
                        //            return;
                        //        }
                        //    }
                        ...
                        using (SqlConnection conn = new SqlConnection(zglobal.constring))
                        {
                            ...
                            string query =
                               "INSERT INTO lbpublisher (ztime,zid,xtrnpub,xlbpubcode,xname,xorg,xmobile,xphone,xfax,xemail1,xcountry,xadd,xdesc02,xrem,zemail) " +
                               "VALUES(@ztime,@zid,@xtrnpub,@xlbpubcode,@xname,@xorg,@xmobile,@xphone,@xfax,@xemail1,@xcountry,@xadd,@xdesc02,@xrem,@zemail) ";


                            //xrow = zglobal.GetMaximumIDWithTRN("xcdate", "xrow", "amtfch", xcdate1);
                            //ViewState["xrow"] = xrow;
                            //hiddenxrow.Value = xrow.ToString();
                            xtrnpub1 = xtrnpub.Text.Trim().ToString();
                            xlbpubcode1 = zglobal.GetMaximumIDWithTRN(xtrnpub1, "xlbpubcode", "lbpublisher");
                            ViewState["xlbpubcode"] = xlbpubcode1;
                            xlbpubcoder = xlbpubcode1;

                            xname11 = xname1.Text.ToString().Trim();
                            xorg1 = xorg.Text.ToString().Trim();
                            //xfirst1 = xfirst.Text.ToString().Trim();
                            xadd11 = xadd1.Value.ToString().Trim();
                            xrem1 = xrem.Value.ToString().Trim();
                            ...
                            cmd.Parameters.AddWithValue("@xtrnpub", xtrnpub1);
                    ...
                    using (SqlConnection conn = new SqlConnection(zglobal.constring))
                    {
                        ...
                        if (xlbpubcoder == "")
                        {
                            ...
                            query = "UPDATE lbpublisher SET zutime=@zutime,xemail=@xemail, xorg=@xorg, " +
                                    "xname=@xname,xadd=@xadd,xcountry=@xcountry," +
                                   "xemail1=@xemail1,xphone=@xphone," +
                                    "xmobile=@xmobile,xfax=@xfax,xrem=@xrem,xdesc02=@xdesc02 " +
                                   "WHERE zid=@zid AND xlbpubcode=@xlbpubcode ";

                            cmd.Parameters.Clear();

                            cmd.CommandText = query;
                            cmd.Parameters.AddWithValue("@zid", _zid);
                            cmd.Parameters.AddWithValue("@xlbpubcode", ViewState["xlbpubcode"].ToString());
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " xlbpubcode,xname,xorg,xadd " +
                         "FROM lbpublisher WHERE zid=@zid  " +
                         "order by xlbpubcode asc";
        ...
        private void fnFillControl(string xlbpubcode1)
        {
            message.InnerHtml = "";
            ...
            string str = "SELECT  * " +
                         "FROM lbpublisher where zid=@zid  and xlbpubcode=@xlbpubcode ";
            ...
            SqlDataAdapter da = new SqlDataAdapter(str, conn1);
            da.SelectCommand.Parameters.AddWithValue("@zid", _zid);
            da.SelectCommand.Parameters.AddWithValue("@xlbpubcode", xlbpubcode1);
            da.Fill(dts, "tempztcode");
            ...
            {
                message.InnerHtml = "Wrong publisher code.";
                message.Style.Value = zglobal.am_warningmsg;
                return;
            }

            ViewState["xlbpubcode"] = xlbpubcode1.ToString();
            //hiddenxrow.Value = ViewState["xrow"].ToString();

            xlbpubcode.Text = dttemp.Rows[0]["xlbpubcode"].ToString();
            xtrnpub.Text = dttemp.Rows[0]["xtrnpub"].ToString();
            xorg.Text = dttemp.Rows[0]["xorg"].ToString();
            xname1.Text = dttemp.Rows[0]["xname"].ToString();
            xadd1.Value = dttemp.Rows[0]["xadd"].ToString();
                    ...
                    if (txtconformmessageValue1.Value == "Yes")
                    {
                        ...
                        using (TransactionScope tran = new TransactionScope())
                        {
                            using (SqlConnection conn = new SqlConnection(zglobal.constring))
                            {
                                ...
                                string query = "DELETE FROM lbpublisher WHERE zid=@zid AND xlbpubcode=@xlbpubcode";
                                cmd.Parameters.Clear();
