...
{
    public partial class lbtrn : System.Web.UI.Page
    {
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dtCurrentTable.Rows.Count - 1; i++)
                    {
                        ...
                        TextBox box3 = (TextBox)GridView1.Rows[i].Cells[3].FindControl("xdate");
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (flag == 0)
                {
                    message.InnerText = "Must Issue at Least One Book.";
                    message.Style.Value = zglobal.warningmsg;
                    return;
                }
                        ...
                        foreach (GridViewRow row in GridView1.Rows)
                        {
                            ...
                            if (xlbord1234.Text != string.Empty)
                            {
                               ...
                               : new DateTime(2999, 12, 31));
                                ...
                                cmd.Parameters.AddWithValue("@xlbmemcode", xlbmemcode1);
                                cmd.Parameters.AddWithValue("@xlbmemtype", xlbmemtype1);
                                cmd.Parameters.AddWithValue("@xlbstatus", "Unavailable");
                                cmd.Parameters.AddWithValue("@xlbrfdate", xlbrfdate12.Text.Trim().ToString() != string.Empty
                               ? DateTime.ParseExact(xlbrfdate12.Text.ToString().Trim(), "dd/MM/yyyy",
                                   CultureInfo.InvariantCulture)
                               : new DateTime(2999, 12, 31));
        ...
        private void fnFillGrid2()
        {
            ...
            using (SqlConnection con = new SqlConnection(zglobal.constring))
            {
                using (DataSet dts1 = new DataSet())
                {
                    con.Open();
                    string query =
                         "SELECT Top " + Int32.Parse(_gridrow.Text.Trim().ToString()) + " xlbtrncode,xlbmemcode,ztime,zemail, " +
                         "(select xname from lbmemlistvw where zid=lbtrn.zid and xlbmemcode=lbtrn.xlbmemcode and xtype=lbtrn.xlbmemtype) as mem_name " +
                         "FROM lbtrn WHERE zid=@zid  " +
                         "order by xlbtrncode desc";
                    ...
                    else
                    {
                        // btnShowList.Visible = false;
                        //pnllistct.Visible = false;
                        dgvgenerateduesnew.DataSource = null;
                        dgvgenerateduesnew.DataBind();
                    }
            ...
            if (xlbmemcode.Text.Trim().ToString() != "")
            {
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
                        con.Open();
                        string query =
                             "SELECT Top " + Int32.Parse(TextBox2.Text.Trim().ToString()) + " xlbtrncode,xlbmemcode,ztime,zemail, " +
                             "(select xname from lbmemlistvw where zid=lbtrn.zid and xlbmemcode=lbtrn.xlbmemcode and xtype=lbtrn.xlbmemtype) as mem_name " +
                             "FROM lbtrn WHERE zid=@zid  and xlbmemcode=@xlbmemcode and xlbmemtype=@xlbmemtype " +
                             "order by xlbtrncode desc";

                        SqlDataAdapter da1 = new SqlDataAdapter(query, con);

                        da1.SelectCommand.Parameters.AddWithValue("@zid", _zid);
                        da1.SelectCommand.Parameters.AddWithValue("@xlbmemcode", xlbmemcode.Text.ToString().Trim());
                        da1.SelectCommand.Parameters.AddWithValue("@xlbmemtype", hxlbmemtype.Value.ToString().Trim());
                ...
                using (SqlConnection con = new SqlConnection(zglobal.constring))
                {
                    using (DataSet dts1 = new DataSet())
                    {
                        con.Open();
                        string query =
                             "SELECT Top " + Int32.Parse(TextBox1.Text.Trim().ToString()) + " xlbtrncode,xlbord,xdate,xlbrfdate, " +
                             "(select xlbtitle from lbbook where zid=lbtrndt.zid and xlbord=lbtrndt.xlbord ) as xlbtitle " +
                             "FROM lbtrndt WHERE zid=@zid  and xlbmemcode=@xlbmemcode and coalesce(xlbstatus,'')='' and xlbmemtype=@xlbmemtype and (select coalesce(xdesc01,'') from lbbook where zid=lbtrndt.zid and xlbord=lbtrndt.xlbord)='' " +
                             "order by xlbtrncode desc";
            ...
            //        string query =
            //           "SELECT Top " + Int32.Parse(_gridrow1.Text.Trim().ToString()) + " xlbtrncode,xdate,xfwh,xaction,xstatustrn " +
