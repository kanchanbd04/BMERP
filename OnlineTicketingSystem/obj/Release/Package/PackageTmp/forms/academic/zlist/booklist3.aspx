<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="booklist3.aspx.cs" Inherits="OnlineTicketingSystem.forms.BMERP.booklist3"
    ...
    <script type="text/javascript">
        ...
        function update(str) {
         ...
         var ctlid = $("#<%= ctlid_v.ClientID%>").val();
            ...
            var ctlid3 = $("#<%= ctlid3_v.ClientID%>").val();
            var ctlid4 = $("#<%= ctlid4_v.ClientID%>").val();
            var ctlid5 = $("#<%= ctlid5_v.ClientID%>").val();
            var ctlid6 = $("#<%= ctlid6_v.ClientID%>").val();
            //alert(ctlid);
            var datepart = res[5].split("/");
            var dateObject = datepart[1].padStart(2, "0").toString() + "/" + datepart[0].padStart(2, "0").toString() + "/" + datepart[2].substring(0, 4).toString();
            window.opener.update11(res[0], res[1], res[2], res[3], res[4], dateObject, res[6], ctlid, ctlid1, ctlid2, ctlid3, ctlid4, ctlid5, ctlid6);
            self.close();
        }
    </script>
</head>
<body>
    <form id="form1" runat="server">
                            ...
                            <asp:GridView ID="_grid_emp" runat="server" AutoGenerateColumns="False" ShowFooter="true"
                                ...
                                <Columns>
                                    <asp:TemplateField HeaderText="Book Code">
                                        ...
                                        <ItemTemplate>
                                            <asp:LinkButton ID="xlbord" runat="server" Text='<%#Eval("xlbord") %>' OnClientClick='<%# String.Format("update(\"{0}\");", Eval("xlbord")+"|"+Eval("xlbtitle")+"|"+Eval("xlbmemcode")+"|"+Eval("mem_name")+"|"+Eval("xlbtrncode")+"|"+Eval("xlbrfdate")+"|"+Eval("xlbmemtype")) %>' CssClass="_gridrow_link"></asp:LinkButton>
                                        </ItemTemplate>
                                        <FooterTemplate>
        ...
        <asp:HiddenField ID="ctlid3_v" runat="server" />
        <asp:HiddenField ID="ctlid4_v" runat="server" />
        <asp:HiddenField ID="ctlid5_v" runat="server" />
        <asp:HiddenField ID="ctlid6_v" runat="server" />
    </form>
</body>
</html>
