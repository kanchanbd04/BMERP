<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/academic_site.Master"
    CodeBehind="amtfcadmission1.aspx.cs" Inherits="OnlineTicketingSystem.forms.BMERP.amtfcadmission1"
    ...
    <script type="text/javascript">
      <%--  function fnPrintNumCandidate() {
        ...
        function fnTFCAdmission(link) {
            ...
            var searchParams = new URLSearchParams(_url.search);
            //alert(searchParams.get('subnav'));
            //return false;
            <%-- // alert("Hi");
            var xclass1 = $("#<%= xclass.ClientID%>").val();
                 var xsession1 = $("#<%= xsession.ClientID%>").val();
         ...
         var grid = document.getElementById('<%=_grid_emp.ClientID %>');
            ...
            //alert(grid.rows[rowIndex + 1].cells[grid.rows[rowIndex + 1].cells.length - 4].children[0].innerText);
            //return false;
            var left = Math.round((screen.width / 2) - (1235 / 2));
            var top = Math.round((screen.height / 2) - (650 / 2));
            var xrow = grid.rows[rowIndex + 1].cells[grid.rows[rowIndex + 1].cells.length - 4].children[0].innerText;
            var openwin = window.open("/forms/academic/collection/amtfcadmission.aspx?subnav=" + searchParams.get('subnav') + "&link=" + searchParams.get('link') + "&xrow=" + xrow, 'tfcadmission', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width=1235, height=650, top=' + top + ', left=' + left + ', targets=_blank');
            openwin.focus();
                 return false;
        }
    </script>
                        ...
                        <asp:GridView ID="_grid_emp" runat="server" AutoGenerateColumns="False" ShowFooter="true"
                            ...
                            <Columns>
                                ...
                                <asp:TemplateField HeaderText="Ref.">
                                    ...
                                    <ItemTemplate>
                                        <asp:LinkButton ID="xrow" runat="server" Text='<%#Eval("xrow") %>' CssClass="_gridrow_link"
                                            OnClientClick="fnTFCAdmission(this);"></asp:LinkButton>
                                ...
                                <asp:BoundField DataField="xcontact" HeaderText="Contact No." ItemStyle-Width="300px"
                                    ItemStyle-HorizontalAlign="Center" ItemStyle-CssClass="pad5 hiddencol" FooterStyle-CssClass="hiddencol" HeaderStyle-CssClass="hiddencol"/>
                                <asp:BoundField DataField="xstdid" HeaderText="Student ID" ItemStyle-Width="100px" ItemStyle-HorizontalAlign="Center"
                                    ItemStyle-CssClass="pad5" />
                                ...
                                <asp:BoundField DataField="xstatus" HeaderText="Status" ItemStyle-Width="100px" ItemStyle-HorizontalAlign="Center"
                                    ItemStyle-CssClass="pad5" />
                                  <asp:BoundField DataField="xtransaction" HeaderText="Transaction No." ItemStyle-Width="120px" ItemStyle-HorizontalAlign="Center"
                                    ItemStyle-CssClass="pad5" />
                                <asp:TemplateField>
                                    <ItemTemplate>
