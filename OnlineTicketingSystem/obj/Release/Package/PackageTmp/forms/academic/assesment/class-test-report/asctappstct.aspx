<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/academic_site.Master"
    CodeBehind="asctappstct.aspx.cs" Inherits="OnlineTicketingSystem.forms.BMERP.asctappstct"
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server">
        <ContentTemplate>
            ...
            $("body").on("click", ".bm_student", function () {
             ...
             $("body").on("click", ".bm_circle_button_class", function () {
                 //alert("Hi");
                 ...
                 //var left = Math.round((screen.width / 2) - (1100 / 2));
                 //var top = Math.round((screen.height / 2) - (600 / 2));
                 //var openwin = window.open("/forms/academic/assesment/mark-input/amexamh123.aspx?subnav=myDropdown44&link=LinkButton44&xsession=" + xsession1.trim() + "&xterm=" + xterm1.trim() + "&xclass=" + xclass1.trim() + "&xgroup=" + xgroup1.trim() + "&xsection=" + xsection1.trim() + "&xsubject=" + xsubject1.trim().replace(/&/gi, "%26") + "&xpaper=" + xpaper1.trim() + "&xext=" + xext1.trim() + "&xcttype=" + xcttype1.trim() + "&xctno=" + xctno1.trim(), 'ctstct', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width=1100, height=600, top=' + top + ', left=' + left + ', targets=_blank');
                 var openwin = window.open("/forms/academic/assesment/mark-input/amexamh1.aspx?subnav=myDropdown44&link=LinkButton44&xsession=" + xsession1.trim() + "&xterm=" + xterm1.trim() + "&xclass=" + xclass1.trim() + "&xgroup=" + xgroup1.trim() + "&xsection=" + xsection1.trim() + "&xsubject=" + xsubject1.trim().replace(/&/gi, "%26") + "&xpaper=" + xpaper1.trim() + "&xext=" + xext1.trim() + "&xcttype=" + xcttype1.trim() + "&xctno=" + xctno1.trim(), 'ctstct', 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width='+screen.width+', height='+screen.height+', top=0, left=0, targets=_blank');
                 openwin.focus();
            ...
            <div class="bm_academic_container">
                <div class="bm_academic_container_header">
                    <div class="header_label1" id="header_label" runat="server">
                        <span class="bm_am_header_round">Class Test Approval Status (Class Teacher)</span>
                    </div>
                </div>
